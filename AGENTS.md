...
**Evidence-completeness leg (2026-08-02, user-directed)**: the closeout is written ONCE, after the evidence is complete — the full pack, the reviewer's own probes, AND the report of any independent agent this review launched. The Stop hook enforces the agent half by reading the session transcript: a review cannot close while an `Agent` launched after arming has not reported back. If that agent genuinely died or was abandoned, the same `Verify` line must carry `agent-aborted:<原因>` — waiting is the default, declaring is the exception. Rationale: writing register / SESSION_LOG / handoff while an agent was still running cost three full rewrites in one round and published a "verified good, do not break this" claim that the agent's report then refuted; a wrong entry in that section is worse than no entry. Rule 7's wall clock parallelises the RUNNING of evidence, never the WRITING of the closeout.
