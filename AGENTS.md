...
- 🟡 Phase 7b-2 P1 US evidence snapshots + readiness review matrix + access/sample plan：六份 P1 snapshot 已基于官方 SEC / Nasdaq / MSCI / S&P DJI / S&P Global / LSEG / Massive / Polygon / Norgate / Intrinio / FMP 文档记录 candidate evidence；`schemas/provider_p1_readiness_review.schema.json` / `docs/provider_evidence_p1_us_readiness_review_matrix_20260529.json` 已固化 field-by-field blocker disposition；`schemas/provider_p1_access_decision_plan.schema.json` / `docs/provider_evidence_p1_us_access_decision_sample_validation_plan_20260531.json` 已把 cost ceiling、access path、license / storage、sample rows、coverage counts、fallback / incident gate 固化为 plan-only artifact。Approved spend = 0；仍不授权 provider selection / contact / token / trial / paid access / sample fetch / data fetch / Phase 7c；A-share `minimal_data_burst` 原 preregistration 在 `research/preregistrations/a_share_minimal_data_burst_20260531.json` 且继续 `BLOCKED_DO_NOT_RUN`；corrected-basis supersession 在 `research/preregistrations/a_share_minimal_data_burst_corrected_basis_20260531.json`，只能在 review + commit 后作为下一条 research-only falsification 运行。
...
- `schemas/research_preregistration.schema.json` / `research/preregistrations/a_share_minimal_data_burst_20260531.json` / `research/preregistrations/a_share_minimal_data_burst_corrected_basis_20260531.json` — research-only preregistration contract, superseded blocked artifact, and corrected-basis A-share `minimal_data_burst` artifact（只能作为 research-only falsification 运行；不进 production、不声称 ship-gate evidence）
