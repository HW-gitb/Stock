...
Codex must use `using-superpowers` when available before `执行` / `修复` to enter the correct workflow. Codex must complete the main-thread A-F self-review before handing work to Claude Code for `审查`; an independent agent is conditional under verification-tiering rule 7, never a default tax on a small bounded change. Record the selected self-review path and short proof in the handoff entry. The rule is: handoff commands belong in `docs/SESSION_LOG.md` / `docs/system_risk_register.md`, not in the final chat; final chat should not repeat long operational commands unless the user asks.
...
0. **Executor/fixer critical path — fixed order, conditional gates**: obtain the cheapest deterministic reproduction evidence (`existing failing focused test` > `direct deterministic probe` > `new focused regression test`), make the minimum repair, validate shared contracts/consumers only when the changed surface crosses that seam, run the final focused acceptance pack after the last behavior/contract edit, complete main-thread A-F, and freeze the behavior/contract diff. Then run an independent agent only if rule 7 requires it, and run one atomic full-ledger command only if rule 3 requires it; otherwise omit those steps. A new red test is not mandatory when existing evidence already reproduces the defect, but a durable regression test is mandatory when it materially prevents the same behavior/contract defect from recurring. No step may be added merely for ceremony, a nicer test count, or because an older handoff used it. The detailed execution/self-review method is single-sourced in `docs/pre_codex_self_review_checklist.md`.
...
4. **One complete run for one unchanged final diff**: when rule 3 triggers, exactly one actor runs the full lane pack after the last behavior/contract edit (default: executor before handoff; reviewer may explicitly take ownership instead). The reviewer independently reruns only rule-1 acceptance tests. Use the single command `.tools\codex_main_python.ps1 .tools\full_pack_ledger.py run <lane> "<trigger>" "<focused evidence>" <timeout-seconds> -- <unittest args>`: it checks the exact-code-state cache, binds the A-F preparation, enforces the deadline, validates the terminal exit and `Ran N tests`, and records only PASS. Manual `prepare`/`record` is retired. Only an exact-fingerprint record written by this prepare-bound `run` path is reusable; a legacy green record without `prepared_fingerprint` is historical evidence, not closeout PASS. A later behavior/contract edit invalidates that run; docs/register/SESSION_LOG-only edits do not.
5. **Bounded process evidence, never process folklore**: every executor/reviewer focused unittest command uses `.tools\run_unittest_with_repo_pythonpath.cmd <unittest args>`; that launcher uses only the hard-coded project Python, clears inherited interpreter overrides, delegates to `.tools\bounded_unittest.py`, and enforces a 300-second maximum. It does not pay an unconditional import probe or full A-short preflight tax before every small test; run `runners/a_short_preflight.py` once only when the changed surface is the A-short runtime/dependency/resolver itself. A rule-3 full run uses rule 4 and has a 1200-second maximum. Do not raise either cap without explicit user approval. On timeout the tool terminates only its own child process tree and reports `TIMEOUT`; do not scan, poll, or kill unrelated Python processes. A PID, CPU change, process existence, or parent/child switch is diagnostic only and never proves test progress, a “subcase switch”, or PASS. Only actual test output plus terminal exit can do that. A zero-test, still-running, timed-out, interrupted, missing-exit, or missing-`Ran N tests` run is `UNKNOWN`/FAIL evidence: stop, narrow or diagnose once, and never wait repeatedly. One knife may not spend more than the 20-minute full-run ceiling waiting on tests; a 40-minute repair loop is a process failure, not acceptable diligence.
...
3. Use `.tools/run_unittest_with_repo_pythonpath.cmd`, which always uses the pinned host Python and bounds the owned unittest process. It intentionally does not run an import probe or full A-short preflight before every small test; the selected test must fail normally if its dependency is missing, and `runners/a_short_preflight.py` is an additional targeted check only when the A-short runtime/dependency/resolver surface changes. Codex must not use its bundled/PATH Python or an interpreter override. When a Codex sandbox blocks the host executable, request approved host-process execution instead of substituting another interpreter. Never accept a silent schema skip.
