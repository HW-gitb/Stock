...
3. Use the repo test wrapper launcher `.tools/run_unittest_with_repo_pythonpath.cmd`, which always uses the pinned dependency-complete host Python and runs the full A-short preflight before tests. The launcher keeps `.tools/python_libs` on `PYTHONPATH` for the repo's `jsonschema` compatibility layer. Codex must not use its bundled/PATH Python or an interpreter override. When a Codex sandbox blocks the host executable, invoke `.tools\codex_main_python.ps1` with the tool's approved host-process execution instead of substituting another interpreter. If the wrapper cannot make `jsonschema` or any A-short dependency importable, stop; do not accept silent schema-skip behavior.
