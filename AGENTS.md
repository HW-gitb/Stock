...
7. **Pre-Codex self-review gate (run before EVERY `起草`/`修复` handoff; detail: `docs/pre_codex_self_review_checklist.md`).** Repeated avoidable round-trips came from fixing only the named instance and not tracing a fix's ripple. Before handing to `审查`:
   ...
   - **B ripple grep(exhaustive + evidenced)**: after any behavior / symbol / mechanism / **rule-wording** change, run **ONE 全仓 grep of the OLD form across ALL surface types**,改完所有命中,并把**零残留证据**(实际 `rg` 命令 + `0 hits`)贴进 proof-of-use 行——不是"我 grep 了",是"0 残留"。必扫的面(本会话反复漏的两类已显式列出):① 旧符号名(重命名/删除的常量/函数/字段);② **所有断言旧行为的文字**——不止 .md,含 **runner docstring/注释、`test_*` 的 docstring 与注释、以及 emit 到输出的字符串字面量**(如 `machine.consumption.*`、log/print、面板/用户可见文案);③ 下游消费者(跨 runner import / weekly pipeline / 面板 / schema 消费方)。**已跨轮复发的规则 → 必须加一条全仓 guard 测试禁旧形态**(B2 单一来源/守护用到整棵树),让 partial fix 直接测试红、不靠记忆。教训:本会话同一 evidence-full 规则因只扫 runner+.md、漏 emit 字符串与 test docstring,被 Codex 同 R-ID re-FAIL 3 轮。
