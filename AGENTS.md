...
5. **Bounded process evidence, never process folklore**: every executor/reviewer focused unittest command uses `.tools\run_unittest_with_repo_pythonpath.cmd <unittest args>`; that launcher uses only the hard-coded project Python, clears inherited interpreter overrides, delegates to `.tools\bounded_unittest.py`, verifies `jsonschema` once, and has a default focused deadline of 300 seconds. A known slow focused pack may explicitly use `.tools\run_unittest_with_repo_pythonpath.cmd --timeout-seconds <N> <unittest args>` for `301 <= N <= 1300`; `Verify` must name the measured reason. Neither the 1300-second ceiling nor the 300-second focused default may be raised without explicit user approval, and a launcher usage error must exit non-zero — a message with no tests run may never report success. It does not pay a full A-short preflight tax before every small test; run `runners/a_short_preflight.py` once only when the changed surface is the A-short runtime/dependency/resolver itself. A rule-3 full run uses rule 4 and has a 1300-second maximum. On timeout the tool terminates only its own child process tree and reports `TIMEOUT`; do not scan, poll, or kill unrelated Python processes. A PID, CPU change, process existence, or parent/child switch is diagnostic only and never proves test progress, a “subcase switch”, or PASS. Only actual test output plus terminal exit can do that. A zero-test, still-running, timed-out, interrupted, missing-exit, or missing-`Ran N tests` run is `UNKNOWN`/FAIL evidence: stop, narrow or diagnose once, and never wait repeatedly. One knife may not spend more than the 22-minute full-run ceiling waiting on tests; a 40-minute repair loop is a process failure, not acceptable diligence.
