...
27. 2026-06-03 A-short steady alpha re-audit 只是预注册，不是结果：`schemas/a_short_steady_alpha_reaudit_preregistration.schema.json` / `research/preregistrations/a_short_steady_alpha_reaudit_20260603.json` / `research/ledgers/a_short_steady_alpha_reaudit_program_test_budget_ledger_20260603.json` 冻结一次 same-anchor 5d / 20d CSI1000 / CSI300 复审，以及 multiple-testing / slice / factor / veto / PIT 检查。该 artifact 不授权 outcome run、抓数据、重跑 EGS、改 runner、进 DataHub、production claim、ship-gate claim 或 full-size 手动使用；后续真正跑结果必须先通过独立审查并获得用户新的 `执行` 指令。即使复审为正，也只能是 candidate alpha clue，仍需固定 12 个月 forward-live ship gate。
