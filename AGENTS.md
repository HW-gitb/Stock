...
**`审查` 命令最终输出固定三段（2026-06-23 用户更新；2026-06-24 `下一步` 简化；2026-06-25 reviewer/committer PASS 后自动提交；仅影响对话框最终回复，落盘文档规则不变）**：仅当用户明确下达 `审查` 命令或要求按审查流程收口时，面向用户的最终结论只写三块，顺序固定为：`Verdict`、`Required / Optional / Options`、`下一步`。**三段之间各留一个空行；每个段落之间也各留一个空行，便于阅读。**必须用中文；每块都要极简，不铺背景、不复述流程、不堆文件清单；不输出 `Findings` 段，不单列“已验证 / Verify / 验证”项，也不要另起“覆盖范围 / 运行阻塞 / 结论边界”等额外栏目。`Verdict` 第一位，直接写 PASS / FAIL / 未完全验证等结论，并在下一行用 `大白话：...` 说明能不能过；`Required / Optional / Options` 第二位，只写具体审查结果，每条具体项必须包含必要的技术标识和技术现状，并在下一行用 `大白话：...` 解释要修什么、可选什么或怎么选；`下一步` 第三位，只写一行最简单给另一个 LLM 的命令（例：`Codex：Pass`、`Codex：修复`、`Codex：执行`），不写 `大白话`、不写解释、不写修复细节；具体指示必须放在 `docs/SESSION_LOG.md` / `docs/system_risk_register.md`。`审查` PASS 后 reviewer/committer 必须在最终回复前自动提交已审查工作树，`下一步` 不再写 `Codex：提交`；Codex 只负责实现/修复。没有对应内容时写“无”。**空类别只写“无”，一个字都不加（2026-07-29 用户更新）**：`Required` / `Optional` / `Options` 任一类别没有内容时只写“无”，不得附 `大白话`、不得解释为什么没有、不得列已闭合项 / 已通过项 / 本轮修好的东西 / 证据摘要 / 过程取舍。已闭合与通过的内容只写进 `docs/system_risk_register.md` / `docs/SESSION_LOG.md`；`大白话` 只出现在有内容的具体项上。
...
**普通执行/修复最终回复三段式（非审查，2026-07-04 用户更新）**：用户下达 `执行` / `修复` / 普通实现收口时，最终回复只写三段，且不要套用 `审查` 的 `Verdict` / `Required / Optional / Options` / `下一步` 三段模板。顺序固定为：① `大白话`：用最简洁的人话说明这轮做了什么；② `给 Claude Code 的命令`：只写最短命令词（如 `审查`、`复审`、`提交`；若不需要则写“无”），不写解释、不写 `Claude Code：...` 长句；③ `下一步我需要做什么`：写当前业务线/设计里的实质下一步（例如 US-short 设计中下一块要执行的内容），不要重复“等 Claude 审查 / PASS 后提交”这类流程话。验证证据仍按上一条规则落 `SESSION_LOG` / register / artifact；如聊天必须提，只能压进第一段一句话，不另起验证段。此规则只约束 executor/fixer 的普通最终回复，不改变 `审查` 结论输出标准。
...
Codex must use `using-superpowers` when available before `执行` / `修复` to enter the correct workflow. Codex must follow the checklist's single-window independent self-review schedule before handing work to Claude Code for `审查`, then record the short proof in the handoff entry. The rule is: handoff commands belong in `docs/SESSION_LOG.md` / `docs/system_risk_register.md`, not in the final chat; final chat should not repeat long operational commands unless the user asks.

Process-speed defaults (user-requested 2026-07-02) — narrowed by user 2026-07-18: both LLMs save time by sharing fixed verification packs, without treating these defaults as new authorization. Use `docs/process` = focused guard first, then `.tools/verify_doc_process.cmd` before handoff / commit (**deleted from the review path 2026-07-27**: the reviewer already runs the doc-governance guard once at closeout; running it a second time mid-review is a deleted step); schema = schema focused test + direct bad-input probe + adjacent consumer when touched. The detailed, authoritative test-tier and no-duplicate-full-run rule is immediately below; do not infer a lane-wide full pack merely from the word `审查`.

### Verification tiering and one-full-run rule (user-directed 2026-07-18)

Test scope follows the **changed surface**, never the actor or the word `审查`.

1. **Acceptance focus pack — mandatory twice**: the executor runs the new/changed module's direct tests, directly reused consumer/dependency tests, relevant schema or document guards, and static checks (`py_compile` / `git diff --check` as applicable) after the last behavior or contract edit. The pack must explicitly name every touched schema and effect-contract guard; they are not inferred automatically from the lane name. Before PASS, Claude Code personally reruns that same focused acceptance pack from the final diff; this is the independent test obligation.
   **Static conformance is a direct consumer when its AST/registry predicate intersects the changed symbol**: include that guard in the focused pack. In particular, an edit to a US-short producer that adds or moves an item loop or `raise` must include `tests.test_us_short_discovery_conformance.LanePerItemConformance`; this is contract-based, not a filename-wide test tax.
2. **Affected-lane pack — conditional**: add the directly affected lane/consumer pack when a shared engine, shared schema, entrypoint, persistence path, or runtime configuration is changed. Do not add unrelated lane tests merely because they share a repository or a historical risk-register entry.
3. **Full lane regression — exceptional, not default**: require it only when (a) a production top-level runner/entrypoint is changed (for example `runners/a_short_weekly_pipeline.py` or `A-EGS/egs_main.py`), (b) a shared engine/schema can change multiple production consumers, (c) provider, credential, authorization, raw-payload, live-data, or account/position safety is involved, (d) focused tests cannot bound the impact, or (e) the user explicitly asks for full regression. A new isolated private module with no production wiring does not meet this threshold solely because it is new.
4. **One complete run for one unchanged final diff**: when rule 3 triggers, exactly one actor runs the full lane pack after the last behavior/contract edit, and **that actor is the executor/fixer, before handoff**. **The reviewer does NOT re-run it** — it cites the executor's ledger record and independently reruns only rule-1 acceptance tests (user-directed 2026-08-04; the earlier "reviewer may explicitly take ownership instead" option is retired). A reviewer re-run is only the rule-6 escalation, with its reason named in `Verify` — including the merge-state carve-out defined there, since a merge on its own never justifies one. And rule 3 is a threshold, not an invitation: **if a full lane is not actually required, it is not run** — an unnecessary full lane is over-verification under rule 8, not diligence. Use the single command `C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe .tools\full_pack_ledger.py run <lane> "<trigger>" <timeout-seconds> -- <unittest args>`: it checks the exact-code-state cache, binds the A-F preparation, runs `git diff --check` and `py_compile` before the lane, enforces the deadline, validates the terminal exit and `Ran N tests`, and records only PASS. A focused receipt may exist as local audit metadata, but its absence, age, corruption, or replacement never blocks full-pack execution and never invalidates a matching cached green. This direct form is mandatory because PowerShell consumes a bare `--` before `codex_main_python.ps1` can forward it. `.tools\codex_main_python.ps1` remains the strict host-Python entrypoint for command vectors that do not contain a literal `--`; rule 5 remains the focused-test entrypoint. Invoke either argument vector directly; do not wrap it in `Start-Process` or concatenate it into one `ArgumentList` string. Manual `prepare`/`record` is retired. A later behavior/contract edit invalidates that run; docs/register/SESSION_LOG-only edits do not.
   The ledger internally adds only `unittest`'s fixed `-b -f --durations 25` runtime flags: buffering suppresses passing-test chatter, fail-fast stops at the first real red, and the slowest 25 cases are printed for test-only optimization. These flags never narrow discovery or skip a green test; callers cannot override them, and the lane selector after `--` remains the exact fixed discovery selector above.
   The internal `prepare()` API binds only the trigger and exact code fingerprint; it does not accept or validate focused receipt evidence.
5. **Bounded process evidence, never process folklore**: every executor/reviewer focused unittest command uses `.tools\run_unittest_with_repo_pythonpath.cmd <unittest args>`; that launcher uses only the hard-coded project Python, clears inherited interpreter overrides and PATH/PYTHONPATH pollution, delegates to `.tools\bounded_unittest.py`, verifies `jsonschema` once, and has a default focused deadline of 300 seconds. A successful focused run writes a local receipt containing the exact unittest args, positive `Ran N tests` evidence, pinned Python identity, and the current code fingerprint. The receipt is non-blocking audit metadata: `.githooks/pre-commit` may print its validation result as an advisory reminder, but a missing, stale, corrupt, incomplete, or overwritten receipt must never block commit, full-pack execution, or cached-green reuse, and must never trigger a test re-run solely to refresh the receipt. Focused evidence remains the bounded command's terminal output plus the `SESSION_LOG` record, independently rerun by the reviewer under rule 1. A known slow focused pack may explicitly use `.tools\run_unittest_with_repo_pythonpath.cmd --timeout-seconds <N> <unittest args>` for `301 <= N <= 1300`; `Verify` must name the measured reason. Neither the 1300-second focused ceiling nor the 300-second focused default may be raised without explicit user approval, and a launcher usage error must exit non-zero — a message with no tests run may never report success. It does not pay a full A-short preflight tax before every small test; run `runners/a_short_preflight.py` once only when the changed surface is the A-short runtime/dependency/resolver itself. A rule-3 full run uses rule 4 and has an 860-second maximum. On timeout the tool terminates only its own child process tree and reports `TIMEOUT`; do not scan, poll, or kill unrelated Python processes. A PID, CPU change, process existence, or parent/child switch is diagnostic only and never proves test progress, a “subcase switch”, or PASS. Only actual test output plus terminal exit can do that. A zero-test, still-running, timed-out, interrupted, missing-exit, or missing-`Ran N tests` run is `UNKNOWN`/FAIL evidence: stop, narrow or diagnose once, and never wait repeatedly. One knife may not spend more than the 860-second full-run ceiling waiting on tests; a 40-minute repair loop is a process failure, not acceptable diligence.
6. **Evidence and escalation**: `SESSION_LOG` `Verify` records the chosen tier, exact bounded command and final terminal result. Claude Code may escalate to its own full-lane run only when the recorded evidence is unavailable/inconsistent, a failure is flaky, or review finds material impact outside the declared tier; state that reason in `Verify`.
   **Merge-state carve-out (user-directed 2026-08-18)**: a merge by itself is NOT a reason to re-run the lane. The reviewer re-runs on the integrated state only when one of two conditions holds and is named: **(a)** the defect under review can only reproduce in the main tree / integrated environment, so a green measured in the executor's worktree cannot show the fix works where the failure lived; or **(b)** the merge actually brings in code affecting that lane. Both are evidence, not assertion — **(b)** records the exact `git diff --stat <base>..<merge> -- <lane paths>` and whether it came back empty; **(a)** names why the executor's tree could not reproduce the failure. When neither holds, `Verify` still carries one line saying the merge brought in nothing that touches the lane and the executor's ledger record is cited instead, so the SKIP is auditable too and not only the escalation.
7. **审查执行调度 —— 墙钟是硬指标（user-directed 2026-07-27；与本节及任何旧表述冲突时，以本条为准）**：一刀审查（读 diff + 探针 + 跑包 + 落盘 + 提交）目标墙钟 **10–15 分钟**；超过 30 分钟即为流程缺陷，须在 `Verify` 写明原因。强制顺序与禁令：**(a) 慢包先起、后读代码** —— 收到 `审查` 的第一条命令是 `git status` / `git diff` 出改动清单，**第二条命令**就把本轮唯一最慢的那个**超集**测试命令交给 rule 5/4 的 bounded runner 后台执行，再去整读函数体 / 写探针；禁止读完、探针跑完才起（并行掉的是纯等待，不降任何严格度）。**(b) 只跑超集，禁跑其子集** —— 同一模块的单测 → 测试类 → 整模块只跑最大的那个；超集绿 = 其内全部绿，不得为"再确认"回头补跑子集或重跑同一包。**(c) 同一时刻只跑一个重包** —— 并发重包会互拖甚至无 traceback 崩溃（实测 exit 127），其余结论用探针 / 静态证据取得。**(d) 测试红了先查环境残留再怀疑代码** —— 先检查 gitignored 的 scratch·输出目录（`provider_samples/`、`state/`、临时根）及 mtime，禁止用重复慢跑去二分一个可能由残留造成的红；不得扫描全局 Python 进程猜进度。**(e) 一件事只确认一次** —— 重跑同一命令若不产生新信息即为浪费。**不可删的三样**（发现几乎全部出自它们，任何提速都不许砍）：整读被消费的函数体、reviewer 自写的反向 / 植入探针、放松类改动的强制腿反向控制。
8. **Whole-review proportionality — over-review is a process defect, not diligence (user-directed 2026-07-18)**: the same change-surface tiering governs the WHOLE review, not only tests. The scope manifest, the item-16 defect-class matrix, adversarial recompute, and the §6a independent-agent gate all scale to the change's size and blast radius. A small, low-selection-impact change — small diff; no new/changed fail-closed engine · validator · authorization · provider · secret surface; not touching core_score / veto / sizing / PIT-into-selection / real-money — takes the fast path: integral-read the diff + one reverse/planted probe + the rule-1 acceptance pack + a minimal `SESSION_LOG`. Do NOT, for such a change, run the full lane pack, spin an independent agent, expand the defect matrix beyond the touched surface, or re-run an already-green pack to get a nicer count. Escalate to full rigor only when the change touches a sensitive seam (the §6a highest-risk subset) or focused evidence cannot bound its impact — and name that reason in `Verify`. Spending review effort disproportionate to a small fix's blast radius is itself a defect; a big fix earns full rigor, a small one does not.

## Claude review anti-fabrication gate

When Claude Code is the current reviewer/committer, review evidence is valid only if it is visible as one of: an actual tool result returned by the harness, a user-run `!` terminal output pasted into the conversation by the app, or the local hook's `REVIEW EVIDENCE SNAPSHOT` generated by `.tools/claude_review_gate.py`. A reviewer must not write simulated Bash / Read / Agent output, fake agent IDs, fake diffs, fake line numbers, fake file content, or fake test results. If the reviewer is unsure whether something was truly executed, the only allowed status is `NOT_VERIFIED`; that material cannot support PASS.

Local Claude Code hardening lives in `.tools/claude_review_gate.py` plus the project-local `.claude` hooks/skill. On command-like review prompts (`审查当前...`, `复审...`, `/stock-review...`) the `UserPromptSubmit` hook records a bounded git snapshot and issues a token shaped `review-evidence:<id>`. The review-cycle `docs/SESSION_LOG.md` `Verify` line must include that exact token before the Stop hook lets the review response close. If the local hook is unavailable, the reviewer must explicitly write `review-evidence:not_available` in `Verify`, state the gap, and avoid any PASS claim that depends on self-transcribed command output. User-run `!` commands remain the strongest fallback evidence.

**Evidence-completeness leg (2026-08-03, user-directed)**: the closeout is written once, after the full pack, the reviewer's own probes, and every independent agent launched by this review has reported. The Stop hook reads the session transcript and recognizes real task notifications across their transport wrappers, including top-level `content` and `attachment`; it blocks while an async `Agent`/`Task` launched after arming remains outstanding. If that agent genuinely died or was abandoned, the same `Verify` line must carry `agent-aborted:<reason>`; waiting is the default. Structured `toolUseResult.isAsync/status` is the primary launch signal, with the observed prose marker as fallback. This prevents a completed agent from being misclassified as pending and removes the pressure to record a false abandonment.

This gate does not create a parallel `REVIEW_PACKET.md` or a third truth source. Material findings still live only in `docs/system_risk_register.md`; `docs/SESSION_LOG.md` stays the minimal review-cycle pointer; final chat output still follows exactly `Verdict` / `Required / Optional / Options` / `下一步`.
...
3. Use `.tools/run_unittest_with_repo_pythonpath.cmd`, which always uses the pinned host Python, bounds the owned unittest process, and verifies `jsonschema` once before any test can silently skip it. It does not run the full A-short preflight before every small test; `runners/a_short_preflight.py` is an additional targeted check only when the A-short runtime/dependency/resolver surface changes. Codex must not use its bundled/PATH Python or an interpreter override. When a Codex sandbox blocks the host executable, request approved host-process execution instead of substituting another interpreter. Never accept a silent schema skip.
...
6a. **Highest-risk independent-agent gate (2026-07-03, user-directed; scoped)**: before PASS on a `审查` whose slice touches the HIGHEST-RISK subset — (i) a real live provider execution / data fetch, (ii) secret / credential / raw-payload-to-disk handling, or (iii) a NEW or substantially-changed (~≥50 lines or a new function) fail-closed engine / validator / authorization gate — the reviewer MUST run ≥1 independent adversarial agent (a separate `Agent` — worktree-isolated when the reviewed code is committed, OR read-only on the current working tree when the change is uncommitted (a worktree-from-a-ref cannot capture uncommitted changes); told to derive the invariants itself and attack blackbox for a reproducible bypass; forbidden to modify/commit the repo or make network calls), IN ADDITION TO the reviewer's own integral read of the consumed engine bodies + own adversarial probes (the agent does not replace them). Lower-risk code (incremental tweaks to existing engines, docs, test-infra, config) is reviewed with integral-read + at least one reverse probe, no mandatory agent. **Optional-fix carve-out (2026-07-18, user-directed):** reviewing a fix that ONLY addresses Optional (non-blocking) findings never requires an independent sub-agent — the main reviewing agent alone suffices (it still performs its own integral-read + reverse probe). Optional items are low-impact by definition, so an Optional-only fix always sits in this lower-risk tier. **Lightweight fast-path for an Optional-only fix (2026-07-18, user-directed — a review, not a full adversarial pass; applies whether the fixer self-reviews or a separate reviewer does):** (i) ONE scope grep FIRST — if no test/schema pins the changed behavior or string, the change cannot break an assertion, so smoke-verify only; (ii) run the SMALLEST test target that actually covers the changed path (or a direct probe) ONCE, with a timeout — never a broad/slow suite that does not touch the change, never re-run the same package; (iii) fold the outcome into a first-pass-compliant minimal-template SESSION_LOG entry (base labels only, Optional detail inside `Verify`/`Verdict`, no separate `Optional` label). Escalating an Optional-only fix to broad regression sweeps, repeated runs, or a post-hoc SESSION_LOG rewrite is itself the over-verification this carve-out exists to prevent; keep it to compile + import + one targeted check. Every `审查` verdict in `docs/SESSION_LOG.md` records — WITHIN the fixed minimal-template labels, NOT as new bullets/labels (the doc-governance guard enforces the label set: put this content inside `Verify` and `Verdict/Action`) — the risk classification, the independent-agent run + result (for the highest-risk subset), and the explicitly-unreviewed dimensions + residual risk (item 16b); a PASS omitting this content is non-compliant. When the classification is uncertain, default to the mandatory-agent tier and state the classification. Rationale: the highest-risk agent runs (run_fetch wiring, status injection, live probe) caught real gaps the reviewer's diff+assume shortcut missed, and the independent pass also corrected the reviewer's own severity overstatement — the gate makes that expensive-but-verifiable rigor auditable rather than reliant on reviewer diligence.
...
16d. **Review-completeness selective guard (2026-07-02)**: when a review round exposes a recurring review-completeness miss, add a narrow machine guard only for the exact material class / surface that failed (for example a missing scope manifest, false PASS shape, or active-doc drift surface). This is not a repo-wide protocol expansion: do not add broad new standing rules, full-file scans, or always-on review tax unless the failed class is material under item 15a and can be guarded with a planted-failure test.
...
9. **Scope gate**: On PASS, the reviewer/committer must auto-commit the reviewed slice before sending the final response, after rerunning the relevant local verification and confirming the staged files match the PASS scope. Stage only PASS-covered files; unrelated or concurrent dirty work stays outside the commit and outside the PASS verdict; do not stage or commit unreviewed files. If safe separation is blocked, record the blocker in `docs/SESSION_LOG.md` and say exactly what remains uncommitted.
...
5. Record Optional dispositions (accept / reject + reason) and run the relevant schema / runner tests only via `.tools/run_unittest_with_repo_pythonpath.cmd` or `.tools\\codex_main_python.ps1`, with the pinned host Python, before handing back for review. Run the A-short full preflight once only when the changed surface is its runtime/dependency/resolver; do not add it as a tax to unrelated small fixes. Codex must not use the bundled/PATH runtime or any interpreter override.
...
**精确集合契约(最强不变式,非子集放行)**:compliant-zone(marker 之上)引用 R-ID 的评审 entry,正文 bullet 标签集合必须**恰好**= `{Verdict/Action, Required(指 R-ID + register), Verify, Next}`(`修复` 再加恰好一个 `Pre-Codex self-review`/`Proof-of-use`)。**缺项 / 多项 / 重复标签 / 任何自由段落(任何语言)/ `Verify` 占位符(`N OK`/`<N>`/`TODO`/`TBD`/`XXX`/占位)/ 单条 bullet 超 ~500 字符(把 register 全文塞一条)都会 FAIL**——完整 finding 详情只进 `system_risk_register.md`。守护覆盖四类 header:`审查`/`修复`/`已闭`/`PASS`(纯 `<LLM> PASS (R-ID)` 也照查)。**`已闭` 是 2026-08-07 补进来的**(`R-DOCGOV-A-REPAIR-ROUND-ESCAPES-THE-TEMPLATE-GUARD-BY-NOT-SAYING-修复`):header 写成「…Optional 已闭(…)」的修复回合此前四道检查一道都不跑。**`已闭` header 默认按 `修复` 论**,同样必须带那一行 `Pre-Codex self-review`/`Proof-of-use`;只有当该 entry **自己的 `Verdict/Action` 值以 `PASS`/`FAIL` 开头**时才是审查方 verdict、不欠该行。**判别器读 `Verdict/Action` 值、不读 header**(`R-DOCGOV-CLOSEOUT-HEADER-THAT-MENTIONS-PASS-STILL-SKIPS-THE-PROOF-LINE`):本仓 header 是描述性句子,原先按 header 里有没有 verdict token 判,于是「Optional 已闭(复审 PASS 后收口)」这种写法仍能靠措辞免掉证据行——同一个洞往下缩一层;锚在值的**开头**而非全值搜索,否则正文复述「上一轮 PASS」会重新打开它。`收口` 故意不入表——它同为收尾动词,但 compliant-zone 里已有 30 条自由格式 session entry 用它,收进来是把历史判红而不是管住下一条。这是结构化精确集合,不靠列举禁词,换措辞/换语言都绕不过;**换 header 形态曾经绕得过,该缺口即由本次补键关闭**。守护:`tests/test_doc_governance_guard.py::test_review_cycle_minimal_template_enforced_above_marker` + `test_review_cycle_guard_planted_failures`(十四植入失败 + 三合规通过,含收尾式 header 的四类植入与两条假阳性对照)。
...
## Codex Python runtime pin

For this checkout, Codex must run project commands with the dependency-complete host interpreter at `C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe`. Use `.tools\\codex_main_python.ps1` or `.tools\\run_unittest_with_repo_pythonpath.cmd`; do not use the Codex bundled/PATH Python, and do not replace a blocked host interpreter with the current agent runtime. If the sandbox blocks host-process execution, request the approved host-process execution for the pinned interpreter.
