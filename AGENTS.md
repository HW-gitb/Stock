...
- 🟡 Phase 7b-2 P1 US evidence snapshots：`docs/provider_evidence_p1_us_public_sources_20260528.json` 与 `docs/provider_evidence_p1_us_market_data_candidates_20260528.json` 已基于官方 SEC / Nasdaq / MSCI / Massive / Polygon / Norgate 文档记录 public-source 与 market-data-candidate evidence；P1 为 `partial`，仍 implementation-blocked，后续仍需 authorization / cost、sandbox / trial feasibility、coverage count、direct benchmark return source、issuer-level PIT GICS membership、fundamentals / filing observed-date provider candidates、fallback / stability evidence
...
| 7b-2 | Provider capability evidence population：按 P1-P4 读取/核验 provider 文档、字段、PIT、coverage、cost、fallback、stability 证据；不默认选择 provider、不建 adapter / DataHub table | 1-2 周 | 🟡 P1 public-source + market-data-candidate snapshots complete；继续 P1 |
...
14. Phase 7 implementation 顺序采用 alpha-leverage-first，不再默认从已证明的 A-share EOD / benchmark surface 消耗下一刀资源；Phase 7a schema-first audit / routing / feasibility / report contracts 与 Phase 7b-1 provider evidence / drift monitor contract 已建立。Phase 7b-2 已有 P1 US public-source 与 market-data-candidate evidence snapshots，但 P1 仍只是 partial / blocked：SEC / Nasdaq / MSCI / Massive / Polygon / Norgate docs 不等于完整 provider selection、authorization、PIT security master、benchmark、fundamentals observed-date 或 production readiness。后续 Phase 7c DataHub / report / reproducibility work 必须消费 reviewed P1-P4 provider evidence 与 drift-monitor contract，不能把 P4 ready helper surface 当作 broad implementation 起点。
