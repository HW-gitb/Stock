...
- 🟡 Phase 7b-2 P1 US evidence snapshots + readiness review matrix + access/sample plan：六份 P1 snapshot 已基于官方 SEC / Nasdaq / MSCI / S&P DJI / S&P Global / LSEG / Massive / Polygon / Norgate / Intrinio / FMP 文档记录 candidate evidence；`schemas/provider_p1_readiness_review.schema.json` / `docs/provider_evidence_p1_us_readiness_review_matrix_20260529.json` 已固化 field-by-field blocker disposition；`schemas/provider_p1_access_decision_plan.schema.json` / `docs/provider_evidence_p1_us_access_decision_sample_validation_plan_20260531.json` 已把 cost ceiling、access path、license / storage、sample rows、coverage counts、fallback / incident gate 固化为 plan-only artifact。Approved spend = 0；仍不授权 provider selection / contact / token / trial / paid access / sample fetch / data fetch / Phase 7c；A-share `minimal_data_burst` preregistration 已在 `schemas/research_preregistration.schema.json` / `research/preregistrations/a_share_minimal_data_burst_20260531.json` 建立但因 benchmark entry-basis measurement issue 标记 `BLOCKED_DO_NOT_RUN`，下一步必须先做 alpha measurement integrity / corrected-basis supersession。
...
- `schemas/research_preregistration.schema.json` / `research/preregistrations/a_share_minimal_data_burst_20260531.json` — research-only preregistration contract and current A-share `minimal_data_burst` artifact（当前 `BLOCKED_DO_NOT_RUN`；先做 corrected-basis supersession，不改 runner、不进 production、不声称 ship-gate evidence）
