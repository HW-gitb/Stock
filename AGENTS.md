...
35. 2026-06-04 A-long broader materialization packet 已写好但未执行：`schemas/a_long_tushare_broader_materialization_packet.schema.json` / `docs/a_long_tushare_broader_materialization_packet_20260604.json` / `runners/a_long_tushare_broader_materialization_packet.py` / `schemas/a_long_tushare_broader_materialization_execution_summary.schema.json` 固定 2018-2025、8 只活跃股 + `000666.SZ` 退市样本、CSI300 / CSI1000、71 planned calls / max 80、gitignored raw path `data/a_long/raw/tushare/materialization_full_period_panel_20260604/`、checkpoint resume、no-secret tracked summary。简单说：下一次可审后跑更大的固定样本池，但现在没有跑，也仍不能找 alpha。必须 Claude 独立审查通过 + 用户再次 `执行`，才可带确认参数跑；跑完也只能进入新的 full-period panel data-integrity audit，不得全市场 / 全 universe 拉取、搜 signal、声称 alpha / production / ship-gate / full-size。
