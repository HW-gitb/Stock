...
41. 2026-06-04 A-long `000666.SZ` SW membership supplement packet is prepared but not executed：`schemas/a_long_000666_sw_membership_supplement_packet.schema.json` / `docs/a_long_000666_sw_membership_supplement_packet_20260604.json` / `runners/a_long_000666_sw_membership_supplement_packet.py` define a fixed single-symbol, 3-call, zero-retry Tushare supplement packet for the remaining delisted SW membership gap. The packet itself performs no Tushare call, reads no raw payload, and authorizes no audit rerun or signal search. 简单说：下一步只是等 Claude 审查这个补探包；审查通过并且用户再次 `执行` 后，才能跑最多 3 个 Tushare calls。跑完即使找到候选来源，也仍要单独修 audit 并重跑通过，A-long 才能继续走信号预注册。
