...
| 7c | DataHub shared layer / report contracts / reproducibility plumbing / local resource budget：先写 schema-first contract；implementation 另起 reviewed slice | 1-2 周 | 🟡 resource-budget + job-spec precondition contracts + code-enforcement helper complete；shared-layer / report implementation not started |
