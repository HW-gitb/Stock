...
- `schemas/research_preregistration.schema.json` / `research/preregistrations/a_share_minimal_data_burst_20260531.json` — research-only preregistration contract and current A-share `minimal_data_burst` single frozen test artifact（不改 runner、不进 production、不声称 ship-gate evidence）
