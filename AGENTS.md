...
4. **One complete run for one unchanged final diff**: when rule 3 triggers, exactly one actor runs the full lane pack after the last behavior/contract edit (default: executor before handoff; reviewer may explicitly take ownership instead). The reviewer independently reruns only rule-1 acceptance tests. Use the single command `.tools\codex_main_python.ps1 .tools\full_pack_ledger.py run <lane> "<trigger>" "<focused evidence>" <timeout-seconds> -- <unittest args>`: invoke that argument vector directly; do not wrap it in `Start-Process` or concatenate it into one `ArgumentList` string. It checks the exact-code-state cache, binds the A-F preparation, enforces the deadline, validates the terminal exit and `Ran N tests`, and records only PASS. Manual `prepare`/`record` is retired. A later behavior/contract edit invalidates that run; docs/register/SESSION_LOG-only edits do not.
