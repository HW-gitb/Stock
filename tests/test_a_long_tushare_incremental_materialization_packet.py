...
from runners import a_long_tushare_incremental_materialization_packet as runner
...
class FakeTusharePro:
    def trade_cal(self, **kwargs):
        return pd.DataFrame(
            [
                {"cal_date": "20220104", "is_open": "1", "exchange": "SSE"},
...
class ALongTushareIncrementalMaterializationPacketTest(unittest.TestCase):
    def setUp(self) -> None:
    ...
    def test_fake_client_writes_pass_summary_and_raw_payloads(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            summary = runner.execute_incremental_materialization(
            ...
            self.assertEqual(summary["decision"]["materialization_status"], "passed_thin_slice_materialization_shape")
            ...
            self.assertTrue(
                all(ref.as_posix().startswith("data/a_long/raw/tushare/materialization_thin_slice_20260604/") for ref in raw_refs)
            )
    ...
    def test_fake_summary_validates_against_schema_when_jsonschema_available(self) -> None:
        ...
        schema = json.loads(
            Path("schemas/a_long_tushare_incremental_materialization_execution_summary.schema.json").read_text(
                encoding="utf-8"
            )
        )
    ...
    def test_second_run_reuses_existing_raw_without_network(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            runner.execute_incremental_materialization(
                pro_factory=lambda: FakeTusharePro(),
                raw_root=self.raw_root,
    ...
    def test_call_plan_matches_packet_budget(self) -> None:
        ...
        self.assertEqual(sum(1 for call in calls if call["table_id"] == "income"), 3)
        self.assertEqual(sum(1 for call in calls if call["api_family"] == "daily"), 3)
