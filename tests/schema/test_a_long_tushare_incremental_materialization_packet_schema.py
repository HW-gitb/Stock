...
PACKET_SCHEMA_PATH = Path("schemas/a_long_tushare_incremental_materialization_packet.schema.json")
PACKET_PATH = Path("docs/a_long_tushare_incremental_materialization_packet_20260604.json")
SUMMARY_SCHEMA_PATH = Path("schemas/a_long_tushare_incremental_materialization_execution_summary.schema.json")


class ALongTushareIncrementalMaterializationPacketSchemaTest(unittest.TestCase):
    ...
    def test_packet_schema_meta_validates_when_jsonschema_available(self) -> None:
        ...
        self.assertEqual(schema["properties"]["schema_name"]["const"], "a_long_tushare_incremental_materialization_packet")
        self.assertIn("does not itself run Tushare calls", schema["description"])
    ...
    def test_summary_schema_meta_validates_when_jsonschema_available(self) -> None:
        ...
        self.assertEqual(
            ...
            "a_long_tushare_incremental_materialization_execution_summary",
        )
    ...
    def test_scope_is_execution_packet_not_execution(self) -> None:
        ...
        self.assertTrue(scope["thin_slice_materialization_allowed_after_gates"])
        for field in [
    ...
    def test_thin_slice_boundary_and_budget_are_fixed(self) -> None:
        artifact = self._load_packet()
        boundary = artifact["thin_slice_boundary"]
        budget = artifact["call_budget"]

        self.assertEqual(boundary["slice_id"], "a_long_tushare_thin_slice_2022_2023")
        self.assertEqual(boundary["start_date"], "20220101")
        self.assertEqual(boundary["end_date"], "20231231")
        self.assertEqual(boundary["active_symbols"], ["000001.SZ", "600519.SH"])
        ...
        self.assertTrue(boundary["not_full_2018_2025"])
        self.assertEqual(budget["planned_total_endpoint_calls"], 29)
        self.assertEqual(budget["max_total_endpoint_calls"], 32)
    ...
    def test_tables_are_bounded_and_do_not_authorize_downstream_use(self) -> None:
        ...
        self.assertEqual(tables["income"]["planned_calls"], 3)
        self.assertEqual(tables["daily_price_adj_factor_dividend"]["planned_calls"], 9)
    ...
    def test_storage_gates_and_claims_are_locked(self) -> None:
        ...
        self.assertEqual(storage["raw_output_root"], "data/a_long/raw/tushare/materialization_thin_slice_20260604/")
        self.assertEqual(
    ...
    def test_scope_creep_is_rejected_when_jsonschema_available(self) -> None:
        ...
        invalid["scope"]["full_2018_2025_materialization_allowed"] = True
        invalid["thin_slice_boundary"]["active_symbols"].append("000002.SZ")
        invalid["thin_slice_boundary"]["not_full_market"] = False
        invalid["call_budget"]["planned_total_endpoint_calls"] = 100
