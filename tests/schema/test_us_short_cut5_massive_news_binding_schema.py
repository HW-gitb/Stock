# -*- coding: utf-8 -*-
"""Adversarial schema tests for the US-short Cut 5-d Massive news binding.

Finding D: the schema must FREEZE the load-bearing PIT / duplicate / publisher-normalization / checked-empty /
lineage / authorization POLICIES, not just the vocabularies. Each mutant flips exactly one const and asserts a
schema error; the genuine binding validates with zero errors.
"""
...
SCHEMA_PATH = ROOT / "schemas" / "us_short_cut5_massive_news_binding.schema.json"


class MassiveNewsBindingSchemaTest(unittest.TestCase):
    def setUp(self):
        from jsonschema import Draft7Validator
        self._validator = Draft7Validator(json.loads(SCHEMA_PATH.read_text(encoding="utf-8")))
        self._binding = nw.load_binding()
    ...
    def test_chronology_order_drift_rejected(self):
        self.assertTrue(self._errors(self._mutated(
            ["pit_clock_contract", "chronology_order"], ["observed_at", "published_utc", "source_as_of", "as_of"])))
    ...
    def test_duplicate_identity_drift_rejected(self):
        self.assertTrue(self._errors(self._mutated(["duplicate_policy", "source_row_identity"], "title")))

    def test_publisher_normalization_drift_rejected(self):
        self.assertTrue(self._errors(self._mutated(["duplicate_policy", "publisher_identity_normalization"], "verbatim")))
    ...
    def test_sentiment_allowed_drift_rejected(self):
        # adding a fabricated sentiment (e.g. bullish) beyond Massive's own enum must fail
        self.assertTrue(self._errors(self._mutated(["sentiment_allowed"], ["positive", "negative", "neutral", "bullish"])))
