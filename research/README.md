...
- `research/preregistrations/a_long_data_integrity_audit_20260603.json` - planned A-long data-integrity gate preregistration. It freezes one executable hard-check audit plus usable-window coverage characterization for PIT fundamentals, restatement as-of, PIT universe / survivorship, terminal delisting returns, return / benchmark measurement, and time-varying coverage before any A-long signal search.
...
- `research/ledgers/a_long_data_integrity_audit_program_test_budget_ledger_20260603.json` - singleton data-integrity gate ledger for A-long. It records one planned, not-yet-reviewed audit unit, frozen planted-violation self-test expectations, and zero alpha-test budget consumption; no audit run or signal search is authorized until Claude review passes and the user gives a later execute command.
