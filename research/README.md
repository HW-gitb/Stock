...
- `research/preregistrations/a_share_minimal_data_burst_full_universe_redesign_20260531.json` - ledger-gated redesigned preregistration for one research-only full EGS candidate-surface preflight / test. It is registered but not reviewed or run-authorized; first executable step after review is pre-outcome event-count / input-integrity preflight only.
...
- `research/ledgers/a_share_burst_program_test_budget_ledger_20260531.json` - singleton program-level test-budget ledger for any redesigned A-share burst test after the zero-event preflight. It now has one user-approved planned test pending Claude review for the full-universe redesign; it does not authorize running that test until review passes.
...
Do not run a blocked preregistration. Do not run the corrected-basis artifact for outcome / excess calculation after the zero-event preflight. Do not run the full-universe redesigned preregistration until Claude review passes; after review, the first allowed step is pre-outcome event-count / input-integrity preflight only.
