"""Build the reviewed, policy-bound parent plan for the 20260815 probe.
...
independent 20260815 probe packet, freezes the provider envelope, and publishes
...
DEFAULT_PROBE_PACKET_PATH = ROOT / "docs" / "us_short_soft_discovery_query_quality_probe_packet_20260815.json"
PROBE_PACKET_SCHEMA_PATH = ROOT / "schemas" / "us_short_soft_discovery_query_quality_probe_packet_20260815.schema.json"
...
def _load_probe_packet(
    ...
    try:
        ...
        raise ParentPlanBuilderError("20260815 probe packet or schema is unreadable") from exc
    if type(packet) is not dict or type(schema) is not dict:
        raise ParentPlanBuilderError("20260815 probe packet and schema must be objects")
    try:
        ...
        raise ParentPlanBuilderError("jsonschema is required for the 20260815 probe packet") from exc
    if errors:
        raise ParentPlanBuilderError(f"20260815 probe packet schema rejected: {errors[0].message}")
    return packet
...
def build_parent_plan_from_reviewed_policy(
    *,
    ...
    if decision_date != expected_date or decision_date in forbidden_dates:
        raise ParentPlanBuilderError(
            "decision date is not the independent 20260815 probe packet slot"
        )
    try:
...
def build_argument_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Build the offline, policy-bound US-short 20260815 parent plan."
    )
