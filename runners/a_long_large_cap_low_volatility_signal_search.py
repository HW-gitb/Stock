...
SUMMARY_SCHEMA_PATH = ROOT / "schemas" / "a_long_large_cap_low_volatility_signal_search_execution_summary.schema.json"
PREREGISTRATION_PATH = ROOT / "research" / "preregistrations" / "a_long_large_cap_low_volatility_20260608.json"
PREREGISTRATION_SCHEMA_PATH = ROOT / "schemas" / "a_long_large_cap_low_volatility_preregistration.schema.json"
LEDGER_PATH = ROOT / "research" / "ledgers" / "a_long_large_cap_low_volatility_program_test_budget_ledger_20260608.json"
...
OUTPUT_DIR = ROOT / "research" / "results" / "a_long_large_cap_low_volatility_20260608"
SUMMARY_PATH = OUTPUT_DIR / "execution_summary.json"

SUMMARY_ARTIFACT_ID = "a_long_large_cap_low_volatility_signal_search_execution_summary_20260608"
PLANNED_TEST_ID = "a_long_large_cap_low_volatility_20260608"
PREREGISTRATION_ARTIFACT_ID = "a_long_large_cap_low_volatility_20260608"
LEDGER_FAMILY_ID = "a_long_large_cap_low_volatility_v1"

# The single primary factor under test (price-only) plus the frozen diagnostic-only factors.
...
LOW_VOLATILITY_VIEWS = ["industry_size_neutral", "non_neutral", "industry_neutral", "size_neutral"]
PRIMARY_HORIZON = 504
...
# Frozen low_volatility signal construction. The trailing volatility lookback (252 trading days)
# is a DISTINCT axis from the 252d diagnostic forward holding horizon; they must not be conflated.
TRAILING_WINDOW_TRADING_DAYS = 252
MIN_VALID_DAILY_RETURNS = 120
...
PRIMARY_CELL_ID = "primary_low_volatility_industry_size_neutral_504d_csi300"
FALSIFIED_VERDICT = "falsified_large_cap_low_volatility_under_frozen_rules"
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Run the reviewed A-long large-cap low_volatility single-factor signal search from local raw data. "
            "low_volatility is the negative trailing 252-trading-day realized volatility of daily adj_factor total "
            "returns; it reads only the already-materialized full-main-board daily close series and the reviewed "
            "top-500 market-cap universe, executes no provider call, and spends the low_volatility singleton ledger "
            "after a valid summary write."
        )
    )
...
def require_execution_confirmations(*, confirm_independent_review_pass: bool, confirm_post_review_execute: bool) -> None:
    if not confirm_independent_review_pass:
        raise RuntimeError("low_volatility signal search requires --confirm-independent-review-pass")
    if not confirm_post_review_execute:
        raise RuntimeError("low_volatility signal search requires --confirm-post-review-execute")
...
def load_and_validate_preregistration(path: Path = PREREGISTRATION_PATH) -> dict[str, Any]:
    ...
    if prereg.get("schema_name") != "a_long_large_cap_low_volatility_preregistration":
        raise ValueError("low_volatility preregistration schema_name mismatch")
    if prereg.get("artifact_id") != PREREGISTRATION_ARTIFACT_ID:
        raise ValueError("low_volatility preregistration artifact_id mismatch")

    scope = prereg.get("scope") or {}
    if scope.get("preregistration_review_status") != "passed_independent_review_ready_for_freeze":
        raise ValueError("low_volatility preregistration is not review-passed")
    for field in [
        "research_only",
        "externally_motivated_not_prior_rescue",
        "new_hypothesis_not_prior_reslice",
        "reuses_reviewed_materialized_market_cap_universe",
        "reuses_reviewed_full_main_board_daily_close_series",
        "manual_order_only",
    ]:
        if scope.get(field) is not True:
            raise ValueError(f"low_volatility preregistration scope.{field} must be true")
    for field in [
        ...
        if scope.get(field) is not False:
            raise ValueError(f"low_volatility preregistration scope.{field} must be false")

    provenance = prereg.get("hypothesis_provenance") or {}
    if provenance.get("externally_literature_motivated") is not True:
        raise ValueError("low_volatility provenance must remain externally literature motivated")
    if provenance.get("derived_from_in_sample_diagnostic_of_prior_run") is not False:
        raise ValueError("low_volatility must not be an in-sample diagnostic of a prior run")
    if provenance.get("no_trailing_window_search") is not True:
        raise ValueError("low_volatility trailing-window search must remain forbidden")
    if provenance.get("no_idiosyncratic_vs_total_vol_search") is not True:
        raise ValueError("low_volatility idiosyncratic-vs-total search must remain forbidden")
    if provenance.get("old_result_reslice_allowed") is not False:
        raise ValueError("low_volatility old-result reslice must remain forbidden")
    if provenance.get("prior_clue_or_diagnostic_relabel_as_pass_allowed") is not False:
        raise ValueError("low_volatility prior-clue relabel must remain forbidden")

    data_reuse = prereg.get("data_reuse") or {}
    if data_reuse.get("no_new_provider_call_required") is not True:
        raise ValueError("low_volatility data reuse no-provider-call flag drifted")
    if data_reuse.get("no_new_daily_bar_pull_required") is not True:
        raise ValueError("low_volatility data reuse no-daily-bar-pull flag drifted")
    if data_reuse.get("daily_close_series_source") != "full_main_board_raw_root_daily_call_close_times_adj_factor":
        raise ValueError("low_volatility daily close series source drifted")
    if data_reuse.get("selected_market_cap_field") != SELECTED_MARKET_CAP_FIELD:
        raise ValueError("low_volatility data reuse market-cap field drifted")
    if data_reuse.get("monthly_as_of_count") != len(MONTHLY_AS_OF_DATES):
        raise ValueError("low_volatility data reuse as-of count drifted")

    design = prereg.get("frozen_design") or {}
    if design.get("design_id") != LEDGER_FAMILY_ID:
        raise ValueError("low_volatility design id drifted")

    universe = design.get("universe_rule") or {}
    if universe.get("board_scope") != "main_board_only":
        raise ValueError("low_volatility board scope drifted")
    if universe.get("as_of_selection_rule") != "last_open_A_share_trading_day_of_each_calendar_month":
        raise ValueError("low_volatility as-of selection rule drifted")
    if universe.get("universe_size_n") != UNIVERSE_SIZE_N:
        raise ValueError("low_volatility universe size drifted")
    if universe.get("universe_size_n_search_allowed") is not False:
        raise ValueError("low_volatility universe size search must remain forbidden")
    if universe.get("selection_basis") != "top_500_by_pit_market_cap_as_of_each_as_of_date":
        raise ValueError("low_volatility universe selection basis drifted")
    if universe.get("include_later_delisted_names_at_pre_delisting_asofs") is not True:
        raise ValueError("low_volatility PIT delisted-name inclusion drifted")
    if universe.get("pit_list_delist_required") is not True:
        raise ValueError("low_volatility PIT list/delist requirement drifted")
    if universe.get("selection_time_namechange_veto_required") is not True:
        raise ValueError("low_volatility PIT namechange veto requirement drifted")
    if universe.get("reviewed_data_quality_exclusion_boundary_ref") != cap_audit.DATA_QUALITY_EXCLUSION_DECISION_REF:
        raise ValueError("low_volatility data-quality exclusion ref drifted")
    ...
    if exclusion_policy.get("excluded_symbols") != sorted(cap_audit.EXPECTED_REVIEWED_EXCLUSION_SYMBOLS):
        raise ValueError("low_volatility data-quality exclusion symbol drifted")
    if exclusion_policy.get("affected_as_of_dates") != ["20191129"]:
        raise ValueError("low_volatility data-quality exclusion date drifted")
    if exclusion_policy.get("max_excluded_symbols") != 1:
        raise ValueError("low_volatility data-quality exclusion symbol cap drifted")
    if exclusion_policy.get("max_excluded_observations") != 1:
        raise ValueError("low_volatility data-quality exclusion observation cap drifted")
    if exclusion_policy.get("drop_excluded_symbols_before_signal_scoring") is not True:
        raise ValueError("low_volatility signal universe must drop reviewed exclusions")
    if exclusion_policy.get("backfill_next_main_board_by_circ_mv") is not True:
        raise ValueError("low_volatility signal universe must backfill by circ_mv")
    if exclusion_policy.get("materialized_top500_rederivation_unchanged") is not True:
        raise ValueError("low_volatility materialized top500 rederivation boundary drifted")
    if exclusion_policy.get("threshold_rescue_allowed") is not False:
        raise ValueError("low_volatility threshold rescue must remain forbidden")
    if universe.get("st_star_bse_chinext_excluded") is not True:
        raise ValueError("low_volatility board/status exclusion boundary drifted")

    signal = design.get("signal_rule") or {}
    if signal.get("primary_signal_id") != PRIMARY_SIGNAL_ID:
        raise ValueError("low_volatility primary signal drifted")
    if signal.get("primary_signal_type") != "single_factor_percentile":
        raise ValueError("low_volatility primary signal type drifted")
    if signal.get("primary_factor") != PRIMARY_FACTOR:
        raise ValueError("low_volatility primary factor drifted")
    if signal.get("primary_factor_definition") != PRIMARY_FACTOR_DEFINITION:
        raise ValueError("low_volatility primary factor definition drifted")
    if signal.get("volatility_estimator") != VOLATILITY_ESTIMATOR:
        raise ValueError("low_volatility volatility estimator drifted")
    if signal.get("trailing_window_trading_days") != TRAILING_WINDOW_TRADING_DAYS:
        raise ValueError("low_volatility trailing window drifted")
    if signal.get("trailing_window_search_allowed") is not False:
        raise ValueError("low_volatility trailing-window search must remain forbidden")
    if signal.get("realized_vol_min_valid_daily_returns") != MIN_VALID_DAILY_RETURNS:
        raise ValueError("low_volatility minimum valid daily returns drifted")
    if signal.get("small_sample_trailing_window_guard_required") is not True:
        raise ValueError("low_volatility small-sample trailing-window guard drifted")
    if signal.get("low_volatility_is_high_score_direction") is not True:
        raise ValueError("low_volatility high-score direction drifted")
    if signal.get("idiosyncratic_or_total_vol_choice") != "total_volatility_frozen_no_search":
        raise ValueError("low_volatility total-volatility frozen choice drifted")
    if signal.get("idiosyncratic_or_total_vol_search_allowed") is not False:
        raise ValueError("low_volatility idiosyncratic-vs-total search must remain forbidden")
    if signal.get("factor_definition_change_allowed") is not False:
        raise ValueError("low_volatility factor-definition-change must remain forbidden")
    if signal.get("percentile_rank_required") is not True:
        raise ValueError("low_volatility percentile-rank requirement drifted")
    if signal.get("zscore_allowed") is not False:
        raise ValueError("low_volatility z-score must remain forbidden")
    if signal.get("multi_factor_composite_allowed") is not False:
        raise ValueError("low_volatility multi-factor composite must remain forbidden")
    if signal.get("single_factor_winner_take_all_from_prior_result_allowed") is not False:
        raise ValueError("low_volatility winner-take-all must remain forbidden")
    if signal.get("insufficient_trailing_window_months_excluded_from_cohorts") is not True:
        raise ValueError("low_volatility startup trailing-window exclusion drifted")
    if signal.get("trailing_window_coverage_gate_applies_to_cohort_forming_months_only") is not True:
        raise ValueError("low_volatility trailing-window coverage gate scope drifted")
    if signal.get("diagnostic_factors") != DIAGNOSTIC_FACTORS:
        raise ValueError("low_volatility diagnostic factors drifted")
    if signal.get("diagnostic_factor_can_rescue_primary_failure") is not False:
        raise ValueError("low_volatility diagnostic rescue must remain forbidden")

    neutral = design.get("neutralization_rule") or {}
    if neutral.get("primary_view") != "industry_and_size_neutral":
        raise ValueError("low_volatility preregistered primary view drifted")
    if neutral.get("neutralization_method") != "marginal_double_neutralization":
        raise ValueError("low_volatility neutralization method drifted")
    if neutral.get("primary_score_construction") != "equal_weight_average_of_marginal_industry_neutral_and_marginal_size_neutral_percentile_scores":
        raise ValueError("low_volatility primary score construction drifted")
    if neutral.get("industry_neutral_score_rule") != "percentile_within_industry_l2_fallback_l1":
        raise ValueError("low_volatility industry-neutral score rule drifted")
    if neutral.get("size_neutral_score_rule") != "percentile_within_market_cap_quintile":
        raise ValueError("low_volatility size-neutral score rule drifted")
    if neutral.get("combined_score_rule") != "0_5_industry_neutral_percentile_plus_0_5_size_neutral_percentile":
        raise ValueError("low_volatility combined score rule drifted")
    if neutral.get("crossed_industry_size_bucket_allowed") is not False:
        raise ValueError("low_volatility crossed industry-size buckets must remain forbidden")
    if neutral.get("industry_basis") != "SW_L2_then_SW_L1_if_sample_lt_20":
        raise ValueError("low_volatility industry basis drifted")
    if neutral.get("industry_l2_min_count") != 20:
        raise ValueError("low_volatility industry L2 minimum drifted")
    if neutral.get("industry_l1_min_count") != 2:
        raise ValueError("low_volatility industry L1 fallback minimum drifted")
    if neutral.get("size_bucket_rule") != "pit_market_cap_quintile_inside_top_500_per_as_of":
        raise ValueError("low_volatility size bucket rule drifted")
    if neutral.get("size_bucket_count") != len(SIZE_BUCKETS):
        raise ValueError("low_volatility size bucket count drifted")
    if neutral.get("expected_names_per_size_bucket") != UNIVERSE_SIZE_N // len(SIZE_BUCKETS):
        raise ValueError("low_volatility expected size bucket count drifted")
    if neutral.get("minimum_size_bucket_count_for_primary_percentile") != MIN_SIZE_BUCKET_COUNT_FOR_PRIMARY:
        raise ValueError("low_volatility size bucket minimum drifted")
    if neutral.get("non_neutral_view_role") != "diagnostic_only_not_primary":
        raise ValueError("low_volatility non-neutral diagnostic role drifted")
    if neutral.get("cap_weighted_view_role") != "diagnostic_only_not_primary":
        raise ValueError("low_volatility cap-weighted diagnostic role drifted")

    measurement = design.get("measurement_rule") or {}
    if measurement.get("primary_horizon_trading_days") != PRIMARY_HORIZON:
        raise ValueError("low_volatility primary horizon drifted")
    if measurement.get("diagnostic_horizons_trading_days") != [DIAGNOSTIC_HORIZON]:
        raise ValueError("low_volatility diagnostic horizon drifted")
    if measurement.get("trailing_volatility_lookback_and_forward_horizon_are_distinct_axes") is not True:
        raise ValueError("low_volatility distinct-axes flag drifted")
    if measurement.get("entry_rule") != "next_trading_day_close_after_as_of":
        raise ValueError("low_volatility entry rule drifted")
    if measurement.get("round_trip_cost") != base.ROUND_TRIP_COST:
        raise ValueError("low_volatility round-trip cost drifted")
    if measurement.get("stock_return_basis") != base.STOCK_RETURN_BASIS:
        raise ValueError("low_volatility stock return basis drifted")
    if measurement.get("benchmark_return_basis") != base.BENCHMARK_RETURN_BASIS:
        raise ValueError("low_volatility benchmark return basis drifted")
    if measurement.get("same_anchor_required") is not True:
        raise ValueError("low_volatility same-anchor requirement drifted")
    if measurement.get("missing_scheduled_exit_policy") != base.MISSING_SCHEDULED_EXIT_POLICY:
        raise ValueError("low_volatility missing scheduled exit policy drifted")
    if measurement.get("total_return_required") is not True:
        raise ValueError("low_volatility total-return requirement drifted")
    if measurement.get("price_index_fallback_allowed") is not False:
        raise ValueError("low_volatility price-index fallback must remain forbidden")

    benchmark = design.get("benchmark_rule") or {}
    if benchmark.get("primary_benchmark") != PRIMARY_BENCHMARK:
        raise ValueError("low_volatility primary benchmark drifted")
    if benchmark.get("diagnostic_benchmark") != SECONDARY_BENCHMARK:
        raise ValueError("low_volatility diagnostic benchmark drifted")
    if benchmark.get("both_benchmark_pass_required") is not False:
        raise ValueError("low_volatility CSI1000 must remain diagnostic")
    if benchmark.get("benchmark_access_probe_ref") != display_path(base.BENCHMARK_ACCESS_PROBE_SUMMARY_PATH):
        raise ValueError("low_volatility benchmark access probe ref drifted")
    if benchmark.get("benchmark_access_status") != base.BENCHMARK_ACCESS_STATUS:
        raise ValueError("low_volatility benchmark access status drifted")
    if benchmark.get("derived_total_return_open_allowed") is not False:
        raise ValueError("low_volatility derived total-return open must remain forbidden")

    cell = design.get("decision_cell") or {}
    if cell.get("cell_id") != PRIMARY_CELL_ID:
        raise ValueError("low_volatility primary cell id drifted")
    if cell.get("signal") != PRIMARY_SIGNAL_ID:
        raise ValueError("low_volatility primary cell signal drifted")
    if cell.get("view") != "industry_and_size_neutral":
        raise ValueError("low_volatility primary cell view drifted")
    if cell.get("horizon_trading_days") != PRIMARY_HORIZON:
        raise ValueError("low_volatility primary cell horizon drifted")
    if cell.get("benchmark") != PRIMARY_BENCHMARK:
        raise ValueError("low_volatility primary cell benchmark drifted")
    if cell.get("top_fraction") != TOP_FRACTION:
        raise ValueError("low_volatility top fraction drifted")
    if cell.get("minimum_top_count_per_month") != MIN_TOP_COUNT:
        raise ValueError("low_volatility minimum top count drifted")
    if cell.get("decision_is_two_tier") is not True:
        raise ValueError("low_volatility two-tier decision drifted")
    clue_gates = cell.get("statistical_alpha_clue_gates") or {}
    if clue_gates.get("mean_net_excess_must_be_positive") is not True:
        raise ValueError("low_volatility mean excess positivity gate drifted")
    if clue_gates.get("minimum_hac_t_stat") != MIN_HAC_T_STAT:
        raise ValueError("low_volatility HAC threshold drifted")
    if clue_gates.get("minimum_monthly_cohorts") != MIN_MONTHLY_COHORTS:
        raise ValueError("low_volatility minimum cohort count drifted")
    if clue_gates.get("name_concentration_guard_max_share") != MAX_TOP_SYMBOL_SELECTION_SHARE:
        raise ValueError("low_volatility name concentration guard drifted")
    if clue_gates.get("single_year_positive_return_guard_max_share") != MAX_SINGLE_YEAR_POSITIVE_RETURN_SHARE:
        raise ValueError("low_volatility single-year concentration guard drifted")
    if clue_gates.get("sub_period_both_halves_mean_excess_positive_required") is not True:
        raise ValueError("low_volatility sub-period both-halves gate drifted")
    ...
    if tradeable_gates.get("risk_gate_metric") != "rolling_overlapping_portfolio_relative_nav_max_drawdown_vs_csi300":
        raise ValueError("low_volatility tradeable risk-gate metric drifted")
    if tradeable_gates.get("minimum_allowed_relative_nav_drawdown") != MIN_ALLOWED_RELATIVE_NAV_DRAWDOWN:
        raise ValueError("low_volatility relative-NAV drawdown threshold drifted")
    if tradeable_gates.get("absolute_nav_drawdown_is_diagnostic_only") is not True:
        raise ValueError("low_volatility absolute-NAV drawdown diagnostic role drifted")
    if tradeable_gates.get("risk_gate_affects_tradeable_label_only_not_alpha_clue") is not True:
        raise ValueError("low_volatility risk-gate scope (tradeable label only) drifted")
    if cell.get("multiple_testing_adjustment_for_decision") != "not_applicable_single_primary_cell":
        raise ValueError("low_volatility multiple-testing decision policy drifted")
    ...
    if sub_period.get("split_rule") != "median_split_of_valid_504d_entry_cohorts_into_two_equal_halves":
        raise ValueError("low_volatility sub-period split rule drifted")
    if sub_period.get("natural_calendar_year_split_allowed") is not False:
        raise ValueError("low_volatility calendar-year split must remain forbidden")
    if sub_period.get("requires_both_halves_mean_excess_positive") is not True:
        raise ValueError("low_volatility sub-period both-halves requirement drifted")
    if sub_period.get("report_each_half_hac_t_stat") is not True:
        raise ValueError("low_volatility sub-period per-half HAC reporting drifted")

    risk_gate = design.get("risk_gate") or {}
    if risk_gate.get("method") != "rolling_overlapping_monthly_tranche_portfolio_nav":
        raise ValueError("low_volatility risk-gate method drifted")
    if risk_gate.get("benchmark_construction") != "option_a_parallel_same_as_of_schedule_horizon_and_ramp_holding_csi300_total_return_instead_of_selected_basket":
        raise ValueError("low_volatility risk-gate benchmark construction drifted")
    if risk_gate.get("cost_applied_to_benchmark_tranches") is not False:
        raise ValueError("low_volatility risk-gate benchmark cost flag drifted")
    if risk_gate.get("relative_nav_formula") != "strategy_nav_divided_by_benchmark_nav":
        raise ValueError("low_volatility relative-NAV formula drifted")
    if risk_gate.get("primary_risk_metric") != "max_drawdown_of_relative_nav":
        raise ValueError("low_volatility primary risk metric drifted")
    if risk_gate.get("minimum_allowed_relative_nav_drawdown") != MIN_ALLOWED_RELATIVE_NAV_DRAWDOWN:
        raise ValueError("low_volatility risk-gate drawdown threshold drifted")
    if risk_gate.get("threshold_frozen_before_run") is not True:
        raise ValueError("low_volatility risk-gate frozen-threshold flag drifted")
    if risk_gate.get("summed_overlapping_cohort_excess_drawdown_as_gate_allowed") is not False:
        raise ValueError("low_volatility summed-excess drawdown gate must remain forbidden")
    ...
    for field in [
        ...
        "report_trailing_window_coverage",
    ]:
        if diagnostics.get(field) is not True:
            raise ValueError(f"low_volatility diagnostic reporting gate drifted: {field}")
    if diagnostics.get("diagnostics_can_define_alpha") is not False:
        raise ValueError("low_volatility diagnostics must not define alpha")

    anti = design.get("anti_p_hacking_controls") or {}
    if anti.get("test_budget_units") != 1:
        raise ValueError("low_volatility test budget units drifted")
    for field in [
        "parameter_sweep_allowed",
        "universe_n_search_allowed",
        "factor_definition_search_allowed",
        "trailing_window_search_allowed",
        "idiosyncratic_vs_total_vol_search_allowed",
        "multi_factor_composite_search_allowed",
        "post_result_rescue_slicing_allowed",
    ]:
        if anti.get(field) is not False:
            raise ValueError(f"low_volatility anti-p-hacking control drifted: {field}")
    if anti.get("risk_gate_threshold_frozen_before_run") is not True:
        raise ValueError("low_volatility frozen-risk-gate control drifted")
    if anti.get("new_ledger_required_before_any_followup") is not True:
        raise ValueError("low_volatility new-ledger follow-up rule drifted")

    hygiene = design.get("pit_and_hygiene_controls") or {}
    if hygiene.get("restatement_exclusion_required") is not False:
        raise ValueError("low_volatility restatement-exclusion-not-required flag drifted")
    if not hygiene.get("restatement_exclusion_not_applicable_reason"):
        raise ValueError("low_volatility restatement not-applicable reason missing")
    if hygiene.get("pit_namechange_required") is not True:
        raise ValueError("low_volatility PIT namechange requirement drifted")
    if hygiene.get("current_stock_basic_name_veto_allowed") is not False:
        raise ValueError("low_volatility current stock_basic name veto must remain forbidden")
    if hygiene.get("trailing_window_uses_only_pre_as_of_daily_returns") is not True:
        raise ValueError("low_volatility trailing-window pre-as-of requirement drifted")
    if hygiene.get("no_look_ahead_in_trailing_window_required") is not True:
        raise ValueError("low_volatility trailing-window no-look-ahead requirement drifted")
    for field in [
        ...
        if hygiene.get(field) is not False:
            raise ValueError(f"low_volatility tracked summary hygiene drifted: {field}")

    planned = prereg.get("planned_test_budget") or {}
    if planned.get("test_budget_units") != 1:
        raise ValueError("low_volatility planned test budget drifted")
    if planned.get("required_preregistration_review_status_for_execution") != "passed_independent_review_ready_for_freeze":
        raise ValueError("low_volatility required execution review status drifted")
    return prereg
...
def load_and_validate_ledger(path: Path = LEDGER_PATH) -> dict[str, Any]:
    ...
    if ledger.get("family_id") != LEDGER_FAMILY_ID:
        raise ValueError("low_volatility ledger family drifted")
    if ledger.get("ledger_status") != "active_planned_test_pending_review":
        raise ValueError("low_volatility ledger is not in the active pre-execution state")
    policy = ledger.get("budget_policy") or {}
    if policy.get("tests_spent_count") != 0:
        raise ValueError("low_volatility singleton signal-search test was already spent")
    if policy.get("tests_available_without_new_review") != 0:
        raise ValueError("low_volatility ledger must not allow unreviewed tests")
    if policy.get("next_test_requires_reviewed_preregistration") is not True:
        raise ValueError("low_volatility ledger must require reviewed preregistration")
    if policy.get("next_test_requires_user_approval") is not True:
        raise ValueError("low_volatility ledger must require user approval")
    planned = ledger.get("planned_tests") or []
    if len(planned) != 1 or planned[0].get("test_id") != PLANNED_TEST_ID:
        raise ValueError("low_volatility planned test mismatch")
    if planned[0].get("planned_status") != "planned_not_reviewed":
        raise ValueError("low_volatility planned test status drifted")
    # Prove this exact singleton maps to this preregistration and this result, spends exactly one
    # test, and is the user-approved planned test, so a schema-valid but misrouted / multi-spend
    # ledger cannot unlock execution.
    if planned[0].get("planned_preregistration_ref") != display_path(PREREGISTRATION_PATH):
        raise ValueError("low_volatility ledger planned preregistration ref drifted")
    if planned[0].get("planned_result_ref") != display_path(SUMMARY_PATH):
        raise ValueError("low_volatility ledger planned result ref drifted")
    if planned[0].get("expected_tests_spent") != 1:
        raise ValueError("low_volatility ledger expected_tests_spent must be exactly 1")
    if planned[0].get("approval_status") != "user_approved_pending_review":
        raise ValueError("low_volatility ledger planned test approval status drifted")
    if ledger.get("test_spend_log") != []:
        raise ValueError("low_volatility ledger spend log must be empty before execution")
    return ledger
...
def load_and_validate_market_cap_audit_report(path: Path = MARKET_CAP_AUDIT_REPORT_PATH) -> dict[str, Any]:
    ...
    if report.get("schema_name") != "a_long_large_cap_market_cap_audit_report":
        raise ValueError("low_volatility market-cap audit report schema_name mismatch")
    decision = report.get("decision") or {}
    if decision.get("audit_status") != "passed_large_cap_market_cap_audit_for_signal_package":
        raise ValueError("low_volatility market-cap audit must pass before execution")
    if decision.get("hard_checks_pass") is not True:
        raise ValueError("low_volatility market-cap audit hard checks must pass")
    if decision.get("signal_search_authorized_by_this_report") is not False:
        raise ValueError("low_volatility market-cap audit must not self-authorize signal search")
    if decision.get("alpha_found") is not False:
        raise ValueError("low_volatility market-cap audit must not claim alpha")
    boundary = report.get("audit_boundary") or {}
    if boundary.get("monthly_as_of_dates") != MONTHLY_AS_OF_DATES:
        raise ValueError("low_volatility market-cap audit as-of dates drifted")
    if boundary.get("selected_market_cap_field") != SELECTED_MARKET_CAP_FIELD:
        raise ValueError("low_volatility market-cap audit selected field drifted")
    if boundary.get("universe_size_n") != UNIVERSE_SIZE_N:
        raise ValueError("low_volatility market-cap audit universe size drifted")
    ...
    if metrics.get("total_unresolved_outside_prior_audited_universe_observations") != 0:
        raise ValueError("low_volatility market-cap audit still has unresolved bridge gaps")
    if metrics.get("documented_data_quality_exclusion_observations") != 1:
        raise ValueError("low_volatility market-cap audit documented exclusion count drifted")
    if metrics.get("signal_universe_backfill_observations") != 1:
        raise ValueError("low_volatility market-cap audit backfill count drifted")
    return report


def load_full_main_board_sources(
    full_raw_root: Path,
) -> tuple[dict[str, Any], base.audit.PayloadStore, base.SignalContext, int]:
    # low_volatility is a price-only signal: it reads the daily adj_factor total-return close series
    # and applies no fundamental restatement-ambiguity exclusion (that exclusion does not affect
    # price-only signal scoring). The full-main-board audit PASS, benchmark route amendment,
    # materialization manifest, and PIT signal context are still validated and reused unchanged.
    ...
    return full_audit_report, store, context, len(manifest)
...
def load_large_cap_signal_universes(
    *,
    ...
    if len(endpoint_results) != len(MONTHLY_AS_OF_DATES):
        raise ValueError("low_volatility materialization endpoint count drifted")
    for result, as_of in zip(endpoint_results, MONTHLY_AS_OF_DATES):
        if result.get("trade_date") != as_of:
            raise ValueError("low_volatility materialization as-of order drifted")
        raw_path = cap_audit.resolve_raw_ref(market_cap_raw_root, result.get("raw_payload_ref"))
        payload = read_json(raw_path)
        if payload.get("call_status") != "success":
            raise ValueError(f"low_volatility market-cap raw payload did not succeed: {result.get('call_id')}")
        ...
        if len(ranked) < UNIVERSE_SIZE_N:
            raise ValueError(f"low_volatility ranked universe is smaller than {UNIVERSE_SIZE_N}: {as_of}")
    ...
    if diagnostics["incomplete_large_cap_universe_month_count"]:
        raise ValueError("low_volatility signal universe is incomplete after reviewed exclusion/backfill")
    if diagnostics["outside_prior_audited_universe_after_backfill_observation_count"]:
        raise ValueError("low_volatility signal universe contains symbols outside prior audited full-main-board raw universe")
    if diagnostics["documented_data_quality_exclusion_observation_count"] != 1:
        raise ValueError("low_volatility documented data-quality exclusion observation count drifted")
    if diagnostics["backfilled_after_documented_exclusion_observation_count"] != 1:
        raise ValueError("low_volatility backfill observation count drifted")
    return universes, diagnostics


def trailing_window_dates_by_as_of(trade_dates: list[str]) -> dict[str, list[str]]:
    """For each monthly as-of, the trailing window of up to 252 global trading days <= as_of.
    Uses only pre/at-as-of trading dates, so the signal carries no look-ahead."""
    windows: dict[str, list[str]] = {}
    for as_of in MONTHLY_AS_OF_DATES:
        cutoff = bisect.bisect_right(trade_dates, as_of)
        windows[as_of] = trade_dates[max(0, cutoff - TRAILING_WINDOW_TRADING_DAYS):cutoff]
    return windows


def downside_semideviation(returns: list[float]) -> float:
    """Sample downside semideviation versus a zero target: sqrt(sum(min(r,0)^2)/(n-1)). Diagnostic only."""
    count = len(returns)
    if count < 2:
        return 0.0
    downside_sum_sq = sum(min(value, 0.0) ** 2 for value in returns)
    return math.sqrt(downside_sum_sq / (count - 1))


def idiosyncratic_volatility(stock_returns: list[float], benchmark_returns: list[float]) -> float | None:
    """Sample standard deviation of OLS residuals of stock daily returns on CSI300 daily returns over
    the trailing window. Diagnostic only; returns None when the market variance is zero."""
    count = len(stock_returns)
    if count < 2:
        return None
    mean_x = mean(benchmark_returns)
    mean_y = mean(stock_returns)
    variance_x = sum((x - mean_x) ** 2 for x in benchmark_returns)
    if variance_x <= 0:
        return None
    covariance = sum((x - mean_x) * (y - mean_y) for x, y in zip(benchmark_returns, stock_returns))
    beta = covariance / variance_x
    alpha = mean_y - beta * mean_x
    residuals = [y - alpha - beta * x for x, y in zip(benchmark_returns, stock_returns)]
    return stdev(residuals)


def realized_volatility_signals(
    *,
    window_dates: list[str],
    stock_close_by_date: dict[str, float],
    benchmark_close_by_date: dict[str, float],
) -> dict[str, float]:
    """Compute the frozen low_volatility primary factor and the two diagnostic factors from daily
    total-return closes over the trailing window. Daily simple returns are taken only between
    calendar-adjacent trading dates where both closes exist and are positive, so a suspension breaks
    the pair instead of spanning the gap. A factor is emitted only when at least the minimum number of
    valid daily returns is available."""
    stock_returns: list[float] = []
    paired_stock_returns: list[float] = []
    paired_benchmark_returns: list[float] = []
    for index in range(1, len(window_dates)):
        prev_date = window_dates[index - 1]
        cur_date = window_dates[index]
        prev_close = stock_close_by_date.get(prev_date)
        cur_close = stock_close_by_date.get(cur_date)
        if prev_close is None or cur_close is None or prev_close <= 0 or cur_close <= 0:
            continue
        stock_return = (cur_close / prev_close) - 1.0
        stock_returns.append(stock_return)
        prev_bench = benchmark_close_by_date.get(prev_date)
        cur_bench = benchmark_close_by_date.get(cur_date)
        if prev_bench is None or cur_bench is None or prev_bench <= 0 or cur_bench <= 0:
            continue
        paired_stock_returns.append(stock_return)
        paired_benchmark_returns.append((cur_bench / prev_bench) - 1.0)

    values: dict[str, float] = {}
    if len(stock_returns) >= MIN_VALID_DAILY_RETURNS:
        values[PRIMARY_FACTOR] = -stdev(stock_returns)
        values["downside_semideviation_low"] = -downside_semideviation(stock_returns)
    if len(paired_stock_returns) >= MIN_VALID_DAILY_RETURNS:
        idio = idiosyncratic_volatility(paired_stock_returns, paired_benchmark_returns)
        if idio is not None:
            values["idiosyncratic_volatility_vs_csi300_low"] = -idio
    return values
...
def update_primary_size_coverage_diagnostics(
    ...
    # quintile. A point-in-time startup month where some quintile has fewer than the minimum
    # primary-factor observations (here, early-panel months where most names do not yet have a full
    # trailing realized-volatility window) cannot form an across-quintile size-neutral score, so it
    # is not a size-neutral cohort-forming month and is excluded, exactly like the zero-score startup
    # months. This is PIT trailing-window availability, not a threshold relaxation: the minimum and
    # every decision gate are unchanged, and the minimum-monthly-cohorts gate still protects against
    # too few valid months.
...
def monthly_cohort_rows(
    *,
    ...
    benchmark_close_by_date = {date: row["close"] for date, row in index_prices[PRIMARY_BENCHMARK].items()}
    window_dates_by_as_of = trailing_window_dates_by_as_of(context.trade_dates)
    stock_price_cache: dict[str, dict[str, dict[str, float]]] = {}
    stock_close_by_date_cache: dict[str, dict[str, float]] = {}
    ...
    diagnostics: dict[str, Any] = {
        ...
        "trailing_window_trading_days": TRAILING_WINDOW_TRADING_DAYS,
        "realized_vol_min_valid_daily_returns": MIN_VALID_DAILY_RETURNS,
        "trailing_window_valid_low_volatility_observation_count": 0,
        "insufficient_trailing_window_observation_count": 0,
        "trailing_window_coverage_by_month": [],
        "trailing_window_startup_excluded_month_count": 0,
    ...
    for as_of in MONTHLY_AS_OF_DATES:
        ...
        for member in members:
            ...
            if symbol not in stock_price_cache:
                prices = base.stock_total_return_close_rows(store, symbol)
                stock_price_cache[symbol] = prices
                stock_close_by_date_cache[symbol] = {date: row["close"] for date, row in prices.items()}
            values = realized_volatility_signals(
                window_dates=window_dates,
                stock_close_by_date=stock_close_by_date_cache[symbol],
                benchmark_close_by_date=benchmark_close_by_date,
            )
            if PRIMARY_FACTOR not in values:
                diagnostics["missing_signal_rows"] += 1
                diagnostics["insufficient_trailing_window_observation_count"] += 1
                continue
            valid_low_volatility_count += 1
        ...
        diagnostics["trailing_window_valid_low_volatility_observation_count"] += valid_low_volatility_count
        diagnostics["trailing_window_coverage_by_month"].append(
            {
                "as_of": as_of,
                "pit_member_count": pit_member_count,
                "low_volatility_valid_count": valid_low_volatility_count,
                "insufficient_trailing_window_count": pit_member_count - valid_low_volatility_count,
            }
        )
    ...
    startup_excluded = sorted(
        set(diagnostics["primary_no_cohort_zero_score_months"])
        | set(diagnostics["primary_incomplete_size_coverage_months"])
    )
    diagnostics["trailing_window_startup_excluded_months"] = startup_excluded
    diagnostics["trailing_window_startup_excluded_month_count"] = len(startup_excluded)
    return rows, stock_price_cache, diagnostics


def result_specs() -> list[dict[str, str | int]]:
    specs: list[dict[str, str | int]] = []
    # low_volatility across all views (equal-weight) including the primary industry_size_neutral 504d CSI300.
    for view in LOW_VOLATILITY_VIEWS:
        for horizon in HORIZONS:
    ...
    # low_volatility cap-weighted diagnostic (industry_size_neutral only).
    for horizon in HORIZONS:
        for benchmark in [PRIMARY_BENCHMARK, SECONDARY_BENCHMARK]:
            specs.append(
                {
...
def summarize_results(
    ...
    for spec in result_specs():
        ...
        # Incomplete-size-coverage (trailing-window startup) months cannot form a valid across-quintile
...
def decision_from_results(
    ...
    if primary is None:
        raise ValueError("low_volatility result set is missing the single primary decision cell")
    ...
    if statistical_clue and tradeable_candidate:
        ...
        plain = (
            "Large-cap low_volatility passed the frozen CSI300 504d statistical-alpha-clue gates and the "
        ...
        verdict = STATISTICAL_ALPHA_CLUE_VERDICT
        plain = (
            "Large-cap low_volatility passed the frozen statistical-alpha-clue gates but failed the rolling "
        ...
        plain = (
            "Large-cap low_volatility failed at least one frozen statistical-alpha-clue gate, so it is falsified "
    ...
    return {
        ...
        "next_action": (
            ...
            "factors, an alternative trailing window, idiosyncratic volatility, threshold, horizon, benchmark, or "
...
def validate_pipeline_result_sanity(
    ...
    if not rows:
        raise ValueError(
            "low_volatility signal-search pipeline failure: no evaluated return rows; do not emit a verdict or spend ledger"
        )
    primary_cells = [item for item in results if item.get("diagnostic_role") == "primary_decision_cell"]
    if len(primary_cells) != 1:
        raise ValueError("low_volatility signal-search pipeline failure: result set must contain exactly one primary cell")
    primary = primary_cells[0]
    if primary.get("cell_id") != PRIMARY_RESULT_CELL_ID:
        raise ValueError("low_volatility signal-search pipeline failure: primary cell identity drifted")
    if int(primary.get("monthly_cohort_count") or 0) == 0:
        raise ValueError(
            "low_volatility signal-search pipeline failure: primary cell has zero cohorts; do not emit a verdict or spend ledger"
        )
    if primary_series is None or len(primary_series.get("cohort_returns") or []) < 2:
        raise ValueError(
            "low_volatility signal-search pipeline failure: primary cohort series is too short for a sub-period split"
        )
    if int(diagnostics.get("primary_size_neutral_thin_month_count") or 0) != 0:
        raise ValueError(
            "low_volatility signal-search pipeline failure: primary size-neutral bucket coverage is thin; "
    ...
    if coverage_month_count < int(primary.get("monthly_cohort_count") or 0):
        raise ValueError(
            "low_volatility signal-search pipeline failure: primary size-neutral coverage month count "
            "is below the primary cohort count"
        )
    # Trailing-window startup months (zero-score + incomplete-size-coverage) must be disjoint from the
    # primary decision cohort so that no insufficient-trailing-window month leaks into the primary result.
    ...
    if startup_excluded_months & primary_cohort_as_ofs:
        raise ValueError(
            "low_volatility signal-search pipeline failure: a trailing-window startup month "
...
def validate_summary_internal_consistency(summary: dict[str, Any]) -> None:
    ...
    The summary schema pins cell identity and decision flag/verdict consistency, but JSON Schema
    draft-07 cannot compare a count field to its list length, so those invariants are enforced here.
    Defense in depth: cell-id<->metadata and decision consistency are re-checked too."""
    ...
    if len(primary_cells) != 1 or primary_cells[0].get("cell_id") != PRIMARY_RESULT_CELL_ID:
        raise ValueError("low_volatility summary must contain exactly one primary cell with the frozen primary cell id")
    for cell in cells:
        ...
        if cell.get("cell_id") != expected_cell_id:
            raise ValueError(
                f"low_volatility summary cell_id/metadata mismatch: {cell.get('cell_id')} vs {expected_cell_id}"
            )
    ...
    if (decision.get("research_verdict") == STATISTICAL_ALPHA_CLUE_VERDICT) is not bool(clue):
        raise ValueError("low_volatility summary research_verdict contradicts is_statistical_alpha_clue")
    if decision.get("primary_cell_passed_statistical_gates") is not bool(clue):
        raise ValueError("low_volatility summary primary_cell_passed_statistical_gates contradicts is_statistical_alpha_clue")
    if decision.get("statistical_alpha_clue_count") != (1 if clue else 0):
        raise ValueError("low_volatility summary statistical_alpha_clue_count contradicts is_statistical_alpha_clue")
    if decision.get("tradeable_candidate_count") != (1 if tradeable else 0):
        raise ValueError("low_volatility summary tradeable_candidate_count contradicts is_tradeable_candidate")
    if bool(tradeable) is not (bool(clue) and bool(decision.get("relative_nav_drawdown_gate_passed"))):
        raise ValueError(
            "low_volatility summary is_tradeable_candidate must equal (is_statistical_alpha_clue and relative_nav_drawdown_gate_passed)"
        )
    ...
    for count_field, list_field in [
        ...
        ("trailing_window_startup_excluded_month_count", "trailing_window_startup_excluded_months"),
    ]:
        if diagnostics.get(count_field) != len(diagnostics.get(list_field) or []):
            raise ValueError(f"low_volatility summary {count_field} does not match len({list_field})")
    expected_startup_excluded = sorted(
        set(diagnostics.get("primary_no_cohort_zero_score_months") or [])
        | set(diagnostics.get("primary_incomplete_size_coverage_months") or [])
    )
    if (diagnostics.get("trailing_window_startup_excluded_months") or []) != expected_startup_excluded:
        raise ValueError(
            "low_volatility summary trailing_window_startup_excluded_months must be the sorted union of "
            "the zero-score and incomplete-size startup month lists"
        )
    if diagnostics.get("result_cell_count") != len(cells):
        raise ValueError("low_volatility summary result_cell_count does not match the number of result cells")
...
def build_summary(
    *,
    ...
    full_audit_report, store, context, endpoint_results_count = load_full_main_board_sources(full_raw_root)
    ...
    return {
        ...
        "search_design": {
            ...
            "low_volatility_views_reported": list(LOW_VOLATILITY_VIEWS),
            ...
            "restatement_exclusion_required": False,
            "restatement_exclusion_not_applicable_reason": (
                "low_volatility is a price-only signal with no fundamental restatement dependency, so the fundamental "
                "restatement-ambiguity exclusion does not apply to signal scoring"
            ),
        ...
        "limitations": [
            ...
            "low_volatility is externally literature-motivated rather than in-sample-diagnostic-derived (cleaner provenance than cash_conversion), but it is still a single 2018-2025 in-sample period test and the vs-CSI300 evidence is single-sample; even a passing result is research-only.",
            "It was selected at the program level after three prior spent singletons, so the new singleton ledger bounds but does not eliminate program-level multiple testing.",
            "Statistical power is limited: 504d holds plus the trailing 252d volatility window bound cohorts at both ends, so the HAC t-stat rests on a short effective sample.",
...
def spend_ledger_after_success(
    *,
    ...
    ledger["test_spend_log"] = [
        {
            ...
            "allowed_followup": (
                "No rerun, factor-definition change, trailing-window change, volatility-estimator change, "
                "idiosyncratic-vs-total search, horizon change, benchmark change, universe change, diagnostic rescue, "
                "or result slicing without a new reviewed preregistration and ledger. "
    ...
    ledger["next_required_actions"] = [
        "Do not rerun or rescue this large-cap low_volatility signal search without a new reviewed preregistration and ledger update.",
...
def write_summary_and_spend_ledger(*, summary_path: Path, summary: dict[str, Any], generated_at: str) -> None:
    if summary_path.exists():
        raise FileExistsError(
            f"refusing to overwrite existing low_volatility signal-search summary: {display_path(summary_path)}"
        )
