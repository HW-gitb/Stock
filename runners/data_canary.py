...
每周选股完成后跑一次，对 Tier1 候选随机抽样，对比 close/pe/pb/name
在 Tushare（egs_main.py 已落盘）和 akshare（实时快照）之间是否一致。
...
- pe/pb 差异 > 10% → warning；数量级 / 符号差异 → warning
...
def _compare_one(row, ak_row) -> dict:
    rec = {
    ...
    rec["name_akshare"] = str(ak_row[AK_SPOT_FIELDS["name"]])

    diffs = []

    # name (忽略 ST 前缀)
    if _normalize_name(row.get("name")) != _normalize_name(ak_row[AK_SPOT_FIELDS["name"]]):
        diffs.append({"field": "name", "ts": rec["name_tushare"],
                      "ak": rec["name_akshare"], "severity": "warn"})

    # close
    d = _compare_field("close", row.get("close"), ak_row[AK_SPOT_FIELDS["close"]],
                       CLOSE_WARN_PCT, CLOSE_ERROR_PCT)
    if d:
        diffs.append(d)

    # pe (prefer ttm, fall back to static; ak 是动态 PE)
    pe_ts, pe_source = _pick_pe(row)
    if pe_ts is not None:
        rec["pe_tushare_source"] = pe_source
        d = _compare_field("pe", pe_ts, ak_row[AK_SPOT_FIELDS["pe"]], PE_PB_WARN_PCT)
        if d:
            d["note"] = (d.get("note") or "") + f"|ts_uses_{pe_source}|ak_uses_dynamic"
            diffs.append(d)

    # pb
    d = _compare_field("pb", row.get("pb"), ak_row[AK_SPOT_FIELDS["pb"]], PE_PB_WARN_PCT)
    if d:
        diffs.append(d)
...
def main() -> int:
    ...
    print(f"[INFO] fetching akshare stock_zh_a_spot_em ({sample_size} candidates to check)...")
    try:
    ...
    payload = {
        ...
        "sample_seed": seed,
        "akshare_source": "stock_zh_a_spot_em (real-time snapshot, 东财源)",
        "summary": {
        ...
        "thresholds": {
            ...
            "pe_pb_warn_pct": PE_PB_WARN_PCT,
            "missing_error_count": MISSING_ERROR_COUNT,
        },
        "comparisons": comparisons,
        "limitations": [
            "akshare spot 是实时/最近收盘快照，非历史 PIT；若脚本晚于 as_of 超过 1-2 个交易日，pe/pb 可能已偏移。",
            "Tushare 的 pe 优先用 pe_ttm（滚动），akshare 的 pe 是动态 PE，口径轻微差异。",
