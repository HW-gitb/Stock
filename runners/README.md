...
- `weekly_screening.ps1` — 周五一键脚本：依次跑 `egs_main.py` + `data_canary.py` + `forward_tracker.py` + **M6.7 advisory**(Slice 3b-2: build market IV feed + `a_short_weekly_pipeline.py`, 语义 cninfo+DeepSeek 行内 -> `research/results/a_short/<as_of>/weekly_m67.json`; `-Account` 传 available_cash 供 sizing, `-SkipSemanticRisk` 可关, advisory 旁路非阻断)；historical `-AsOf` requires explicit `-L3Mode pit` / `neutralize`, rejects `today`, and refuses to overwrite existing official outputs unless `-AllowHistoricalOverwrite` is passed.
