...
- `weekly_screening.cmd` / `weekly_screening.ps1` — 周实盘一键入口及实现：依次跑 EGS、canary、forward capture + cache-only backfill、唯一操作输出 M6.7、V14.3 comparison sidecar。`-Account` 只接受五张手工 CSV 经转换器生成、摘要绑定的 `a_short_account_bundle`；确认文件精确转发，持仓确认必须与账户 bundle 同用并在私有路径验证。请求后 M6.7 输入失败会写 failed receipt 并非零退出；语义和 V14.3 仍是 advisory/comparison-only，不改生产评分、veto 或建仓授权。historical `-AsOf` 必须显式 `-L3Mode pit|neutralize`。
