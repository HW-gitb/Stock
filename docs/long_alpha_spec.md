...
Current blocked finding: the local cache-only audit passed all 6 planted-violation self-tests, but local A-short derived `financial_*.pkl` caches omit `ann_date` / `end_date`; raw PIT income / balancesheet / cashflow / fina_indicator lineage, full PIT universe proof, dividend / total-return source, and terminal delisting return lineage were not present. `docs/a_long_tushare_route_validation_execution_summary_20260604.json` executed a small existing-account Tushare route validation, and `docs/a_long_tushare_route_gap_repair_execution_summary_20260604.json` repaired the two failed route pieces at tiny-sample field-presence level. `docs/a_long_tushare_incremental_materialization_packet_20260604.json` now fixes a 2022-2023 thin-slice materialization packet, but it has not run. This still does not make the A-long data usable: execute the reviewed thin slice, then re-audit the data-route pieces before any A-long signal-search preregistration.
