...
Current blocked finding: the local cache-only audit passed all 6 planted-violation self-tests, but local A-short derived `financial_*.pkl` caches omit `ann_date` / `end_date`; raw PIT income / balancesheet / cashflow / fina_indicator lineage, full PIT universe proof, dividend / total-return source, and terminal delisting return lineage were not present. `docs/a_long_tushare_route_validation_execution_summary_20260604.json` has since executed a small existing-account Tushare route validation. It is only `partial_or_failed_field_presence`: raw PIT fundamentals, PIT universe, price / adjustment / dividend, and benchmark fields passed the small field-presence check, but SW industry membership fields and terminal delisting daily price coverage remain unresolved. Repair and re-audit these data-route pieces before any A-long signal-search preregistration.
