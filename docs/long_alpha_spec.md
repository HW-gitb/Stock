...
Current blocked finding: the local cache-only audit passed all 6 planted-violation self-tests, but local A-short derived `financial_*.pkl` caches omit `ann_date` / `end_date`; raw PIT income / balancesheet / cashflow / fina_indicator lineage, full PIT universe proof, dividend / total-return source, and terminal delisting return lineage were not present. `docs/a_long_tushare_route_validation_execution_summary_20260604.json` executed a small existing-account Tushare route validation, and `docs/a_long_tushare_route_gap_repair_execution_summary_20260604.json` repaired the two failed route pieces at tiny-sample field-presence level. `docs/a_long_tushare_incremental_materialization_execution_summary_20260604.json` records a successful 2022-2023 thin-slice materialization run, and `research/results/a_long_materialized_thin_slice_data_integrity_audit_20260604/audit_report.json` records `passed_thin_slice_data_integrity_not_alpha_ready`. The first 2018-2025 fixed-panel run failed because all 9 `daily` price calls returned 0 rows; `docs/a_long_tushare_daily_price_route_diagnostic_execution_summary_20260604.json` proved the 8-year `daily` route works in isolation, so the failure was pacing / rate-limit, not window size. The repaired `docs/a_long_tushare_broader_materialization_execution_summary_20260604.json` now records 9 paced daily refetches, 62 reused raw payloads, 11/11 table rollups passed, and `passed_full_period_panel_materialization_shape`. This still does not make the A-long data usable for alpha: only a later full reviewed data-integrity audit can allow A-long signal-search preregistration.
