...
Current blocked finding: the local cache-only audit first failed because raw PIT and total-return sources were missing. The later Tushare route-validation, route-gap repair, thin-slice materialization / audit, daily-route diagnostic, and paced fixed-panel materialization steps made a 2018-2025 nine-symbol raw panel available, but that historical panel included `300750.SZ` (ChiNext). Because the user only has main-board A-share access, the current A-long path must be main-board-only before any audit rerun or signal-search preregistration. The corrected `000666.SZ` supplement found no usable SW membership source in the current Tushare route; this is now handled as a bounded already-delisted missing-industry exception only for industry-normalization denominators. The symbol must remain in universe, returns, terminal delisting return, risk, and coverage reporting; silent fill or dropping it from returns/risk is forbidden. None of these make A-long data usable for alpha.
