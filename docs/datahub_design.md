...
- **US operating model**: current US DataHub / runner use is active-only + forward-live only; US historical backtests are idea-only and inactive / delisted historical coverage is scoped out unless the user later reopens paid / specialized data.
- **Phase 7c**: define the DataHub shared layer, report contracts, reproducibility plumbing, local resource budget, data-quality monitor, and minimal local read-path plan before broad implementation. Current schema-first baselines are the local job-spec enforcement helper plus `schemas/datahub_shared_layer_contract.schema.json`, `schemas/datahub_report_contract.schema.json`, `schemas/datahub_reproducibility_manifest.schema.json`, `schemas/datahub_data_quality_monitor_contract.schema.json`, and `schemas/datahub_minimal_a_share_read_path_plan.schema.json`; implementation is still separate.
...
`schemas/datahub_data_quality_monitor_contract.schema.json` and `docs/datahub_data_quality_monitor_contract_20260603.json` define future data-quality checks: coverage, freshness, schema drift, PIT/as-of integrity, survivorship/security master, corporate actions / revisions, calendar / timezone, provider incident / quota, and outlier / revision-rate monitoring. Plain result: if these checks are missing or fail, future outputs cannot quietly claim production readiness or ship-gate evidence.

`schemas/datahub_minimal_a_share_read_path_plan.schema.json` and `docs/datahub_minimal_a_share_read_path_plan_20260603.json` define the smallest future implementation starting point: `market=A`, `lane=a_short`, one as-of day, `provider_family=local_cache`, reviewed job spec, job-spec helper pass, manifest, and data-quality summary. Plain result: the first executable DataHub read-path should prove local plumbing on a tiny A-share slice, not jump into provider calls or full-system jobs.

These contracts do not fetch data, call providers, read cache, create DataHub tables, implement a monitor, implement a manifest writer, change runners, authorize production runner consumption, or provide ship-gate evidence. Future implementation must still be a separate reviewed slice with a job spec that passes `engine/datahub/job_spec_contract.py`.
...
- Shared-layer, report, reproducibility manifest, data-quality monitor, and minimal A-share read-path planning contracts exist before DataHub implementation creates reusable tables, reports, manifests, monitors, or read-path jobs. Current baseline: `schemas/datahub_shared_layer_contract.schema.json`, `schemas/datahub_report_contract.schema.json`, `schemas/datahub_reproducibility_manifest.schema.json`, `schemas/datahub_data_quality_monitor_contract.schema.json`, and `schemas/datahub_minimal_a_share_read_path_plan.schema.json`; they are schema-first contracts only and do not authorize implementation.
...
- Data quality / provider drift monitoring exists before provider-backed or local-cache evidence is treated as stable. It must cover coverage, freshness, schema drift, PIT/as-of integrity, survivorship/security master, corporate actions / revisions, calendar / timezone, provider incidents / quota, outliers, revision rate, and silently changed provider semantics.
