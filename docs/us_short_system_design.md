...
> **状态**：US-short 子系统的**单一权威设计**。设计层定稿 + **批1（离线数据契约 + governance preset + CSV→`us_short_account_state` 转换器/对账，§18.2）已实现进 repo 并经 Codex 逐片审查**（`docs/README.md` 已路由全部 schema/preset/converter/tests）；**批2 纯决策引擎（§18.2 全部决策引擎）亦已实现进 repo 并经 Codex 逐片审查**（`docs/README.md` 已路由）；**批3 校验+输出+纸面+比较+lifecycle eval 实现中**（按 §18.2 分刀 schema-first+tests+Codex 审查；首刀 = §10 no-dangling/证据反查/字段 registry 校验器 + 机器层记录契约，已落，进度见 `docs/README.md` / SESSION_LOG）；**批4 周末 pipeline / 批5 provider·live / DataHub / Skill / production evidence 仍未实现，均需用户单独授权**——任何后续写实现仍 schema-first + tests + Codex 审查、多 LLM 串行、**不交叉 A 股**。
...
## 19. 设计结论
US Short System 是一次性完整设计；进攻在选股/发现、克制在执行/下注、同一风险只罚一次；所有"暂定值"挂 forward 校准提醒（§13）。**设计层定稿；本 docs-only landing 已将设计写进 repo（本稿 = 单一权威）+ 降级旧 `docs/us_short_spec.md` 为归档指针 + 更新路由（`docs/README.md` / `AGENTS.md` / `docs/CURRENT.md` / `docs/strategy_design_synthesis.md`）+ 补 `.gitignore` private 路径 + 登记 §18.0 P0 硬门进 register**。**批1（§18.2 离线数据契约 + 手工输入层 = 全部 schema + governance preset + CSV→`us_short_account_state` 转换器/对账）已实现进 repo 并经 Codex 逐片审查**（本地提交至 `b96976c2`，`docs/README.md` 已路由)；**批2 纯决策引擎（§18.2 全部决策引擎）亦已实现进 repo 并经 Codex 逐片审查**；**批3 校验+输出+纸面+比较+lifecycle eval 实现中**（按 §18.2 分刀；首刀 = §10 no-dangling/证据反查/字段 registry 校验器 + 机器层记录契约，已落，进度见 `docs/README.md` / SESSION_LOG）**；批4 周末 pipeline / 批5 provider·live 仍未实现**。下一步 = 批3 续刀（执行顺序 / 并轮策略见 §18.2——离线实现 + 审查 overhead 可并批，provider 授权门 / provider·live / Codex 审查 / §18.0 P0 不并），每片 schema-first + tests + Codex 审查、多 LLM 串行、不交叉 A 股；每个实现 slice 均需用户单独授权。
