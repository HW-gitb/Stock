...
- **正式模式生产（问题2）**：`engine/us_short_weekend_orchestrator.py::_build_analysis_rows` 对未显式给出 `sub_mode` 的新候选只按既有 `selection_bucket` 精确映射：`theme_momentum / overlap → breakout`，`core_top / core_backfill → pullback`；表外值在进入 analysis/price engine 前抛 `WeekendOrchestratorError`，不静默缺省为 pullback。显式 `pullback/breakout` 仍优先；持仓行不参与建仓模式生产；不新增 `auto`、枚举、阈值或路由参数。
...
- **§4/§10 事实同源**：本周核心结论与风险/降级段只从最终 machine rows 生成；private writer 在写入前再次按同一 canonical 投影对账，调用方不得注入自由文本。
...
- ⑨ **非阻断阶段失败提醒**：`stage_outcomes` 是唯一失败事实源；只显示本轮 `weekly_bridge` 前已归一的 `failed_nonblocking`，不重新判断异常、文件或阶段名。若 Serenity overlay 或横幅⑨自身交付失败，由 `weekly_bridge` outcome 兜底。
...
- **成交判定（确定性顺序）**：
  - **`pullback_limit`**：保持现有 Step 0，`open` 不在 `[valid_entry_low, valid_entry_high]` → `not_filled`；开盘在带内后，`low ≤ limit_order_price` → 成交 @ `limit_order_price`，否则 `not_filled`。
  - **`breakout_stop_limit`**：先要求 `breakout_entry_price ∈ [valid_entry_low, valid_entry_high]`；`open > valid_entry_high`（跳空越过追价帽）或 `high < breakout_entry_price`（未触发）→ `not_filled`；否则即使 `open < breakout_entry_price` 也按 `fill = max(open, breakout_entry_price)` 成交，且成交价必不高于 `valid_entry_high`。
