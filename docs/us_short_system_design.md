...
5. Full-candidate Pass2 + full source packet live run, including the live fetch half of the corporate-action / price-adjustment gate: preflight/forecast implemented as `runners/us_short_batch5_full_candidate_pass2_preflight.py` + `docs/us_short_batch5_full_candidate_pass2_preflight_summary_20260706.json`. The canonical preflight reads the current `2404` Pass1-eligible candidate artifact, forecasts `12021` Pass2 + split/dividend capture calls, and blocks before network because current local momentum/theme projections cover only `3` tickers. This is not a repeat of the completed bankruptcy full scan; the completed bankruptcy screen is already consumed by `run_fetch`. The actual full-candidate provider run remains gated by full-candidate local score inputs, explicit call budget, and separate network execution approval.
