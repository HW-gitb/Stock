...
32. 一键自动预算（auto-derived weekly Pass2 budget, no user-typed number；`R-USSHORT-CAPSTONE-STANDING-PASS2-BUDGET-CAP-ONECLICK`；2026-07-22 refined）：§18.0 provider 门的**兼容细化、不放宽 P0**——一键 paper 周跑**不要求用户预先计算或填写任何 Pass2 预算数字**；系统在同一次 run 内从 preflight 漏斗**自动派生**所需 Pass2 call 数，并在**内部安全天花板**内使用（天花板 = 免费档日上限的固定代码常量 ~250，用户不设/不算/不填，仅作 fail-closed 兜底）；**运行一键命令本身 = 该次 provider 授权动作**（§18.0"调用预算须显式批准"由"命令调用=授权 + 天花板=预批准的固定预算上界"满足，非放宽 P0）；派生数 > 天花板 → **fail-closed 拒跑 + upfront 明确提示，绝不静默扩预算/超额/烧完配额再 NO-EMIT**；auto-derive 仅 paper 一键路径默认（`auto_authorize_pass2_budget`，代码已 gate 到 executing paper run），显式 `--pass2-call-budget` 精确路径保留供其它受控用途；实际派生调用数仍记录供审计；改天花板常量须显式改代码 + 记录；天花板/auto-derive **不**授权 provider selection/storage/全 universe 之外任何新边界，SR-PROVIDER-001 其余门不动。
