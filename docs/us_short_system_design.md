...
## 19. 设计结论
US Short System remains the single v1 design: offense lives in selection/discovery, restraint lives in execution/sizing, and the same risk is penalized only once; all provisional values still require forward calibration under section 13. Implementation status is now settled for the offline engineering track: batch1, batch2, batch3, and batch4 weekend pipeline through 4d-ii-o are implemented in repo and Codex-reviewed per slice. Batch5 provider/live, DataHub, Skill, and production evidence remain unimplemented and separately gated; no provider call, live data, DataHub, Skill, production evidence, broker/order path, A-share crossing, or full-size ship-gate claim is authorized by this batch4 closure.
