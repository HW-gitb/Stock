...
5. Full-candidate Pass2 + full source packet live run, including the live fetch half of the corporate-action / price-adjustment gate: local projection-input coverage and preflight/forecast are implemented as `runners/us_short_batch5_full_candidate_projection_inputs.py` + `docs/us_short_batch5_full_candidate_projection_inputs_summary_20260706.json` and `runners/us_short_batch5_full_candidate_pass2_preflight.py` + `docs/us_short_batch5_full_candidate_pass2_preflight_summary_20260706.json`; the full-candidate live runner/schema/test now exist as `runners/us_short_batch5_full_candidate_live_source_packet.py` + `schemas/us_short_batch5_full_candidate_live_source_packet_summary.schema.json` + `tests/provider/test_us_short_batch5_full_candidate_live_source_packet.py`. A no-access Massive alternative access/budget/source-packet plan also exists as `schemas/us_short_batch5_massive_alt_cut5_access_budget_source_packet_plan.schema.json` + `docs/us_short_batch5_massive_alt_cut5_access_budget_source_packet_plan_20260706.json` + `tests/schema/test_us_short_batch5_massive_alt_cut5_access_budget_source_packet_plan_schema.py`; it maps Massive candidate sources for ticker metadata/news/splits/dividends while recording analyst grades as an unresolved source gap, and it authorizes no provider call, source switch, source-packet write, yfinance, DataHub, production, or ship-gate path. A follow-on no-access Massive quota/access bounded probe packet exists as `schemas/us_short_batch5_massive_quota_access_bounded_probe_packet.schema.json` + `docs/us_short_batch5_massive_quota_access_bounded_probe_packet_20260706.json` + `tests/schema/test_us_short_batch5_massive_quota_access_bounded_probe_packet_schema.py`; it pins a future 12-call Massive-only AAPL/MSFT/JPM ticker-overview/news/splits/dividends access/shape probe and clears neither the `9617`/`12021` full-candidate budget nor the analyst-grade gap. The canonical projection-input summary covers the current `2404` Pass1-eligible candidates with `3` real scored momentum/theme rows and `2401` explicit neutral-fill rows per projection; the canonical preflight now sees full local scored+neutral coverage and forecasts `12021` Pass2 + split/dividend capture calls. This is not a repeat of the completed bankruptcy full scan; the completed bankruptcy screen is already consumed by `run_fetch`. The actual full-candidate provider run remains gated by explicit call budget, separate network execution approval, and provider entitlement evidence that can cover the `12021` forecast; current repo evidence for FMP Basic (`250` calls/day) does not clear that budget.
