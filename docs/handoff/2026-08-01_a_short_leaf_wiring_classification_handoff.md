...
**批内三件，按序做**：22a 共享取数层 → 19 融资过热 → 22b 北向回看。22a 是另两件的共同前置。
...
5. **多门相遇取最狠、不相乘**：本门与 #06 节前减仓、以及将来任何现金系数门同时命中时，**取最小的那个系数**，不做连乘。依据 v14.2 `:164`「参数分歧处置：取更保守值」，且连乘会把两个各自合理的门叠成一个没人论证过的深度折扣。
