...
**下一步**：下一次主树真实周跑之后，按 register 里那四条逐行核对 `sidecar_health.json`；四条任一不成立即立新条目。桌面 `2a_testrun0816.md` 的问题 3 / 问题 5 与本节无关，按各自条目推进。**编号退役说明**：用户 2026-08-16 已把该文档的问题 4 与问题 6 删除，register 里两处判定记录经用户许可保留（删掉会让同一现象被重新查证），只把桌面编号标注为已退役；本节标题里的「问题 6」按此理解。

## 2026-08-16 追加：桌面 2a_testrun0816 问题 1 的定性（非审查轮；Claude Code；D:\cnhea\Codex\worktrees\8c7a\Stock）

**这轮做了什么**：用户要求只按桌面文档对问题 1 的**现象描述**判断真伪、不读不评价它自带的修复方案。据此只做读码 + 读四次运行产物，判为**真实漏洞、与跑哪棵树无关**，正文与 closure tests 见 `docs/system_risk_register.md` 的 `R-ASHORT-0816-THEME-BENIGN-COHORT-REJECTION-LANDS-AS-A-SELF-INFLICTED-REASON-CONTRACT-VIOLATION`，本节不复述。**零代码改动、未跑测试、未起 agent。**

**方法学教训四：同一个缺陷会在连续几次运行里换三件衣服，「上次没报」不等于「上次没有」。** 这条本周实证了一整轮：第 1 次被更上游的整体失败盖住（那一轮 theme 根本没走到判定）；第 2 次**同一个假失败照样发生**，只是被一条更早的检查先供了合法 error_code，于是标签变成时钟缺失、没人把它和后面的联系起来；第 3 次纯粹因为 health 读 packet 的时刻早于 packet 被写入 rejection 而躲过。**排查时不要用「历史某次是绿的」来否定一个当前可复现的机制**——要么找出使它当次不成立的确切前置条件，要么就承认那次是侥幸。判据是前置条件，不是历史颜色。
...
## 2026-08-17 追加：Claude Code 独立复审 A-short 2.1（PASS，已提交并合入 master）——过程与方法学

- **合并让位于另一窗口在飞的 merge，然后才合（按 closeout gate item 9 全程留痕）**：首次尝试时主树有一个 `10:41:05Z` 建立、到 `11:27:37Z` 仍未释放的 `.git/index.lock`；再查发现 `MERGE_HEAD` 存在、`docs/system_risk_register.md` 与 `tests/provider/us_short_private_test_root.py` 两个 `UU` 未解决——**那是另一个窗口的 us-short merge 正在做，不是死锁**，所以我既不强推也不 `--abort`，更没有替它解冲突。等其收口成 `f7628f77` 后，残余的 `index.lock` 才判定为陈旧并移除：0 字节、20 秒观察窗内 mtime 不变、系统内无任何 git 进程，三条同时成立才动手。合并用 `merge.autoStash=false`，确保 13 个 `research/results/a_short/` 脏产物（08-16 的旧产物，非本轮所写）全程不被 stash、不被提交。**教训：`index.lock` 单独出现不足以判断死活，要连着 `MERGE_HEAD` / 进程表 / 锁文件大小与 mtime 变化一起看**——第一次我只看到锁，差点把「别人正在干活」误读成「卡住了」。
...
## 2026-08-18 Codex 修复 A-short 4.1（509e）

- **Verdict/Action**：`repaired / OPEN-NOT-VERIFIED`；严格按 Desktop `3a_testrun0816.md` §4.1 执行，未 stage/commit/push/merge；`docs/CURRENT.md` 未改。
- **Required 修复**：新增 `engine/a_short_loss_making_admission.py::apply_loss_making_admission()`，在完整 L5 scoring 后、既有 selector/Stage3 前执行唯一 hard admission。仅有限 `ttm_profit_dedt > 0` 准入；`<=0` → `loss_making_ttm_profit_dedt_non_positive`；None/NaN/Inf/非数值 → `ttm_profit_dedt_unavailable`。不读 `pe_ttm` 作 fallback，不读 q0/单季，不填零，不重算 survivor 分数/Tier/排序。
- **产出与消费者**：`rank_universe_reconciliation` 增加 `loss_making_admission` stage 与窄审计列（`decision_as_of`、`run_revision_id`、TTM、final_score、pre rank、pre/post Top15）；`analysis_input` 升至 1.5.0，新增可选 `rank_exclusion_counts.loss_making_admission`；weekly 将该 leaf 作为 `candidate_admission` 独立摘要，L0 `excluded_counts` 未混入。
- **Optional tracker**：新增 `schemas/a_short_loss_making_exclusion_tracker.schema.json` 与纯 Python tracker 实现；唯一研究产物为 `research/results/a_short/loss_making_exclusion_tracker.json`。live Stage 5 将同一 official revision 的 `result/a_short/<as_of>/revisions/<run_revision_id>/rank_universe_reconciliation.csv` 与 tracker path 传给既有 regime runner；官方 marker/SHA/revision 绑定、同 key 幂等、source conflict 原文件字节不变，复用既有 CSI1000 `000852.SH` close-to-close h1/h3/h5/h10 backfill；不登记 TRACKS/指纹/裁决器，不影响排名/M6.7/Phase 5。
- **Fail-closed 与负向控制**：weekly unknown nonzero rank key、tracker missing/mismatched marker、旧 revision/临时或主树路径、identity mismatch、CSI1000 unavailable、reason/TTM mismatch 均阻断或不更新；旧 L1/L2/rank keys 仍只作历史兼容忽略；`pe_ttm` 空但 TTM 正值仍准入。
- **精确验证**：固定 `C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe`=`Python 3.13.8`；项目 launcher 精确焦点包 `tests=810`、`PASS`、`exit=0`、`receipt:ade7c8142bbb46125c5f3239`；A-short preflight JSON PASS；`git diff --check` PASS。覆盖 rank/Top15 partition、analysis contract、weekly dual maps/lineage、live-only guardrail、tracker official binding/idempotency/conflict/backfill、regime regression、effect contract。
- **原始终态/边界**：本轮 4.1 开始前 Git clean；当前改动仅在 `D:\cnhea\Codex\worktrees\509e\Stock`，未提交。尚未运行文档门禁、唯一 full lane、provider/live、真实暖根同源开关差异、reviewer sub-agent；尚未有第一条真实周 tracker 记录。当前状态必须保持 `OPEN-NOT-VERIFIED`，等待 Claude Code independent review/commit 与用户授权的暖根验收。
- **执行者交接命令**：Claude Code：按 §4.1 对本工作树独立审查；先核固定 Python/精确 full lane 与文档门禁，再做 reviewer/committer 边界内的审查和提交，不要启动 provider/live 或暖根。

## 2026-08-18 追加：A-short 4.1 Optional writer registry 最小修复（509e）

- **执行者/状态**：Codex executor/fixer；仍为 `repaired / OPEN-NOT-VERIFIED`，未 stage/commit/push/merge；只在 `D:\cnhea\Codex\worktrees\509e\Stock` 工作，`docs/CURRENT.md` 未改。
- **问题与根因**：方案规定的 full lane 首次执行在既有 `tests.test_a_short_public_json_writer_nonfinite_guard` fail-closed；AST writer discovery 发现新 tracker 函数 `engine/a_short_loss_making_admission.py:merge_loss_making_exclusion_tracker`，但既有 `PUBLIC_WRITER_FUNCTIONS` registry 未登记该 writer。tracker 本身已使用 finite-only JSON serialization；这是 registry 漏登记，不是业务准入、schema、缓存或写盘边界问题。
- **最小改动与调用链/消费者**：仅在 `tests/test_a_short_public_json_writer_nonfinite_guard.py` 的既有 `PUBLIC_WRITER_FUNCTIONS` 增加一行 `engine/a_short_loss_making_admission.py:merge_loss_making_exclusion_tracker`；不新增 writer/helper、防御层、缓存、目录或 provider。调用链仍为 live Stage 5 → 既有 regime runner → tracker merge → 唯一 research JSON；schema、official marker/SHA/revision binding、source conflict/idempotency 和 h1/h3/h5/h10 backfill 未改。
- **负向控制与精确验证**：固定 Python `C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe`=`Python 3.13.8`；修复后 `tests.test_a_short_public_json_writer_nonfinite_guard tests.test_a_short_loss_making_exclusion_tracker` 为 `Ran 14`、`OK`；文档门 `Ran 68`、`OK`；首次 full lane `discovered=3201`、`ran=1234`、exit 1，失败项为 writer registry，sidecar `.tools\\state\\runs\\20260818T113749_a_short_parallel.jsonl`；按同一方案重跑后 `STATIC=PASS`、`py_compile=12`、`discovered=3201`、`ran=3201`、`equal=True`、`Ran 3201`、`exit=0`、`PASS`、sidecar `.tools\\state\\runs\\20260818T114130_a_short_parallel.jsonl`；此前 4.1 精确焦点仍为 `tests=810`、`PASS`、`receipt:ade7c8142bbb46125c5f3239`。
- **自审/原始终态/交接**：已回扫 writer discovery、finite-only serializer、tracker source binding、写盘唯一性和 live-only 参数边界；本轮 4.1 开始 Git clean，当前仅 509e 有未提交改动；provider/live、真实暖根候选差异、reviewer sub-agent 均未运行。Claude Code：按 §4.1 独立审查固定 Python、文档门和 full lane；PASS 后由 reviewer/committer 提交，Codex 不提交。

## 2026-08-18 追加：Claude Code 独立审查 A-short 4.1（FAIL，未提交）——过程与方法学

- **这轮是本项目里第一次「全绿包 + 四条真缺陷」的典型样本，值得记方法**：焦点超集 `717 OK`、tracker 模块 `4/4`、full lane `3201/3201`，全部为真；四条 Required 没有一条被它们覆盖。原因是这些测试钉的是**函数的输入输出**，而缺陷全在**函数与真实产物之间的接缝**上——CSV 的 dtype、selector 的非单调性、`egs_full` 的下游路由、列名与实际口径的错配。**下次审这类「新门 + 新落盘」的刀，测试全绿只说明单元对，接缝要另外走一遍。**
- **起独立对抗 agent 的判据与分工**：本刀命中 §6a(iii)（新建 fail-closed 准入门）且真删候选，故起 1 个（只读当前工作树——改动未提交，worktree-from-a-ref 看不到）。分工是刻意的：**我走设计意图与文档一致性**（对照桌面方案 §4.1 逐条核 reason id／`rank_exclusion_counts` 可选叶／`excluded_counts` 不混装／weekly 两 map／`candidate_admission` stage／evidence_ref 共同父节点／tracker 不进 TRACKS），**agent 走黑盒不变式**（谓词语料、行守恒、下游泄漏、tracker 幂等、refactor 等价）。重叠只发生在 top15 那条——我先由读代码 + 实测 `top_n=50/watch_n=15` 得出，它独立复现，两条证据互为对照。
- **agent 最有价值的一击是端到端而不是单元**：F1 它没有去挑 `int()` 的写法，而是**用真 `build_rank_universe_reconciliation` + 真 `write_csv_atomic` 造出真实 CSV** 再喂 tracker，才看见 left-merge 把 int64 升成 float64、落盘成 `"1.0"`。它还配了对照组（L0==L5 时 dtype 保持 int64、tracker 成功），把触发条件精确锁定在「存在任何上游排除」——也就是每一周。**「用真产物喂真消费者」这一步是本轮唯一能发现 F1 的方法。**
- **一处我先入为主、被 agent 纠正的地方**：我读完 `apply_loss_making_admission` 后判定「门只做行级 partition，不改选择」，因为它确实不重算分数、不动幸存者顺序。agent 的 F3 证明这个推论错了——门后重跑 selector 时，删掉一只别的行业的票会让增量集中度的分母变小，把一只**合格**票挤出去。**「不改分数」不等于「不改选择」，只要下游选择器对输入集合非单调。** 以后凡是「在选择器之前删行」的改动，都要单独证选择器的单调性，不能从「分数没变」推。
- **未起第二个 agent**：一个已经覆盖到我不深挖的面（tracker 幂等/原子性、refactor 600 例随机等价、谓词 35 值语料），再起会撞配额且重叠。
- **本轮未做/未验**：未跑全量（FAIL 已被真实探针坐实，按分级门③先出结论，不必等包）；暖根真实周候选差异 `NOT_RUN`；未跑 provider/live；未 stage/commit/merge。F3 与 F4 在本周真实宇宙上的发生频率**没有数**，需要暖根跑才知道。

## 2026-08-18 追加：A-short 4.1 reviewer findings 最小修复（509e）

本节是当前 A-short executor/fixer 给 Claude Code reviewer/committer 的详细交接。它只承接本轮 reviewer 对 4.1 的四条 Required 与 tracker Optional；不承接其他线程、其他工作树或历史 handoff 的旧问题/结论。完整风险与 Required 状态同步在 `docs/system_risk_register.md` 顶部对应 4.1 entry；`docs/SESSION_LOG.md` 只保留 cycle 摘要；`docs/CURRENT.md` 不写 review/commit 瞬态状态。

- **执行者/状态**：Codex executor/fixer；当前 `repaired / OPEN-NOT-VERIFIED`。Claude Code 是 reviewer/committer；Codex 未 stage、commit、push 或 merge。工作严格在 `D:\cnhea\Codex\worktrees\509e\Stock`；未读取/修改主树或其他工作树。
- **本轮问题与根因**：① Top15 审计列沿用 `CONF["top_n"]`，而该值为 50；② tracker 测试手写理想 CSV，未验证真实 writer 字节；③ admission 删行后重跑对行业集中度敏感的非单调 selector，会把合格票挤出；④ `egs_full` 被误当成门后合格池，破坏其“粗筛后的全量 rank”语义，影响持仓、crash tracker 与 backtest eligible 消费者。
- **最小 Required 修复**：`A-EGS/egs_main.py` 在 L5 scoring 后先计算门前 Top15 与既有 watch pool；`apply_loss_making_admission()` 后不重新 selector，只按 admitted code 过滤门前集合；`df_full = df_l5_scored` 保持完整 `egs_full` writer 输入；reconciliation 的 `loss_making_admission` stage 使用 `df_admitted`；两列 `pre_admission_top15/post_admission_top15` 使用真实 15 只口径。没有复制 selector、重算分数、改 Tier、改集中度算法或新增防御层。
- **Optional tracker 修复**：`tests/test_a_short_loss_making_exclusion_tracker.py` 由真实 EGS 模块加载 `A-EGS/egs_main.py`，构造 scored frame，调用真实 `apply_loss_making_admission()`、`build_rank_universe_reconciliation()` 和 `write_csv_atomic()` 写出 CSV，再将该产物喂给 `update_loss_making_exclusion_tracker()`；不再手写 `pre_admission_rank` 字节。新增 backtest writer→consumer 用例；已有 writer registry、official marker/SHA、revision/idempotency/conflict/backfill 边界保持。
- **调用链**：`runners/weekly_screening.ps1 → A-EGS/egs_main.py --as-of/--run-revision-id → build_master → score_l1/l2/l3/l4/l5 → admission audit + rank reconciliation → existing selector/Stage3 → official transaction writes rank_universe_reconciliation.csv + egs_full_<as_of>.csv → weekly/holdings/crash/backtest/tracker consumers`。live Stage 5 tracker 参数仍只在 live 分支接入；historical/backtest 不写 tracker state。
- **消费者回扫及处置**：已 grep 并逐类核对 `engine/a_short_egs_full_adapter.py`（完整 EGS rank 定义，保留）、`runners/a_short_weekly_pipeline.py`/`a_short_m67_render.py`（持仓复用完整 EGS 风险字段，保留）、`runners/a_short_crash_veto_tracker.py`（完整 rank stage 集合与 `egs_full` 对齐）、`runners/backtest_rank.py`（真实 full writer 产物仍含亏损行，eligible 基准成员不被门静默缩小）、`runners/diagnose_tier1_bad_signals.py`、`runners/data_canary.py`、`engine/a_short_run_revision.py`（路径/绑定语义不改）以及 EGS writer。full lane 同时覆盖现有 holdings/crash/backtest 相关测试。
- **schema / source-binding**：继续使用既有 `schemas/analysis_input.schema.json` 1.5.0、`schemas/a_short_loss_making_exclusion_tracker.schema.json`、`rank_universe_reconciliation` 列契约；`decision_as_of`、`run_revision_id`、official marker/SHA、revision root、CSI1000 `000852.SH` 回填和 tracker key `(as_of, run_revision_id, ts_code)` 未改。没有引入第二个 schema、source、provider 或 receipt。
- **缓存/写盘边界**：没有新缓存、迁移、raw/provider 目录、state 根或 writer。`egs_full` 与 reconciliation 仍在既有 official output transaction 中写出；tracker 仍只写既有 research artifact。测试产物只写当前工作树下 `TemporaryDirectory(dir=ROOT)` 或系统临时目录；未触发 provider/live、主树暖根、账户/下单或真实周 tracker 记录。
- **fail-closed / 负向控制**：保留有限数值化 `ttm_profit_dedt > 0` admission、`<=0`/unavailable reason；无 `fillna(0)`、PE/q0/单季 fallback。新增反例证明删掉一个行业票后重选会丢合格票，生产路径改为门前集合过滤；新增反向控制证明 `top_n=50` 不可冒充 Top15；完整 rank、crash 集合、backtest eligible、tracker producer/writer/consumer、marker/revision/conflict/idempotency 均有回归。
- **自审项目**：逐项检查 producer→CSV 字节、gate→selector 集合、frame→产物→消费者、Top15 列名/值、adapter docstring、crash hard assertion、backtest opportunity set、schema/source-binding、缓存/写盘、live-only/historical 参数、既有 writer registry、`docs/CURRENT.md` 禁写 review gate；未运行 reviewer sub-agent。
- **精确命令与原始终态**：固定解释器先验证 `C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe --version` → `Python 3.13.8`。最终精确命令为 `.tools\run_unittest_with_repo_pythonpath.cmd tests.phase6.test_egs_rank_universe_reconciliation tests.phase6.test_egs_analysis_input_contract tests.test_a_short_weekly_pipeline tests.phase6.test_weekly_screening_guardrails tests.test_a_short_loss_making_exclusion_tracker tests.test_a_short_regime_comparison tests.test_a_short_regime_comparison_runner tests.test_egs_industry_heat tests.test_a_short_effect_contract`，原始终态 `Ran 813 tests` / `OK` / `exit=0` / receipt `receipt:d70b4e7b472df622007edf16`。随后执行固定 full lane：`C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe .tools\full_pack_ledger.py run a_short a_short_4_1_reviewer_findings_repaired 860 -- discover -s tests -p test_a_short*.py`，`STATIC=PASS`、`py_compile=15`、`discovered=3204`、`ran=3204`、`equal=True`、`Ran 3204 tests`、`PASS exit=0`、fingerprint `b6f686e792f3`、sidecar `.tools\\state\\runs\\20260818T124109_a_short_parallel.jsonl`。第一次重跑前的唯一焦点失败是旧 `test_overlay_is_built_from_post_stage3_weekly_candidates` 只匹配旧 selector 源码形状；只更新该静态守卫以接受同一门前 selector+admitted filter 形状后转绿，未改变生产行为。
- **文档/交接/下一步**：本节与 `docs/system_risk_register.md`、`docs/SESSION_LOG.md` 同步；文档门禁在本节落盘后运行，`git diff --check` 已通过（仅 line-ending warning）。当前 Git 仍是承接的未提交 4.1/相关改动集合，未回滚或清理用户改动。下一步为 `Claude Code：按 §4.1 独立审查`；reviewer PASS 后由 reviewer/committer 提交，full lane PASS 不等于 independent review、provider/live、暖根 closure 或 ship。

## 2026-08-18 追加：Claude Code 复审 A-short 4.1 修复轮（FAIL，未提交）——过程与方法学

- **上一轮四条全闭，修法比我要求的更好，先记下来**：`THE-GATE-CAN-EVICT` 我给的处置是「二选一，需用户裁决」，执行方选了第三条路——**只跑一次门前 selector，再按准入集合过滤**。选择器压根不再看缩小后的 frame，非单调性这条路被物理关掉，比我提的两个选项都干净。`HOLDINGS-LOSE-CLASSIFICATION` 的修法（`df_full` 保持完整 L5 frame）**一步同时关掉了我扫出的另外两条连带面**（adapter docstring 重新为真、`backtest_rank` 的 `excess_eligible` 机会集不再缩小），所以 `backtest_rank.py` 无需改动——这是对的，不是漏改。
- **但这轮验出了「修复本身引入新缺陷」，而且是同一类**：`df_full` 从门后翻成门前，`:2236-2243` 那条既有的 `ranked_count == len(df_full)` 校验就必然不成立，每次真排除票都把 data-health 判成 `error`。上一轮我写的教训是「改产物语义前先 grep 消费者」——**这轮 `df_full` 的语义也被翻了一次，同样需要 grep 它的消费者，而 `build_data_health` 没被扫到**。执行方扫了 `top50` / `watch_df` / backtest 补位 / crash tracker 四个，漏了 health 这个。**记法：语义翻转不分方向，翻回去和翻过来一样要重扫。**
- **本轮我改了自己的打法，直接对着上一轮总结的第 1 条**：不再只读函数体 + 打单元探针，而是**用真 `apply_loss_making_admission` + 真 `build_rank_universe_reconciliation` 造出真的 reconciliation**，再拿它的 `ranked_count` 去对 `len(df_full)`。这条缺陷单看任何一个函数都看不见——`build_rank_universe_reconciliation` 返回 `status=pass` 是对的，`build_data_health` 的比较逻辑也是对的，错的是两者中间那个被翻过的语义。**「造真中间产物、跨两个函数对判据」是这轮唯一能发现它的方法。**
- **测试为什么又没抓到**：我的九模块焦点超集 `772 OK`、执行方 full lane `3204/3204` 都绿。`tests/phase6/test_egs_sw_industry_and_watch_pool_health.py` 确实直调 `build_data_health`，但喂的是 `df_full=frame, watch_df=frame` 的自洽夹具——**夹具自洽 = 判据恒真 = 这条校验在测试里从来没被真正行使过**。这跟上一轮 tracker 夹具手写 `"1"/"2"/"3"` 是同一形态的第三次出现。
- **未起 agent 的判据**：上一轮 agent 已把这一面（准入谓词、行守恒、下游泄漏、tracker、refactor 等价）打穿；本轮是增量修复且 diff 集中在四处已知点，按 rule 8 不再起，改为我自己按「翻过的语义 → 重扫消费者」定向查。
- **本轮已核不再重复的点**：`candidates` 由 `watch_df` 构造（`A-EGS/egs_main.py:1481`），亏损票不会进已发布候选——这条我实读确认，是整刀最危险的旁路，没有发生。
- **本轮未做/未验**：未跑全量（FAIL 已被真实探针坐实，按分级门③先出结论）；暖根真实周候选差异仍 `NOT_RUN`；`universe_summary.full_count` 的最终口径待执行方与方案 §五.2 对齐后再核。
## 2026-08-18 追加：Codex 修复 A-short 4.1 复审 Required（509e，working tree）

本节只承接上一节 Claude Code 复审新开的 `R-ASHORT-41-PREGATE-DF-FULL-BREAKS-THE-RANKED-COUNT-HEALTH-INVARIANT`；不承接其他线程、其他工作树或历史 handoff 的旧问题。复审方案判断正确，按用户要求做最小修复。

- **问题与根因**：4.1 为恢复 `egs_full` 的既有语义把 `df_full` 改回门前完整 `df_l5_scored`，但 `build_data_health` 仍在 `A-EGS/egs_main.py` 用门后 `rank_reconciliation.ranked_count == len(df_full)`。真实 `apply_loss_making_admission` + `build_rank_universe_reconciliation` 三票 probe 得 `ranked_count=2`、`expected_excluded_count=1`，完整 L5 `df_full=3`；旧式比较因此每次真实排除都会写入 rank error，`publish_egs_run_manifest` 随后拒绝 official publish。
- **最小改动**：只改 `build_data_health` 既有对照：`reconciled_full_count = ranked_count + _rank_stage_excluded_count(rank_reconciliation, "loss_making_admission")`，再与 `len(df_full)` 比较；不改 admission 的有限且正值谓词，不改分数/Tier/排序/selector/Stage3，不新增 helper/schema/cache/receipt/provider 或防御层。
- **最终口径**：`df_full` 是完整 L5 scored frame（准入前），因此 `egs_full_<as_of>.csv`、`analysis_input.universe_summary.full_count`、`data_health.metrics.full_count` 均表示该完整 L5 数量；`rank_reconciliation.ranked_count` 是准入后 active 数，`rank_exclusion_counts.loss_making_admission` 是独立排除数。这个选择保留 `engine/a_short_egs_full_adapter.py` 的“粗筛后的全量 rank”定义，保持 holdings、crash tracker、`backtest_rank` opportunity set 不被静默缩小；公开 `candidates.csv` 仍由准入后的 `watch_df` 生成。
- **调用链与消费者**：`weekly_screening.ps1` → `A-EGS/egs_main.py::run_egs` → L0/L1/L2/L3/L4 → `score_l5` → `apply_loss_making_admission` → rank reconciliation → `export_analysis_input` / `export_data_health` → existing official transaction；下游 weekly/holdings/crash/backtest/tracker 继续按既有各自产物消费。此轮只修 health 的计数接缝，未回退上一轮已闭合的 Top15、selector 非单调、tracker 真产物、egs_full 消费者或 writer registry 修复。
- **schema / source-binding**：未修改 `schemas/analysis_input.schema.json`、data-health schema、rank reconciliation 列契约、`decision_as_of`、`run_revision_id`、official marker/SHA、revision root、CSI1000 source binding；reconciliation stage 仍是现有 `loss_making_admission`，不增加第二个口径字段。
- **缓存 / 写盘 / 负向控制**：未新增缓存、迁移、provider/raw/state 根或 writer；`egs_full`、reconciliation CSV、analysis_input、data_health 仍在 existing official output transaction 内写盘；health error 分支仍 fail-closed。新增回归使用真实 admission 与真实 reconciliation，再喂真实 data-health 中间消费者；断言旧口径 `ranked_count != len(scored)`，避免自洽 frame 让判据恒真。未运行 provider/live、full live lane、真实暖根、sub-agent；未写主树。
- **精确验证**：固定解释器存在且版本为 `Python 3.13.8`：`C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe --version`。焦点命令：`.tools\run_unittest_with_repo_pythonpath.cmd tests.phase6.test_egs_rank_universe_reconciliation tests.phase6.test_egs_sw_industry_and_watch_pool_health`；原始终态 `Ran 58 tests` / `OK` / `exit=0` / receipt `receipt:27a8ebc3d03134c9b6828c63`。唯一 full lane：`C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe .tools\full_pack_ledger.py run a_short a_short_4_1_data_health_ranked_count_repaired 860 -- discover -s tests -p test_a_short*.py`；原始终态 `STATIC status=PASS` / `diff_check=PASS` / `py_compile=15` / `discovered=3205` / `ran=3205` / `equal=True` / `Ran 3205` / `RESULT status=PASS exit=0 tests=3205 elapsed=154.5s deadline=860s mode=parallel` / fingerprint `c50de3ba6ea4` / sidecar `.tools\state\runs\20260818T131910_a_short_parallel.jsonl`。随后 `full_pack_ledger.py check a_short` 为 `CACHED GREEN` 且确认 `this EXACT code state`。
- **自审项目**：逐项回扫 producer→rank reconciliation→data-health、门前完整 frame/门后 active 集合、`universe_summary.full_count`/data-health metrics、adapter/holdings/crash/backtest consumers、candidates 排除、schema/source-binding、official writer/transaction、缓存/写盘、fail-closed、旧比较反向控制、`docs/CURRENT.md` 禁写 review gate；未启动 reviewer sub-agent。
- **文档门禁**：`.tools\verify_doc_process.cmd` → `Ran 68 tests` / `OK` / `exit=0`；`DOC_ONLY - acceptance receipt left untouched`。
- **原始终态 / 角色边界 / 下一步**：本修复开始时承接 Claude Code `FAIL / 未提交`，当前为 `repaired / OPEN-NOT-VERIFIED`；Git 仍保留本轮及之前 4.1 未提交集合，未回滚用户改动，未 stage/commit/push/merge。Codex 仅执行/修复；Claude Code 为 reviewer/committer。下一步：`Claude Code：按 §4.1 独立审查本轮 Required 修复；PASS 后由 reviewer/committer 提交。`

## 2026-08-18 追加：Claude Code 独立审查 A-short 4.1 收口（PASS，已提交并合入 master）——过程与方法学

- **这轮我做的关键动作，是替执行方验「他挑的公式对不对」，而不是只验「误报消没消」**：新判据是 `ranked_count + gate 阶段排除`。我先用三票场景确认误报消失（新判据 False、旧判据 True），但那个场景**分不出**它和看起来更自然的 `ranked + expected_excluded_count(全部)`——两者都等于 3。于是我另造了带真实上游 stage 的五票场景（L1 掉 1、L2 掉 1、门掉 1），实测 `ranked+gate=3` 命中而 `ranked+全部=5` 不命中。**根因是 L1/L2 排除掉的行根本没进过 L5 frame，而 `df_full` 就是 L5 frame。** 执行方挑对了不显然的那个。
- **教训（可复用）：验「计数类修复」必须构造让候选公式互相分开的输入。** 修复者往往只需一个能让红转绿的夹具，而那个夹具通常太简单，几个错误公式在上面全都是绿的。审查方的增量价值就在于**造出能把它们分开的那一个场景**。
- **植入两枪也是按这个思路分腿的**：P1 打「新判据是否承重」（还原旧式 → 新用例精确红），P2 打「gate-stage 而非全量这个细节是否承重」（换成全量 → 两条**既有**用例转红）。P2 特别值得记：它证明这个细节**早就有既有测试在守**，只是上一轮没人往那个方向想。
- **三轮下来这刀的缺陷曲线**：第一轮 4 条（2 执行 2 方案），第二轮 1 条（修复自身引入，属「语义翻转未重扫消费者」同类），第三轮 0 条。**第二轮那条恰好印证了第一轮总结的第 2 条教训**——`df_full` 语义被翻回去时，同样需要重扫它的消费者，而 `build_data_health` 漏掉了。**语义翻转不分方向。**
