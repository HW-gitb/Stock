...
- **序 13 liquidity 的处置方向仍未定**：「从 schema 删除整个 `liquidity` 对象」与「保留并标 `true_dangling`」互斥，两条路不能同时走，而序 11 要吃序 13 的结果。**这是用户裁决项，本节不代为拍板。**
- **序 19 在桌面汇总视图里仍只有一条「☆纯裁决」**，没有工程刀那一行；本文件（权威侧）已有完整方案。只读桌面排下一步会低估工作量。
