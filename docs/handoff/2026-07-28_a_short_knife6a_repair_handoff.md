...
- ~~每次从券商重新导出持仓后，先确认 `ts_code` 是后缀式再跑转换器。~~ **已由下一节的转换器改动消除**：券商前缀式 `SZ002747` / `SH600519` 现在被直接接受并在内部改写成后缀式，导出后不必再手工归一。
- ~~待用户裁决：转换器是否应直接接受券商前缀式写法并在内部归一。~~ **2026-08-03 用户已裁决：接受**，实现与边界见下一节。
- 手工表与 bundle 均落 gitignored 私密路径，不得写进任何 tracked 文件或提交；本节只记方法与身份字段，不记金额与持仓。

## 2026-08-03 追加：转换器直接接受券商前缀式 `ts_code`（Claude Code 自修自审；用户裁决后实现）

### 改了什么

`runners/a_short_account_state_from_manual_tables.py::_parse_ts_code`：在原有校验之前加一次**只认一种形态**的改写——`^(SH|SZ)(\d{6})$` → `\2.\1`（输入已先 `.strip().upper()`，故大小写与首尾空格照旧被吸收）。原有的格式校验、错误文案与主板门**一行未动**，改写后的代码照常走它们。

### 为什么改

券商导出普遍是前缀式，转换器只认后缀式，于是每周从券商拉完表都要手工归一一次才能生成 bundle，否则带 `-Account` 的周跑第一步就 FATAL。用户 2026-08-03 裁决：由转换器直接接受。

### 验证命令

```
.tools\run_unittest_with_repo_pythonpath.cmd tests.test_a_short_account_state_from_manual_tables
```

### 验证结果

- `RESULT tier=focused status=PASS exit=0 tests=60`（新增 4 条用例，模块在 lane 选择器内，全量亦覆盖）。
- 新增用例覆盖三面：① 前缀式（含小写、含首尾空格）与既有后缀式都得到同一个规范码；② **放松的强制腿反控**——`SZ2747`（丢前导零）、`002747`（无交易所）、`SZ002747.SZ`（重复交易所）、`BJ430047`/`HK00700`（他市场）、`SZ 002747`/`SZ-002747`（分隔符）、空值与裸 `SZ` 全部**仍然被拒**；③ `SZ300750`（创业板）改写后仍被**主板门**拒绝，即放松没有绕过 A-short 只做主板的设计约束；④ 整表用前缀式与用后缀式转换出的 account 与 lineage **逐字段相同**。

### 失效的旧结论

- 「券商导出必须先手工把 `ts_code` 改成后缀式」——已失效。
- 上一节里「待用户裁决 / 裁决前不要放宽该校验」——已裁决并实现。
