...
Verdict: the current official FMP docs identify stable endpoint candidates for profile, income statement, balance sheet statement, cash-flow statement, key metrics, and full historical EOD price / volume. This mapping is docs-only and not live-validated: no FMP stable endpoint retry was performed, no data fetch happened in this slice, limit / timeseries parameter parity remains unvalidated, and the artifact does not authorize provider selection, paid access, `yfinance`, full-market fetch, adapters, DataHub, production runner consumption, Phase 7c, production readiness, or ship-gate claims. A future retry must remain AAPL / MSFT, existing FMP key only, $0, raw samples under ignored `provider_samples/`, and tracked summary without secrets or raw rows.
...
Except for the reviewed 2026-06-02 AAPL / MSFT small-sample validation packet, this Phase 7b baseline does not fetch provider data, implement adapters, create DataHub tables, or modify runners.
