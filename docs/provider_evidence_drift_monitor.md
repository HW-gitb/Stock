...
Safe next slices are docs-only license / storage / retention review, or schema-first fallback / incident / stability playbook design. Any coverage-count, PIT-row, corporate-action, broader sample-validation, `yfinance`, new-token, trial, paid, full-market, provider-selection, adapter, DataHub, runner-consumption, or Phase 7c step still requires separate explicit approval and review.
...
Except for the reviewed 2026-06-02 AAPL / MSFT small-sample validation packet and the same-scope FMP stable retry, this Phase 7b baseline does not fetch provider data, implement adapters, create DataHub tables, or modify production runners. The remaining-blocker plan in §20 does not broaden that boundary.
