...
The current A-short steady alpha re-audit preregistration contract is `schemas/a_short_steady_alpha_reaudit_preregistration.schema.json`; the preregistered artifact is `research/preregistrations/a_short_steady_alpha_reaudit_20260603.json`, with planned-test ledger `research/ledgers/a_short_steady_alpha_reaudit_program_test_budget_ledger_20260603.json`. It freezes one same-anchor re-audit of the existing 5d CSI1000 clue plus required 20d / CSI300 diagnostics and multiple-testing / slice / factor / veto / PIT checks. It authorizes no outcome run by itself; the future run requires independent review and a later explicit user execute command.
...
The next A-share alpha-validation step is the A-short steady re-audit registered in `research/preregistrations/a_short_steady_alpha_reaudit_20260603.json` and `research/ledgers/a_short_steady_alpha_reaudit_program_test_budget_ledger_20260603.json`. It asks one plain question: does the old 5d CSI1000 clue survive same-anchor correction, or is A-short steady only a risk filter / falsified alpha clue? Do not run the outcome until this preregistration passes independent review and the user gives a later execute command.
...
Do not start new provider work, production provider data fetches, DataHub tables, runner changes, or Phase 7c implementation from this alpha re-audit. A positive backtest result would still be only a candidate clue; full-size use still requires the fixed 12-month forward-live ship gate.
