...
The current active alpha-search workstream is A-long only. Before any A-long signal search, the project must review and then execute one real data-integrity audit registered in `research/preregistrations/a_long_data_integrity_audit_20260603.json`. This audit freezes the monthly as-of schedule and requires planted-violation self-tests before the future runner can trust real output. It must produce hard pass / fail / blocked findings for look-ahead `ann_date`, restatement as-of logic, PIT universe / survivorship, terminal delisting returns, total-return and same-anchor benchmark measurement. Missing / invalid `ann_date` rows are excluded and reported; time-varying coverage declares the usable start year rather than globally failing the lane. If any hard check fails or blocks, repair the data route first; do not start signal backtests. If hard checks pass, the next step is a separate reviewed A-long signal-search preregistration limited to the declared usable window.
