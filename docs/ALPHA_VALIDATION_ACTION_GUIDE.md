...
The current A-long data route has passed only the fixed-panel data-integrity gate. `research/results/a_long_materialized_full_period_data_integrity_audit_20260604/audit_report.json` records `passed_fixed_panel_data_integrity_for_signal_preregistration`, 11/11 self-tests, hard checks pass, and usable start year 2018. `schemas/a_long_signal_search_preregistration.schema.json` and `research/preregistrations/a_long_signal_search_preregistration_20260604.json` freeze the first A-long quality / cashflow / balance-sheet / earnings-stability signal-search design, now tied to the approved 191-name no-industry boundary, with ledger `research/ledgers/a_long_signal_search_program_test_budget_ledger_20260604.json`. `schemas/a_long_full_main_board_signal_search_execution_packet.schema.json` / `docs/a_long_full_main_board_signal_search_execution_packet_20260605.json` passed review / commit, and `runners/a_long_full_main_board_materialization_packet.py` now implements only the first executable stop: full raw materialization. This is not an alpha result: the fixed 9-symbol panel is route proof only, and the new runner still needs independent review plus a later explicit user execute command before the raw pull.
...
The current active alpha-search workstream is A-long only, but A-long signal search is currently blocked until the full main-board raw materialization and the later full data-integrity audit pass. The reviewed local-cache-only data-integrity audit in `research/results/a_long_data_integrity_audit_20260603/audit_report.json` originally returned `blocked_missing_required_source`. Later Tushare route-validation, route-gap repair, thin-slice materialization / audit, daily-route diagnostic, and paced fixed-panel materialization produced a fixed-panel route proof, but the old `300750.SZ` (ChiNext) panel is historical-only because the user has A-share main-board access. The current main-board fixed panel passed audit and allowed the signal-search preregistration; the 191-name no-industry boundary and full execution packet are now accepted. Next gate: review `runners/a_long_full_main_board_materialization_packet.py` plus its summary schema/tests. Only after PASS, user approval / commit, and a later explicit execute command may the real full main-board raw pull run; signal search must wait for the full data-integrity audit to pass.
