...
The current active alpha-search workstream is A-long only, but A-long signal search is currently blocked until the new full main-board execution packet passes review. The reviewed local-cache-only data-integrity audit in `research/results/a_long_data_integrity_audit_20260603/audit_report.json` originally returned `blocked_missing_required_source`. Later Tushare route-validation, route-gap repair, thin-slice materialization / audit, daily-route diagnostic, and paced fixed-panel materialization produced a fixed-panel route proof, but the old `300750.SZ` (ChiNext) panel is historical-only because the user has A-share main-board access. The current main-board fixed panel passed audit and allowed the signal-search preregistration; the 191-name no-industry boundary is now accepted. Next gate: review `docs/a_long_full_main_board_signal_search_execution_packet_20260605.json` plus the synchronized preregistration boundary update. Only after PASS, user approval / commit, and a later explicit execute command may the real full main-board pull run; signal search must wait for the full data-integrity audit to pass.
