...
The current alpha-validation slice has established the A-share `minimal_data_burst` preregistration artifact in `research/preregistrations/a_share_minimal_data_burst_20260531.json` under `schemas/research_preregistration.schema.json`. The next alpha-validation action may run only that frozen research-only falsification test. US-long SEC observed-date / parser feasibility remains provider-evidence work; it proves data construction feasibility, not long-alpha existence. Provider sample / trial / paid access remains blocked until the user explicitly approves cost ceiling, access path, and license / storage boundaries, followed by a separate reviewed decision.
...
| Phase 7b-2 follow-up | run the preregistered A-share minimal-data burst frozen research-only falsification; do not route US-long SEC parser feasibility as alpha validation |
...
After the Phase 7b-2 P1 public-source, market-data-candidate, authorization / cost / stability, benchmark / GICS, fundamentals observed-date, coverage / fallback / incident snapshots, readiness review matrix, and access-decision / sample-validation plan, the A-share `minimal_data_burst` preregistration artifact has been created. The next alpha-validation `执行` should run only that frozen research-only falsification test and must not change production runners, feed production decisions directly, or claim ship-gate evidence.
