...
The current preregistration contract is `schemas/research_preregistration.schema.json`; the current artifact is `research/preregistrations/a_share_minimal_data_burst_20260531.json`. It is a single frozen test only: changing the A-share burst trigger thresholds, ranking cap, universe, benchmark, holding period, cost model, or pass/fail criteria before running requires a singleton program-level test-budget ledger first.
...
For the current A-share CSI1000 / CSI300 correction, benchmark open is available through Tushare `index_daily`; the existing materializer only requested / stored close, but that implementation gap does not make open unavailable. The required same-anchor path is to extend `runners/materialize_benchmark_monthly_returns_tushare.py` and any forward-daily benchmark fetch to request, persist, validate, and use benchmark T+1 open. Close-to-close is not an acceptable fallback for the current A-share / CSI1000 corrected revalidation.
...
| Phase 7b-2 | provider capability evidence population: P1 six snapshots, readiness matrix, and access-decision / sample-validation plan complete; provider access, sample collection, selection, and Phase 7c remain separate reviewed decisions |
| Phase 7b-2 follow-up | correct same-anchor benchmark excess and supersede the blocked A-share minimal-data burst preregistration before any research-only falsification; do not route US-long SEC parser feasibility as alpha validation |
...
After the Phase 7b-2 P1 public-source, market-data-candidate, authorization / cost / stability, benchmark / GICS, fundamentals observed-date, coverage / fallback / incident snapshots, readiness review matrix, and access-decision / sample-validation plan, the A-share `minimal_data_burst` preregistration artifact has been created but is now `BLOCKED_DO_NOT_RUN` by the measurement-basis lock. The next alpha-validation `执行` must correct / introduce same-anchor benchmark excess and create a corrected-basis supersession before any burst falsification run. It must not change production runners, feed production decisions directly, or claim ship-gate evidence.

Do not start Phase 7c, provider adapters, production provider data fetches, DataHub tables, or runner changes from the access plan. Phase 7c itself must start as a schema-first DataHub / report / reproducibility contract before any implementation slice.

Provider access remains a separate user decision. Any provider contact, token, trial, paid subscription, endpoint call, or sample-row collection must first consume `docs/provider_evidence_p1_us_access_decision_sample_validation_plan_20260531.json`, obtain explicit user-approved cost / access / license boundaries, and then pass a separate reviewed decision.
