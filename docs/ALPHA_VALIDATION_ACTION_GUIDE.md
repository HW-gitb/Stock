...
Provider access remains a separate user decision except for the 2026-06-02 approval artifact. The next allowed provider step may only consume `docs/provider_evidence_p1_us_sample_validation_access_approval_20260602.json` to implement a narrow AAPL / MSFT FMP existing-key + SEC public-API sample-validation packet with no secrets logged and raw rows only under gitignored `provider_samples/`. Any provider contact, new token, trial, paid subscription, `yfinance` check, full-market endpoint call, provider selection, adapter, DataHub table, runner use, or Phase 7c consumption must obtain separate explicit approval and pass a reviewed decision.
