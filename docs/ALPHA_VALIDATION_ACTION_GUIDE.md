...
The current preregistration contract is `schemas/research_preregistration.schema.json`; the superseded blocked artifact is `research/preregistrations/a_share_minimal_data_burst_20260531.json`, and the current corrected-basis superseding artifact is `research/preregistrations/a_share_minimal_data_burst_corrected_basis_20260531.json`. It is a single frozen test only: changing the A-share burst trigger thresholds, ranking cap, universe, benchmark, holding period, cost model, or pass/fail criteria before running requires a singleton program-level test-budget ledger first.
...
The corrected-basis supersession is `research/preregistrations/a_share_minimal_data_burst_corrected_basis_20260531.json`. Use it for the next research-only falsification; do not run the superseded blocked artifact.
...
| Phase 7b-2 follow-up | after same-anchor benchmark excess and corrected-basis supersession are reviewed and committed, run only the corrected A-share minimal-data burst research-only falsification; do not route US-long SEC parser feasibility as alpha validation |
...
After the Phase 7b-2 P1 public-source, market-data-candidate, authorization / cost / stability, benchmark / GICS, fundamentals observed-date, coverage / fallback / incident snapshots, readiness review matrix, and access-decision / sample-validation plan, the original A-share `minimal_data_burst` preregistration artifact remains `BLOCKED_DO_NOT_RUN` by the measurement-basis lock. The next alpha-validation `执行` after this same-anchor change set is reviewed and committed must run only `research/preregistrations/a_share_minimal_data_burst_corrected_basis_20260531.json` as a research-only falsification. It must not feed production decisions directly or claim ship-gate evidence.
