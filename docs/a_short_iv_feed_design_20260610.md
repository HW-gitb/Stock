...
- 缺失回退:feed 缺 → Slice B coverage 标 `iv_regime_status = observe_only_missing_feed`(父设计 §4),**不得假装执行了 IV 风控**。M0.5 schema 1.2.0 由本 feed 生成唯一觉醒/Rule3 状态；weekly/Phase5 只消费这一路径，不从 EGS 重新推导。
