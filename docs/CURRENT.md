# Stock 项目 - 当前状态快照
**最后更新**：2026-06-04（A-long broader materialization execution partial）
...
- US operating model is active-only + forward-live validation; historical US backtests stay idea-only. A-short steady is `risk_filter_only`. A-long broader materialization executed 71/71 planned Tushare calls and wrote `docs/a_long_tushare_broader_materialization_execution_summary_20260604.json`, but all 9 `daily` price calls returned 0 rows. Simple result: fixed-panel data is incomplete; A-long still cannot audit or search for alpha.
- A-long daily price route diagnostic packet is now recorded for review only: `docs/a_long_tushare_daily_price_route_diagnostic_packet_20260604.json`, `schemas/a_long_tushare_daily_price_route_diagnostic_packet.schema.json`, `runners/a_long_tushare_daily_price_route_diagnostic_packet.py`, and `schemas/a_long_tushare_daily_price_route_diagnostic_execution_summary.schema.json`. It authorizes no call in this slice. After Claude Pass and a separate user `执行`, it may run exactly two fixed `daily` probes for `000001.SZ`: `20180101..20251231` isolated retest plus `20220101..20221231` control, max 2 calls, retry 0.
---

## 1. 当前 Phase 与目标

- **当前 Phase**：A-share alpha validation is the active priority; A-long broader materialization has executed but failed the price-data leg, so signal search and full audit remain blocked. Phase 7c implementation remains not started.
- **当前 P0 / P1 目标**：Claude review the daily price diagnostic packet; only after Pass + user `执行`, run the fixed two-call `daily` diagnostic before any repair, full audit, or signal search。
...
- The current active alpha-search route is A-long, but signal search is still blocked. The broader materialization execution is incomplete because every fixed-panel `daily` price call returned 0 rows. The next step is Claude review of the new daily price route diagnostic packet; after Pass + user `执行`, run only the fixed two-call `000001.SZ` diagnostic: 2018-2025 isolated retest plus 2022 control. Do not run a broader pull, full audit, or signal search from this packet.
...
- Next high-value work is classifying the A-long `daily` failure with the two-call diagnostic, then repairing the price route only if the diagnostic supports it. Until fixed price data exists and a full reviewed audit passes, A-long cannot search for alpha.
