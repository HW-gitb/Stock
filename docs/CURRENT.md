...
**最后更新**：2026-06-01（SR-DATA-001 suspend daily completeness guard）
...
- `SR-DATA-001` is resolved in this reviewed slice: `get_suspend_info` now fails fast on non-empty `pro.daily` payloads below 95% in-universe coverage instead of treating partial daily rows as suspended stocks.
...
- **当前 P0 / P1 目标**：do not rerun or rescue the failed redesigned burst test；默认下一刀回到 risk register hot queue（`SR-EXEC-003/004/005/007` + `SR-CAP-001` + `SR-CONTRACT-002`）before execution-backtest evidence / manual sizing conclusions，除非用户先批准新的 research preregistration 或更窄 override。
...
- If no new research test or provider-access work is user-approved, the next default work is the risk-register hot queue execution-evidence group; do not cite execution-backtest evidence or manual sizing conclusions before those entries are fixed.
