...
**最后更新**：2026-06-01（SR-OPS-003 historical L3 engine guard）
...
- `SR-OPS-003` is resolved in this reviewed slice: direct historical `egs_main.py --as-of <non-current>` now rejects default `--l3-mode=today` unless `--allow-historical-live-l3` explicitly marks a non-evidence live-concept smoke run.
...
- **当前 P0 / P1 目标**：do not rerun or rescue the failed redesigned burst test；默认下一刀回到 risk register hot queue（`SR-DATA-001`）before weekly official capture / direct EGS regeneration，除非用户先批准新的 research preregistration。
...
- **Historical L3 interlocks**（2026-06-01）：weekly wrapper blocks historical `today` L3 official runs; direct `egs_main.py --as-of <non-current>` now also rejects default `--l3-mode=today` unless explicitly declared as non-evidence live-L3 smoke.
...
- If no new research test is user-approved, the next default work is the open hot-queue operational fix before new weekly official capture / direct EGS regeneration.
