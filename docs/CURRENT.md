...
**最后更新**：2026-06-03（FMP paid-tier / license public-docs review）
...
- **当前 P1 provider blocker**：FMP stable AAPL / MSFT response-shape retry and 5-symbol coverage smoke both succeeded; the reviewed validation packet found SIVB FMP endpoint errors, TWTR / SIVB missing SEC CIK follow-up, and continuing missing key-metrics direct fields. The SIVB-only re-probe has now executed within scope: 5/5 HTTP 402 with weak `historical_or_delisted_paid_tier` category signal, not paid-wall proof and not inactive / delisted coverage proof. The FMP paid-tier / license public-docs review only says public pages list paid tiers and endpoint templates; it does not prove paid-tier SIVB / delisted coverage and does not clear license / storage. Current terms / production storage rights / field-level PIT validation / actual price adjustment and corporate-action validation / SEC parser implementation and field mapping / fixture policy / incident-log writer / executed fallback / stability evidence / production readiness remain unresolved; anything beyond these plans still requires separate explicit approval + reviewed decision.
