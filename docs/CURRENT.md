...
- **当前 Phase**：A-share alpha validation is the active priority. A-long full main-board materialization shape has passed under the amended PIT `namechange` / H-code TR-close / statement-table `f_ann_date` / `fina_indicator` ann_date-only contract, but the data-integrity audit has not rerun yet.
- **当前 P0 / P1 目标**：review the new A-long materialization summary, then run the amended full data-integrity audit as a separate local raw gate. Each step remains a Claude-reviewed gate with a commit between; NO alpha until the materialization and audit gates pass and the signal result is reviewed.
...
- **执行锁**：A-long signal-search code exists, but no valid result exists. The invalid zero-cohort run was discarded and the ledger is unspent. The amended materialization count is now 23,718 and shape-passed, but the runner must still abort unless the amended full audit passes, the exclusion CSV is applied, and all close-to-close / terminal / drawdown / concentration / pipeline sanity guards pass. No alpha / production / ship-gate / full-size use is authorized now.
...
- The current active alpha-search route is A-long, but execution is still review-gated. Review target: the new materialization summary `docs/a_long_full_main_board_materialization_execution_summary_20260605.json`. After materialization-summary review, do not run alpha; the next separate `执行` may run only the amended full data-integrity audit.
...
- Next high-value work, after Claude review of the new materialization summary, is the amended full data-integrity audit. Signal search comes only after amended full audit PASS + review. Until then, do not run full alpha search.
