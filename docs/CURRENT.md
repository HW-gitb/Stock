...
## 0. Latest Delta
- 2026-06-07 after Claude PASS on the gate-reader fix and commit `498ab99`, Codex attempted Step 3 twice with the reviewed command. The first run timed out at 120s and the second at 900s, both before any output write. No `execution_summary.json`, `evidence_report.json`, pending summary, or ledger spend exists; `tests_spent_count` remains 0. Static diagnosis found a runtime/termination issue: `PayloadStore.records()` repeatedly re-read raw JSON payloads and `summarize_results()` repeatedly scanned all evaluated rows for each result cell. The working-tree repair now caches payloads/records and pre-groups rows by `(horizon, as_of)` without changing the frozen signal definitions or decision thresholds. Signal search remains locked until this performance repair gets independent review PASS + commit + separate user execute.
- 2026-06-07 Codex attempted Step 3 after commit `1623761` and the user's `execute`, but the run stopped before signal computation: the signal-search runner still read the old audit-report field `checks`, while the current full-audit report uses schema-locked `check_results`. No `execution_summary.json`, `evidence_report.json`, or pending summary was created, and the singleton signal-search ledger remains unspent (`tests_spent_count = 0`, `test_spend_log = []`). The working-tree repair now reads `check_results`, validates the current audit artifact, and rejects the legacy `checks` shape. Signal search remains locked until this repair gets independent review PASS + commit + separate user execute.
- 2026-06-06 latest A-long pre-run repair addresses the remaining signal-search design/code blockers without running signal search: `profitability_quality` now annualizes `fina_indicator.roe` by reported-period suffix before cross-sectional ranking, `cash_conversion` excludes near-zero net-income denominators below the registered 10,000,000 threshold, selection-time delisting-name veto catches `...退` suffix names, and non-terminal missing scheduled exits use the next available tradable close or become missing instead of backfilled to an earlier close. The preregistration/schema/tests lock these choices. Signal search remains locked until independent review PASS + commit + separate user execute.
- 2026-06-06 after commit `bf16a7d`, Codex ran the Step 2 amended A-long full main-board data-integrity audit only. Result: `research/results/a_long_full_main_board_data_integrity_audit_20260605/audit_report.json` records `passed_full_main_board_data_integrity_for_signal_search`, 12/12 self-tests passed, 6/6 check results pass/characterized, zero provider/network calls, and no raw records / endpoint results / secret / request URL in the tracked report. This is data-integrity PASS only; signal search still requires independent review plus a separate user `执行`.
- 2026-06-06 after commit `e6aa705`, Codex ran the Step 1 A-long full main-board materialization retry only. Result: `docs/a_long_full_main_board_materialization_execution_summary_20260605.json` now records `passed_full_main_board_materialization_shape`, 23,718 endpoint results, 3 new Tushare network calls, 23,715 reused raw payloads, all 14 table rollups passed, PIT `namechange_2018_2025` and H-code TR-close payloads included, statement-table `f_ann_date` plus `fina_indicator` ann_date-only contract satisfied. This is materialization shape only; no audit, signal search, alpha, production, ship-gate, or full-size use is authorized.
...
- 2026-06-06 A-long materialization `执行` initially stopped at the required preflight probe and did not clear stale checkpoints or run the 23,718-call materialization: project Tushare helper returned `namechange_2018_2025` 5,053 rows, `H00300.CSI` full-period close 1,940 rows, and `H00852.CSI` full-period close 1,940 rows, but `fina_indicator(ts_code=000001.SZ, fields=...f_ann_date...)` returned 55 rows without `f_ann_date`. This preflight result is now dispositioned by the table-specific PIT contract above; the later Step 1 materialization PASS shape is recorded above.
...
- **当前 Phase**：A-share alpha validation is the active priority. A-long full main-board materialization shape and amended full data-integrity audit have both passed, the pre-run repair was committed in `1623761`, and the gate-reader repair was committed in `498ab99`; the latest Step 3 execute attempt timed out before computation output because the reviewed full-run path was too slow. The current next gate is independent review of the runtime/termination repair before any signal-search rerun.
- **当前 P0 / P1 目标**：review the A-long Step-3 runtime/termination repair. Each step remains a Claude-reviewed gate with a commit between; NO alpha until this repair is reviewed/committed, the signal run executes successfully, and the signal result is reviewed.
  - **Step 0 (commit + packet review)**: done in `e6aa705`; Claude PASS for the `fina_indicator` ann_date-only contract is recorded at the top of `docs/SESSION_LOG.md`.
  - **Step 1 (reviewed materialization `执行`)**: done after `e6aa705`; `docs/a_long_full_main_board_materialization_execution_summary_20260605.json` passed shape with 23,718 endpoint results, 3 new calls, and 23,715 reused payloads. → Claude reviews the materialization summary before any audit.
  - **Step 2 (amended full data-integrity audit, local raw, no alpha)**: done after `bf16a7d`; `research/results/a_long_full_main_board_data_integrity_audit_20260605/audit_report.json` passed with 12/12 self-tests, 6 check results pass/characterized, and zero network calls. Claude review PASS was committed before `1623761`.
  - **Step 3 (first VALID frozen A-long signal search, `执行` double-confirm)**: attempted after commit `1623761`, but stopped before computation because the runner read stale audit field `checks` instead of current `check_results`; that was fixed and committed in `498ab99`. After Claude PASS on `498ab99`, the next attempt timed out twice before output because of full-run repeated raw reads / repeated cohort scans. This did not spend the singleton ledger. Next only after the runtime/termination repair review PASS + commit, and only if the singleton ledger is still unspent AND the 1,504-group `restatement_ambiguous_exclusions.csv` is applied AND both `--confirm-*` gates are set. (The prior zero-cohort run was an invalid bug run, discarded, ledger reverted — so the singleton is unspent; this is still the FIRST VALID use, not "the second run".) → Claude reviews the actual alpha result (candidate-clue vs no-alpha, CSI300+CSI1000 robustness, drawdown, concentration, size caveat). No alpha/production/ship-gate/full-size authorized before that review.
...
- **执行锁**：A-long signal-search code exists, but no valid result exists. The invalid zero-cohort run was discarded, the gate-reader attempt stopped before computation, and the latest post-`498ab99` attempts timed out before output, so the ledger is still unspent. Signal search is locked again until the current runtime/termination repair gets review PASS + commit + separate `执行`; the runner must still enforce the exclusion CSV plus close-to-close / terminal / drawdown / concentration / pipeline sanity guards. No alpha / production / ship-gate / full-size use is authorized now.
...
- The current active alpha-search route is A-long, but execution is still review-gated. Review target: the runtime/termination repair in `runners/a_long_full_main_board_data_integrity_audit.py`, `runners/a_long_full_main_board_signal_search.py`, `tests/test_a_long_full_main_board_data_integrity_audit.py`, and `tests/test_a_long_full_main_board_signal_search.py`. After repair review PASS + commit, the next separate `执行` may run only the frozen signal-search gate.
...
- Next high-value work, after Claude review of the runtime/termination repair, is the first valid frozen A-long signal search. Until review PASS + commit + separate `执行`, do not run full alpha search.
