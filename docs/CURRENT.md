# Stock 项目 - 当前状态快照
**最后更新**：2026-06-04（A-long broader materialization packet recorded）
...
- US operating model is active-only + forward-live validation; historical US backtests stay idea-only. A-short steady is `risk_filter_only`. A-long thin-slice audit passed; `docs/a_long_tushare_broader_materialization_packet_20260604.json` now records the next 2018-2025 fixed-panel packet but has not executed it. Simple result: the next larger data step is ready for review, but A-long still cannot search for alpha.
---

## 1. 当前 Phase 与目标

- **当前 Phase**：A-share alpha validation is the active priority; A-long broader materialization packet is recorded but still needs review + execution + full audit before signal search. Phase 7c implementation remains not started.
- **当前 P0 / P1 目标**：review the A-long broader materialization packet; after Pass + user `执行`, run only the fixed 2018-2025 panel, then run a full data-integrity audit。
...
- **A-long broader materialization packet**（2026-06-04）：after the thin-slice audit passed, `docs/a_long_tushare_broader_materialization_packet_20260604.json` / `runners/a_long_tushare_broader_materialization_packet.py` fixed a 2018-2025 nine-symbol panel packet; it has not executed and does not authorize alpha search.
---
...
- The current active alpha-search route is A-long, but signal search is still blocked. The broader materialization packet is recorded but not executed; next step is Claude review, then a separate post-review `执行` to run only the fixed 2018-2025 panel, then a full data-integrity audit. Do not start signal search from this packet.
...
- Next high-value work is review + execution of the fixed A-long broader materialization packet, followed by a full data-integrity audit. Until that full audit passes, A-long cannot search for alpha.
