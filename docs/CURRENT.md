# Stock 项目 - 当前状态快照
**最后更新**：2026-06-08（A-long 第5个大盘候选 `ep_value`（earnings-to-price 价值因子）设计切片已 Codex `审查` PASS 并入库；下一步是其 runner 切片；前 4 条线（全主板 / 纯质量 / cash_conversion / low_volatility）均已执行入库；实时状态见 §0 + `docs/SESSION_LOG.md` 顶部）
...
- **当前 Phase**：A-share alpha validation。四条已冻结路线均已执行并入库,终判见 §0 + `docs/SESSION_LOG.md` 顶部 + 各自 `execution_summary.json`:全主板 frozen signal search `no_alpha`;large-cap pure-quality composite `falsified`(`cfb4058`);diagnostic-derived 单因子 `cash_conversion`(OCF/|NI|、两层判决、504d/CSI300)`statistical_alpha_clue_research_only`(NOT tradeable,`527ac58`);externally-motivated 单因子 `low_volatility`(price-only trailing-252d realized vol、两层判决、504d/CSI300)`falsified`(`20bd22e`)。四条线均**不得 rerun / 换阈值 / 换基准 / 换 universe / 换 trailing-window / 诊断 rescue**,除非新 reviewed prereg+ledger 或 forward-live。
- **当前 P0 / P1 目标**：A-long 第 5 个大盘候选 `ep_value`（earnings-to-price 价值因子）设计切片已 Codex `审查` PASS 并入库（详 §0）;下一步是其 signal-search RUNNER 切片(另起 审查/提交/执行)。前 4 条线均已执行入库,`R-LOWVOL-ROUTE-CONTRACT-DRIFT-GUARD-GAP` 已由 alias-based route-doc guard 收口。进度见 §0 + `docs/SESSION_LOG.md`(单一实时来源)。四条已入库/已花的线**均不得 rerun / 换阈值 / 换基准 / 换 universe / 换 trailing-window / 诊断 rescue**,除非新 reviewed prereg+ledger 或 forward-live;后续新候选需新 prereg+ledger。
...
- **执行锁**：`low_volatility`、`cash_conversion`、large-cap pure-quality、全主板四条线的 singleton ledger 均已花;**不得 rerun / 换阈值 / 换基准 / 换 universe / 换 trailing-window / 诊断 rescue / production / ship-gate / full-size**,除非新 reviewed prereg+ledger 或 forward-live。各线终判与 ledger 状态见 §0 + `docs/SESSION_LOG.md` + 各自 `execution_summary.json`。
...
- The A-long `low_volatility` signal-search runner (`runners/a_long_large_cap_low_volatility_signal_search.py`, committed `20bd22e`) is EXECUTED → `falsified_large_cap_low_volatility_under_frozen_rules` (verdict + spent-ledger state in §0 + `docs/SESSION_LOG.md` top + `research/results/a_long_large_cap_low_volatility_20260608/execution_summary.json`); its result + spent ledger + tests are clean. The process issue `R-LOWVOL-ROUTE-CONTRACT-DRIFT-GUARD-GAP` is repaired by route-doc cleanup plus alias-based guard hardening. The `cash_conversion` (`527ac58`, statistical-alpha-clue / NOT tradeable), large-cap pure-quality, and full-main-board routes are all executed/closed; do not route work back to any of them. Any further A-long candidate needs a NEW reviewed prereg + ledger — `ep_value` (earnings-to-price) design slice is now review-passed and committed (see §0); its signal-search runner slice + review + commit + user `执行` come next.
