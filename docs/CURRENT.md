...
**最后更新**：2026-06-03（temporary US free-data working boundary）
...
- `schemas/datahub_local_resource_budget.schema.json` / `docs/datahub_local_resource_budget_contract_20260602.json` plus `schemas/datahub_job_spec.schema.json` / `schemas/examples/datahub_job_spec.example.json` define the Phase 7c precondition; `engine/datahub/job_spec_contract.py` now enforces that future local jobs declare budget profile, one market, one lane, bounded window, resource estimates, lazy / incremental / checkpoint / abort policy, and no all-system full refresh. This still does not fetch provider data or authorize provider selection / full-size US / production claims.
...
- `runners/us_egs_validation_packet.py` executed the reviewed five-symbol provider validation packet and wrote `docs/provider_evidence_p1_us_validation_execution_summary_20260603.json`: 37/41 calls used, 32 successes, 5 FMP SIVB endpoint errors, 6 skips, raw only under gitignored `provider_samples/us_egs_validation_packet_20260603/`, tracked summary no secret / request URL / raw rows. The inactive / delisted gap plan and FMP entitlement diagnostic routed SIVB 402; `runners/us_egs_sivb_reprobe_packet.py` then executed the fixed SIVB-only / 5-call / zero-retry packet and wrote `docs/provider_evidence_p1_us_sivb_reprobe_execution_summary_20260603.json`: 5/5 HTTP 402, non-JSON bodies captured only under gitignored `provider_samples/us_egs_sivb_reprobe_20260603/`, tracked summary no body / URL / raw rows / secret, weak category signal = `historical_or_delisted_paid_tier`, no paid-wall / coverage / provider-readiness proof. `docs/provider_evidence_p1_us_fmp_paid_tier_license_public_docs_review_20260603.json` then records only FMP public pricing / docs / Terms signals: Basic remains active-only safest path, paid FMP may help but is unproven for SIVB / delisted coverage, and license / storage is not cleared. Current working boundary is temporary and revisit-ready: without paid / specialized US delisted data, US work stays exploratory / forward-evidence / paper or minimal-size only; when a US idea shows forward promise, or the user decides, reassess paid / specialized data.
---

## 1. 当前 Phase 与目标

- **当前 Phase**：Phase 7c-a local resource budget code enforcement is now in progress / review scope; US EGS direction remains FMP primary candidate + SEC EDGAR fundamentals audit, but provider selection and US production use remain blocked by `SR-PROVIDER-001`.
- **当前 P0 / P1 目标**：do not rerun or rescue the failed redesigned burst test；keep the coverage-count smoke and missing-field plan scoped as evidence-only routing, not provider deployment。
- **当前 P1 provider blocker**：FMP stable AAPL / MSFT response-shape retry and 5-symbol coverage smoke both succeeded; the reviewed validation packet found SIVB FMP endpoint errors, TWTR / SIVB missing SEC CIK follow-up, and continuing missing key-metrics direct fields. The SIVB-only re-probe has now executed within scope: 5/5 HTTP 402 with weak `historical_or_delisted_paid_tier` category signal, not paid-wall proof and not inactive / delisted coverage proof. The FMP paid-tier / license public-docs review only says public pages list paid tiers and endpoint templates; it does not prove paid-tier SIVB / delisted coverage and does not clear license / storage. The current cost-saving boundary is temporary, not final: if the project does not buy or add a specialized US delisted source, US historical backtests are reference-only; they cannot prove alpha, unlock full-size, or authorize Phase 7c / production. Current terms / production storage rights / field-level PIT validation / actual price adjustment and corporate-action validation / SEC parser implementation and field mapping / fixture policy / incident-log writer / executed fallback / stability evidence / production readiness remain unresolved; anything beyond these plans still requires separate explicit approval + reviewed decision.
...
- **DataHub local resource budget + job spec enforcement**（2026-06-03）：`engine/datahub/job_spec_contract.py` enforces the resource-budget / job-spec contracts before future executable DataHub / runner jobs; `SR-RESOURCE-001` is resolved for current code-level enforcement, while future jobs must call the helper before execution.
...
- `docs/datahub_design.md` / `engine/datahub/job_spec_contract.py` / `schemas/datahub_local_resource_budget.schema.json` / `docs/datahub_local_resource_budget_contract_20260602.json` / `schemas/datahub_job_spec.schema.json` / `schemas/examples/datahub_job_spec.example.json` - DataHub / provider / factor-layer guardrails and Phase 7c local resource budget + job-spec enforcement。
...
- If no new research test or provider-access work is user-approved, the next default work is risk-register maintenance; reviewed forward-live evidence must match the aggregate `capital_context`, cover its claimed source window, and use real provenance / reconciliation, not the example artifact.

### P1 - US EGS sample-validation follow-up

- Treat the remaining-blocker plan, fallback playbook, incident-log contract, license-storage-retention review, SEC parser / field-family contracts, FMP PIT / price semantics contracts, coverage-count plan / summary, missing key-metrics resolution plan, 2026-06-03 validation authorization / execution packet / summary, inactive / delisted gap resolution plan, FMP entitlement / corporate-action diagnostic, SIVB-only re-probe packet / execution summary, FMP paid-tier / license public-docs review, and the temporary free-data working boundary as the active `SR-PROVIDER-001` router. Next provider work should not rerun SIVB silently; any broader coverage-count execution, current terms legal clearance, active symbol, SEC call, FMP split/dividend endpoint call, paid upgrade, account change, provider contact, fixture generation, derivation / field-mapping implementation, return calculation, corporate-action reconciliation, log-writer, fallback-execution, status polling, provider selection, DataHub, runner consumption, full-size US sizing, or scope beyond the fixed evidence needs separate approval.
...
- Future Phase 7c / runner implementation must call `engine.datahub.job_spec_contract.validate_datahub_job_spec_contract` / `validate_datahub_job_spec_file`; default all-system / all-market / all-lane / full-refresh runs remain disallowed, market/lane mismatch is invalid, and lane concurrency must stay within reviewed bounds unless explicit approval + reviewed job spec says otherwise.
