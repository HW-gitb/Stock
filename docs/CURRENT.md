# Stock 项目 - 当前状态快照
**最后更新**：2026-06-06（A-long schema-sync / jsonschema-hard repair）
...
- **当前 Phase**：A-share alpha validation is the active priority. A-long full main-board data route is not clean under the latest amended gates because PIT `namechange` and full-period H-code TR-close payloads are not materialized/audited yet.
- **当前 P0 / P1 目标**：Review target is the A-long R-VETO-PIT / audit-benchmark / f_ann_date / checkpoint plus schema-sync / jsonschema-hard repair. Do not run alpha until this repair passes review/commit, a separate reviewed 23,718-call materialization adds PIT `namechange_2018_2025` plus H-code TR-close rows, and the amended full audit passes.
...
- The current active alpha-search route is A-long, but execution is still review-gated. Review target: the latest A-long R-VETO-PIT / audit-benchmark / f_ann_date / checkpoint plus schema-sync / jsonschema-hard repair across `runners/a_long_full_main_board_signal_search.py`, `runners/a_long_full_main_board_materialization_packet.py`, `runners/a_long_full_main_board_data_integrity_audit.py`, their tests/schemas, the amended preregistration / execution packet, `docs/a_long_total_return_benchmark_access_probe_summary_20260606.json`, and `docs/SESSION_LOG.md` top entry. After PASS + commit, do not run alpha; first prepare a separate reviewed materialization/audit step for PIT `namechange_2018_2025` and H-code TR-close rows.
...
- Next high-value work, after Claude review / user approval / commit of this repair, is either reviewed local materialization / verification of the full-period total-return benchmark close rows if absent, or the user-gated frozen A-long signal-search execution if all close-to-close gates already pass. Until then, do not run full alpha search.
