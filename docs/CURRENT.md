...
**最后更新**：2026-06-01（SR-EXEC-007 concurrency evidence gate）
...
- `SR-EXEC-007` is resolved for evidence gating: aggregate execution reports remain diagnostic / `not_evaluable` for full-size permission until capacity / concurrency-adjusted returns exist.
...
- **当前 P0 / P1 目标**：do not rerun or rescue the failed redesigned burst test；默认下一刀回到 risk register hot queue（`SR-CONTRACT-002`）before first reviewed forward-live evidence artifact is produced or consumed，除非用户先批准新的 research preregistration 或更窄 override。
...
- **SR-EXEC-007 concurrency evidence gate**（2026-06-01）：`execution_aggregate_report` v1.1.2 keeps serialized execution returns diagnostic and blocks full-size permission until capacity / concurrency-adjusted returns exist.
...
- If no new research test or provider-access work is user-approved, the next default work is `SR-CONTRACT-002`; do not produce or consume a reviewed forward-live evidence artifact for ship-gate-like aggregation before that schema-first contract exists.
