...
- **A-long full main-board materialization / audit execution**（2026-06-05）：the reviewed runner completed 2018-2025 full main-board raw materialization: 23,717 endpoint results, 23,677 success, 40 empty, 0 error. The local-only full audit passed after route-A exclusion repair: 1,604 duplicate groups resolved by `f_ann_date` as-of, 1,504 ambiguous groups excluded under the preregistered 0.5% cap, 1,189 SW supplements verified from repair raw, 14 post-panel 2026 listings excluded from checks, and 3 extended no-trade terminal cases verified. It is not alpha evidence.
- **A-long signal-search runner package**（2026-06-06）：`runners/a_long_full_main_board_signal_search.py` and `schemas/a_long_signal_search_execution_summary.schema.json` are ready for review. This package adds code/tests only; no true signal result exists yet.
---
...
- The current active alpha-search route is A-long. Review target: `runners/a_long_full_main_board_signal_search.py`, `schemas/a_long_signal_search_execution_summary.schema.json`, `tests/test_a_long_full_main_board_signal_search.py`, and `tests/schema/test_a_long_signal_search_execution_summary_schema.py`. The package has not executed true signal search.
...
- Next high-value work, after Claude review / user approval / commit of the signal-search runner package, is the separate frozen A-long signal-search execution. Until then, do not run full alpha search.
