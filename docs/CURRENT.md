...
- US operating model is fixed as active-only + forward-live validation; historical US backtests stay idea-only, not alpha / ship-gate / full-size evidence. The repaired A-short steady re-audit is `risk_filter_only` (`5d CSI1000 mean = 0.6158673222` pp, monthly t `1.7623850474`) and must not be rescued. The current active alpha-search route is A-long data-integrity first: the preregistration now freezes schedule + planted-violation self-tests, and coverage declares usable window rather than globally failing.
---

## 1. 当前 Phase 与目标

- **当前 Phase**：A-share alpha validation is the active priority; A-long data-integrity audit preregistration is the current review target before any signal search. Phase 7c implementation remains not started; do not add more DataHub contracts unless a concrete implementation need appears.
- **当前 P0 / P1 目标**：review the A-long data-integrity audit preregistration；do not run the audit until Claude review + later user `执行`；do not rerun or rescue A-short steady / failed burst artifacts；keep US work active-only + forward-live。
...
- **执行锁**：A-long signal search is blocked until the registered data-integrity audit hard-checks pass and a usable start year is declared; this preregistration itself does not run the audit. A-short steady has spent its one planned test and is `risk_filter_only`; no rerun / rescue / threshold change is authorized without a new reviewed preregistration + user approval。原 burst prereg 仍为 `BLOCKED_DO_NOT_RUN`；corrected-basis prereg 不得运行 outcome / excess；redesigned burst outcome 已失败。任何 material audit finding 必须修复或进入 risk register。
...
- `research/README.md` / `schemas/research_preregistration.schema.json` / `schemas/a_short_steady_alpha_reaudit_preregistration.schema.json` / `schemas/a_long_data_integrity_audit_preregistration.schema.json` / `schemas/research_preflight_result.schema.json` / `schemas/program_test_budget_ledger.schema.json` / `schemas/evidence_report.schema.json` - research-only contract owners。
- `research/preregistrations/a_long_data_integrity_audit_20260603.json` / `research/ledgers/a_long_data_integrity_audit_program_test_budget_ledger_20260603.json` - A-long data-integrity gate owner files; `research/preregistrations/a_short_steady_alpha_reaudit_20260603.json` / `research/results/a_short_steady_alpha_reaudit_20260603/evidence_report.json` / `research/results/a_short_steady_alpha_reaudit_20260603/diagnostics.json` / `research/ledgers/a_short_steady_alpha_reaudit_program_test_budget_ledger_20260603.json` / `runners/a_short_steady_alpha_reaudit.py` - A-short steady re-audit owner files and output.
...
- The current active alpha-search route is A-long data-integrity first. Review `research/preregistrations/a_long_data_integrity_audit_20260603.json` and its ledger; do not run the audit until Claude review passes and the user gives a later `执行`. A-short steady is spent / `risk_filter_only`; do not rerun or rescue it without a new reviewed preregistration and user approval.
...
- Next high-value work is Claude review of the repaired A-long data-integrity audit preregistration. If review passes and the user later says `执行`, run only that frozen data audit; hard failures require repair, while coverage shortfall limits the usable signal-search window.
