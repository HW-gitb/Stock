# Stock 项目 - 当前状态快照
**最后更新**：2026-06-03（A-long Tushare data route repair plan）
...
- US operating model is fixed as active-only + forward-live validation; historical US backtests stay idea-only, not alpha / ship-gate / full-size evidence. The repaired A-short steady re-audit is `risk_filter_only` and must not be rescued. The A-long data-integrity audit is blocked, and `docs/a_long_tushare_data_route_repair_plan_20260603.json` now defines the next repair attempt: existing Tushare raw PIT route validation first; no data fetch, audit rerun, signal search, production, ship-gate, or full-size claim is authorized by that plan.
---

## 1. 当前 Phase 与目标

- **当前 Phase**：A-share alpha validation is the active priority; A-long data-integrity is blocked until the Tushare data-route validation/materialization path is reviewed and then re-audited. Phase 7c implementation remains not started.
- **当前 P0 / P1 目标**：review the A-long Tushare data-route repair plan, then if approved create a small route-validation execution packet；do not fetch data, rerun the spent audit, or start signal search without separate reviewed authorization。
...
- The current active alpha-search route is A-long, but signal search is blocked by the executed data-integrity audit. The next step is Claude review of the Tushare data-route repair plan; if it passes, create a separate small route-validation execution packet. Do not fetch data, rerun the audit, or start signal search from this plan.
...
- Next high-value work is to review and, only after approval, execute a small A-long Tushare route-validation packet for raw PIT fundamentals, PIT universe, total-return/dividend, and terminal delisting lineage. Until a new audit passes, A-long cannot search for alpha.
