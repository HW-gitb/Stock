...
**最后更新**：2026-06-03（US active-only + forward-validation model）
...
- US operating model is now fixed as active-only + forward-live validation: US historical backtests are exploration / idea-only forever and cannot prove alpha, unlock full-size, authorize DataHub / Phase 7c / production, or count as ship-gate evidence. Inactive / delisted historical coverage is user-accepted as scoped out, not solved; forward universes must be frozen point-in-time at forward start and must capture real delisting / halt / merger / no-trade outcomes during the forward window. No delisted / specialized US data purchase for now; reassess only when a US idea shows forward-live promise or the user asks. `SR-PROVIDER-001` stays open for license / storage, active-PIT if fundamentals are used, active price / corporate-action semantics if used, SEC parser / mapping if used, fallback / stability, provider selection, DataHub / runner consumption, and production readiness.
---

## 1. 当前 Phase 与目标

- **当前 Phase**：Phase 7c schema-first contracts cover local job enforcement, shared layers, report boundaries, reproducibility manifests, data-quality monitor boundaries, and a minimal A-share local-cache read-path plan; implementation remains not started. Default priority shifts away from more contracts toward A-share alpha validation and, later, minimal read-path implementation.
- **当前 P0 / P1 目标**：do not rerun or rescue the failed redesigned burst test；keep US work active-only + forward-live；treat US historical backtests as idea-only, not evidence。
...
- Next high-value work should be real A-share alpha validation on existing data: multiple-testing, regime, forward evidence, and decision-effect checks. Do not add more Phase 7c contracts unless a concrete implementation need appears.
