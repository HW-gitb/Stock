# Stock 项目 - 当前状态快照
**最后更新**：2026-06-08（A-long large-cap cash_conversion design committed `63f5048`; protocol: dropped 批准修改 + added Claude implementer judge-before-execute standard; next = signal-search runner slice）
...
## 0. Latest Delta
- 2026-06-08 the A-long `cash_conversion` design slice was reviewed by Codex (PASS) and committed (`63f5048` design + `f35f786` protocol): prereg flipped to `passed_independent_review_ready_for_freeze`, new singleton ledger unspent, 15/15 schema tests. Protocol committed: `批准修改` is no longer required between a Codex `审查` and a Claude `修复` (the user's `修复` authorizes directly), and `AGENTS.md` now carries the Claude implementer judge-before-execute standard. Next gate is the cash_conversion **signal-search runner slice** (Claude builds → Codex `审查` → Claude `提交` → user `执行`), NOT another design review/submit cycle.
- 2026-06-07 ROLE SWAP in effect (Claude implements, Codex reviews). First Claude-implemented slice: the diagnostic-derived A-long large-cap `cash_conversion` design four-piece — `research/preregistrations/a_long_large_cap_cash_conversion_20260607.json`, `research/ledgers/a_long_large_cap_cash_conversion_program_test_budget_ledger_20260607.json` (new singleton, unspent), `schemas/a_long_large_cap_cash_conversion_preregistration.schema.json`, `tests/schema/test_a_long_large_cap_cash_conversion_preregistration_schema.py`. cash_conversion was the strongest single-factor diagnostic in the falsified large-cap pure-quality run (504d t=2.3542 vs CSI300, 4.5165 vs CSI1000); this is a diagnostic-derived NEW hypothesis, not a rescue. Design: single-factor cash_conversion (OCF/|net income|, unchanged definition), reuse the reviewed top-500 circ_mv universe + 000043 exclusion + full-main-board fundamentals (NO new fetch), marginal 0.5 industry + 0.5 size neutral, 504d/CSI300 primary, two-tier verdict (statistical alpha clue by HAC t≥2 + cohorts≥48 + both sub-period halves mean>0 + concentration; tradeable candidate additionally by rolling relative-NAV-drawdown vs CSI300 ≥ −15%). Adopts Codex's review (conservative single-sample vs-CSI300 framing, effective-cohort median sub-period split, frozen real-NAV drawdown gate) plus Claude sharpenings (risk gate gates only the tradeable label not the alpha verdict; rolling-NAV construction frozen precisely; per-half HAC-t reported). 15/15 schema tests pass in jsonschema env; design-only, NO runner, NO run, ledger unspent. (Superseded by the 2026-06-08 top entry: this design slice was later reviewed PASS and committed `63f5048` / `f35f786`; the live next gate is the signal-search runner slice, not another design review.)
...
- **当前 Phase**：A-share alpha validation。两条已冻结路线均已证伪并入库(全主板 frozen signal search `no_alpha`;large-cap pure-quality composite `falsified`, `cfb4058`)。NEW diagnostic-derived 单因子 `cash_conversion` **设计已审过并入库**(`63f5048`;prereg = review-passed,singleton ledger 未花)。当前活动线是它的**信号搜索 runner slice**(尚未构建)。
- **当前 P0 / P1 目标**：构建 `cash_conversion` **信号搜索 runner slice**——复用 vetted 测量链(PIT / 年化 ROE / 同锚总收益 / HAC)+ 实现冻结的两层判决与滚动相对 NAV 回撤门(Option A 平行同档 CSI300 tranche,只决定 tradeable 标签)+ 子区间中位切。流程:Claude `修复`/起草 runner → Codex `审查` → Claude `提交` → 用户单独 `执行`(花 singleton 一次,出 cash_conversion 最终判决)。两条已证伪路线不得 rerun / 换阈值 / 换基准 / 换 universe / 诊断 rescue。
...
- **执行锁**：`cash_conversion` 设计已审过入库,但 singleton ledger 未花、runner 尚未构建;无新数据抓取、无信号搜索,直到 runner 建好审过 + 用户单独 `执行`。已证伪的 large-cap pure-quality / 全主板路线的 ledger 均已花,不得 rerun 或 rescue。
...
- The A-long `cash_conversion` design slice is reviewed and committed (`63f5048`; prereg review-passed, singleton ledger unspent). The current active route is its signal-search runner slice: Claude builds the runner (reuse the vetted measurement chain + the frozen two-tier verdict and rolling relative-NAV Option-A gate) → Codex `审查` → Claude `提交` → separate user `执行` (spends the singleton once). The old full-main-board and large-cap pure-quality routes are both falsified, committed, and closed; do not route work back to them.
