...
- **当前 Phase**：A-share alpha validation is the active priority. A-long full main-board materialization shape and amended full data-integrity audit have both passed, and the pre-run repair was committed in `1623761`; the first Step 3 execute attempt stopped before computation on a runner/audit-report field mismatch. The current next gate is independent review of the small gate-reader repair before any signal search rerun.
- **当前 P0 / P1 目标**：review the A-long Step-3 gate-reader repair. Each step remains a Claude-reviewed gate with a commit between; NO alpha until this repair is reviewed/committed, the signal run executes successfully, and the signal result is reviewed.
...
- **执行锁**：A-long signal-search code exists, but no valid result exists. The invalid zero-cohort run was discarded and the 2026-06-07 Step 3 attempt stopped before computation, so the ledger is still unspent. The amended materialization, full audit, and pre-run repair have passed review/commit through `1623761`, but signal search is locked again until the current gate-reader repair gets review PASS + commit + separate `执行`; the runner must still enforce the exclusion CSV plus close-to-close / terminal / drawdown / concentration / pipeline sanity guards. No alpha / production / ship-gate / full-size use is authorized now.
...
- Next high-value work, after Claude review of the gate-reader repair, is the first valid frozen A-long signal search. Until review PASS + commit + separate `执行`, do not run full alpha search.
