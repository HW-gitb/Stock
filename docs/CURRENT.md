...
**最后更新**：2026-06-01（SR-OPS-004 weekly xlsx overwrite guard）
...
- `SR-OPS-004` is resolved: weekly historical overwrite guard now checks the actual default `A-EGS\egs_tier1_<AsOf>.xlsx` path in addition to the existing official outputs.
...
- **当前 P0 / P1 目标**：do not rerun or rescue the failed redesigned burst test；默认下一刀回到 risk register hot queue maintenance 组（`SR-DATA-002` + `SR-DATA-004` + `SR-OPS-005` + `SR-RANK-001` + `SR-OPS-006`），除非用户先批准新的 research preregistration、provider-access work 或更窄 override。
