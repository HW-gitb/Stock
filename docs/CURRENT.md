...
- **当前目标**：下一条最小流程任务是起步 fill simulation。
...
1. **Fill simulation 起步** — Tushare provider materializer 已提交为 `cfa1c57`；下一条 scope 单独实现 entry/exit + 涨停不可买 + 止损 + 时间止损 + 组合约束，不再混入 provider materializer 变更。
