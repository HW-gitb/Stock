...
- **当前 P0 / P1 目标**：review the broader materialization execution summary, then repair the `daily` price route / parameter issue before any full data-integrity audit。
...
- Next high-value work is repairing the A-long `daily` price route after this execution summary is reviewed. Until fixed price data exists and a full reviewed audit passes, A-long cannot search for alpha.
