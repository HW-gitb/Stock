# Stock 项目 - 当前状态快照
**最后更新**：2026-06-04（A-long Tushare route validation partial）
...
- US operating model is fixed as active-only + forward-live validation; historical US backtests stay idea-only. The repaired A-short steady re-audit is `risk_filter_only`. A-long route validation has now touched existing Tushare data: `docs/a_long_tushare_route_validation_execution_summary_20260604.json` is `partial_or_failed_field_presence`. Calendar, PIT universe, raw fundamentals, restatement fields, price / adj_factor / dividend, and benchmark fields passed the small field-presence check; SW industry membership lacks required `index_code` / `con_code` fields, and the selected recent delisted sample has empty terminal daily price rows. A-long still cannot search for alpha.
---

## 1. 当前 Phase 与目标

- **当前 Phase**：A-share alpha validation is the active priority; A-long data-integrity is blocked until the Tushare route gaps are repaired, materialized under review, and then re-audited. Phase 7c implementation remains not started.
- **当前 P0 / P1 目标**：review the A-long Tushare route-validation result, then decide the smallest repair for SW industry membership fields and terminal delisting return coverage；do not materialize full history, rerun the spent audit, or start signal search without separate reviewed authorization。
...
- **执行锁**：A-long signal search is blocked because the data audit is `blocked_missing_required_source` and the route-validation summary is only partial. A-short steady is `risk_filter_only` and spent；burst paths remain blocked/failed. Material audit findings must be fixed or entered in the risk register.
...
- The current active alpha-search route is A-long, but signal search is blocked by the executed data-integrity audit and the partial Tushare route validation. The next step is Claude review of the route-validation result and then a narrow repair plan for SW industry membership fields plus terminal delisting return coverage. Do not full-materialize, rerun the audit, or start signal search from this partial result.
...
- Next high-value work is to repair the two failed A-long route pieces shown by the Tushare validation summary. Until a new audit passes, A-long cannot search for alpha.
