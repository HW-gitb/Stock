...
## 0. Latest Delta
- 2026-06-08 the next A-long candidate — externally-literature-motivated single-factor `low_volatility` (negative trailing-252d realized volatility) — was DRAFTED, Codex-`审查` PASSED, and committed as a design slice (`research/preregistrations/a_long_large_cap_low_volatility_20260608.json` review-passed + bespoke schema + UNSPENT singleton ledger + 15/15 schema tests), mirroring the cash_conversion design-slice pattern and held to the SAME frozen -15% relative-NAV drawdown gate. Reuses the reviewed top-500 universe + full-main-board daily close series (no new fetch). Committed with a residual post-exec stale-test fix for the cash_conversion + pure_quality prereg-schema tests (full `tests/schema` 578/578). Singleton ledger still UNSPENT; next is the low-vol RUNNER slice, then review/commit, then a separate user `执行`. No `执行` yet.
