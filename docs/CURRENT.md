...
**最后更新**：2026-06-01（SR-DATA-002 daily stats insufficiency guard）
...
- `SR-DATA-002` is resolved: severe daily-stat insufficiency now aborts before neutral liquidity / crash-veto defaults can pass through L0 as normal screening evidence.
...
- **当前 P0 / P1 目标**：do not rerun or rescue the failed redesigned burst test；默认下一刀回到 risk register hot queue maintenance 组（`SR-DATA-004` + `SR-OPS-005` + `SR-RANK-001` + `SR-OPS-006`），除非用户先批准新的 research preregistration、provider-access work 或更窄 override。
