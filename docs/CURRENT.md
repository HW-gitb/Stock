# Stock 项目 - 当前状态快照
**最后更新**：2026-06-04（A-long thin-slice materialization packet recorded）
...
- US operating model is fixed as active-only + forward-live validation; historical US backtests stay idea-only. The repaired A-short steady re-audit is `risk_filter_only`. A-long route validation and route-gap repair passed small-sample route checks, and `docs/a_long_tushare_incremental_materialization_packet_20260604.json` now fixes a 2022-2023 three-symbol thin-slice materialization packet. It has not run yet; A-long still cannot search for alpha until the thin slice runs under review and a new data-integrity audit passes.
---
...
- **当前 P0 / P1 目标**：review the A-long thin-slice materialization packet, then run only that packet after Claude Pass + user `执行`；do not full-materialize, rerun the spent audit, or start signal search without separate reviewed authorization。
...
- **A-long thin-slice materialization packet**（2026-06-04）：`docs/a_long_tushare_incremental_materialization_packet_20260604.json` / `runners/a_long_tushare_incremental_materialization_packet.py` fix a reviewed 2022-2023 three-symbol packet with checkpoint resume and 29 planned calls / max 32. It has not executed; no data is ready for alpha.
...
- The current active alpha-search route is A-long, but signal search is still blocked by the executed data-integrity audit. The route gaps have passed small-sample repair validation, and the next packet is recorded but unexecuted. The next step is Claude review of the thin-slice materialization packet, then a separate user `执行` to run only that packet. Do not full-materialize, rerun the audit, or start signal search from this packet.
...
- Next high-value work is to review and then execute the bounded A-long Tushare thin-slice materialization packet. Until a new audit passes after that materialization, A-long cannot search for alpha.
