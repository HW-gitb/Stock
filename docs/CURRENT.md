# Stock 项目 - 当前状态快照
**最后更新**：2026-06-03（A-long data-integrity audit blocked）
...
- US operating model is fixed as active-only + forward-live validation; historical US backtests stay idea-only, not alpha / ship-gate / full-size evidence. The repaired A-short steady re-audit is `risk_filter_only` (`5d CSI1000 mean = 0.6158673222` pp, monthly t `1.7623850474`) and must not be rescued. The A-long data-integrity audit ran local-cache-only and is `blocked_missing_required_source`: self-tests 6/6 passed, but raw PIT fundamentals, complete PIT universe, dividend/total-return, and terminal delisting return lineage are missing, so no A-long signal search is allowed.
---

## 1. 当前 Phase 与目标

- **当前 Phase**：A-share alpha validation is the active priority; A-long data-integrity is blocked on data-route repair before any signal search. Phase 7c implementation remains not started; do not add more DataHub contracts unless a concrete implementation need appears.
- **当前 P0 / P1 目标**：repair or replace the A-long data route for raw PIT fundamentals, PIT universe, total-return/dividend, and terminal delisting return lineage；do not rerun the spent audit without a new reviewed authorization；do not rerun or rescue A-short steady / failed burst artifacts；keep US work active-only + forward-live。
...
- **执行锁**：A-long signal search is blocked because `research/results/a_long_data_integrity_audit_20260603/audit_report.json` is `blocked_missing_required_source`; no signal search until a repaired data route passes a new reviewed audit and declares a usable start year. A-short steady has spent its one planned test and is `risk_filter_only`; no rerun / rescue / threshold change is authorized without a new reviewed preregistration + user approval。原 burst prereg 仍为 `BLOCKED_DO_NOT_RUN`；corrected-basis prereg 不得运行 outcome / excess；redesigned burst outcome 已失败。任何 material audit finding 必须修复或进入 risk register。
...
- `runners/a_long_data_integrity_audit.py` / `research/preregistrations/a_long_data_integrity_audit_20260603.json` / `research/results/a_long_data_integrity_audit_20260603/audit_report.json` / `research/ledgers/a_long_data_integrity_audit_program_test_budget_ledger_20260603.json` - A-long data-integrity gate owner files and blocked execution result; `research/preregistrations/a_short_steady_alpha_reaudit_20260603.json` / `research/results/a_short_steady_alpha_reaudit_20260603/evidence_report.json` / `research/results/a_short_steady_alpha_reaudit_20260603/diagnostics.json` / `research/ledgers/a_short_steady_alpha_reaudit_program_test_budget_ledger_20260603.json` / `runners/a_short_steady_alpha_reaudit.py` - A-short steady re-audit owner files and output.
...
- The current active alpha-search route is A-long, but signal search is blocked by the executed data-integrity audit. Repair the data route first; do not rerun the spent audit or start signal search without a new reviewed authorization. A-short steady is spent / `risk_filter_only`; do not rerun or rescue it without a new reviewed preregistration and user approval.
...
- Next high-value work is choosing the A-long data-route repair: raw PIT fundamentals with `ann_date` / `end_date` lineage, full PIT universe, dividend / total-return handling, and terminal delisting return lineage. Until that is repaired and re-audited, A-long cannot search for alpha.
