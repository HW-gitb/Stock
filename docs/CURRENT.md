# Stock 项目 - 当前状态快照
**最后更新**：2026-06-08（A-long large-cap low_volatility signal-search EXECUTED → `falsified_large_cap_low_volatility_under_frozen_rules`，NOT a clue；user-authorized Codex process/docs guard repair for `R-LOWVOL-ROUTE-CONTRACT-DRIFT-GUARD-GAP` completed in this closeout；实时状态见 §0 + `docs/SESSION_LOG.md` 顶部）
...
- **当前 P0 / P1 目标**：`R-LOWVOL-ROUTE-CONTRACT-DRIFT-GUARD-GAP` 已由 user-authorized Codex process/docs guard repair 收口（`research/README.md` stale contract/status rows + alias-based `tests/test_route_doc_ledger_status_consistency.py` guard）。进度见 §0 + `docs/SESSION_LOG.md`(单一实时来源)。四条已入库/已花的线**均不得 rerun / 换阈值 / 换基准 / 换 universe / 换 trailing-window / 诊断 rescue**,除非新 reviewed prereg+ledger 或 forward-live;后续新候选需新 prereg+ledger。
