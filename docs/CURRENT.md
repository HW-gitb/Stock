...
**最后更新**：2026-06-01（SR-CAP-001 capital ceiling guard）
...
- `SR-CAP-001` is resolved in this execution slice: execution backtest now rejects a loaded bucket capital above `market_capital * bucket_ceiling_pct` before sizing can use it.
...
- **当前 P0 / P1 目标**：do not rerun or rescue the failed redesigned burst test；默认下一刀回到 risk register hot queue（`SR-EXEC-007` + `SR-CONTRACT-002`）before execution-backtest evidence / manual sizing conclusions，除非用户先批准新的 research preregistration 或更窄 override。
...
- **SR-CAP-001 capital ceiling guard**（2026-06-01）：`runners/backtest_execution.py` rejects cash-state bucket capital above policy ceiling before report generation / sizing.
...
- If no new research test or provider-access work is user-approved, the next default work is the remaining risk-register execution-evidence group (`SR-EXEC-007` + `SR-CONTRACT-002`); do not cite execution-backtest evidence or manual sizing conclusions before those entries are fixed.
