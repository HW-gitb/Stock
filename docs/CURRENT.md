...
- 2026-06-05 A-long full main-board local data-integrity audit R1 repair is updated after Claude FAIL. The 0.5% restatement exclusion cap now lives in `research/preregistrations/a_long_data_integrity_audit_20260603.json`, future signal search must consume `restatement_ambiguous_exclusions.csv`, and the runner reads / cross-checks that preregistered policy. The rerun report still records `passed_full_main_board_data_integrity_for_signal_search`, but only with mandatory exclusion of 1,504 ambiguous same-ann-date groups; this needs Claude review before commit and is not alpha.
...
- **当前 P0 / P1 目标**：Claude review the R1 route-A repair: preregistered 0.5% restatement exclusion cap, mandatory CSV exclusion-list consumption by future signal search, lean tracked JSON, and extended no-trade terminal policy. If Claude passes and the user commits, the next separate executable gate may run the frozen A-long signal search. Do not run signal search before review / commit.
...
- **执行锁**：A-long full alpha search is not yet authorized by this report. The repaired full audit says the next separate reviewed gate may run frozen signal search, but no signal / alpha / production / ship-gate / full-size use is authorized now. The fixed 9-symbol panel is route proof only, not full-universe alpha evidence.
...
- **A-long full main-board materialization / audit execution**（2026-06-05）：the reviewed runner completed 2018-2025 full main-board raw materialization: 23,717 endpoint results, 23,677 success, 40 empty, 0 error. The local-only full audit now uses a preregistered route-A exclusion policy: 1,604 duplicate groups resolved by `f_ann_date` as-of, 1,504 ambiguous groups excluded under a preregistered 0.5% cap, 1,189 SW supplements verified from repair raw, 14 post-panel 2026 listings excluded from 2018-2025 return checks, and 3 extended no-trade terminal cases verified. It still needs Claude review and does not authorize signal search by itself, alpha, production, ship-gate, or full-size use.
---
...
- `research/README.md` / `schemas/research_preregistration.schema.json` / `schemas/a_short_steady_alpha_reaudit_preregistration.schema.json` / `schemas/a_long_data_integrity_audit_preregistration.schema.json` / `schemas/a_long_data_integrity_audit_report.schema.json` / `schemas/a_long_materialized_thin_slice_data_integrity_audit_report.schema.json` / `schemas/a_long_materialized_full_period_data_integrity_audit_report.schema.json` / `schemas/a_long_signal_search_preregistration.schema.json` / `research/preregistrations/a_long_signal_search_preregistration_20260604.json` / `research/ledgers/a_long_signal_search_program_test_budget_ledger_20260604.json` / `schemas/a_long_main_board_candidate_universe_preflight_execution_summary.schema.json` / `docs/a_long_main_board_candidate_universe_preflight_execution_summary_20260604.json` / `runners/a_long_main_board_candidate_universe_preflight.py` / `schemas/research_preflight_result.schema.json` / `schemas/program_test_budget_ledger.schema.json` / `schemas/evidence_report.schema.json` - research-only contract owners。
...
- The current active alpha-search route is A-long. Review target: `runners/a_long_full_main_board_data_integrity_audit.py`, `schemas/a_long_full_main_board_data_integrity_audit_report.schema.json`, `research/results/a_long_full_main_board_data_integrity_audit_20260605/audit_report.json`, `research/results/a_long_full_main_board_data_integrity_audit_20260605/restatement_ambiguous_exclusions.csv`, and their tests. The fixed 9-symbol panel must not be used as alpha proof.
...
- Next high-value work, after Claude review / user approval / commit of the repaired audit slice, is the separate frozen A-long signal-search execution gate. Until then, do not run full alpha search.
