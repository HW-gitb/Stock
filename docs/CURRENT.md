...
- **当前 Phase**：Phase 7c schema-first contracts now cover local job enforcement, shared layers, report boundaries, reproducibility manifests, data-quality monitor boundaries, and a minimal A-share local-cache read-path plan; implementation remains not started. US EGS direction remains FMP primary candidate + SEC EDGAR fundamentals audit, but provider selection and US production use remain blocked by `SR-PROVIDER-001`.
