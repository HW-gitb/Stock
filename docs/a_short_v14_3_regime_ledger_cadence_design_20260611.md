...
- **切片2b-impl,拆两步降风险**:
  ...
  - **②a(已起草 2026-06-11)纯对比逻辑**:`engine/a_short_regime_comparison.py` —— `backfill_forward_returns`(给对比记录回填到期的 h1/3/5/10,CSI1000 raw close-to-close、percent,匹配 const basis;无 look-ahead;**已有非空 horizon 不盲信**——对其重算确定性 CSI1000 收益,不符或目标在 as_of_now 下未到期则 **raise**(仅保留经审计匹配的值);索引日期规范+唯一、收盘须 **isfinite 且 >0**(0/负/NaN/Inf→unavailable→该 horizon 留 null,不填)、回填后过 `validate_comparison_record`)+ **审计证据路径** `summarize_comparison_records(records, csi1000, as_of_now)` 与 `render_regime_comparison_block(record, csi1000, as_of_now, records=None)`:`csi1000`+`as_of_now` **必填**(None/非规范即 raise),内部跑 `backfill_forward_returns`(按 as_of_now PIT 截断、审计已有值、逐条 validate、拒重复 as_of)后才计数/渲染——伪造/未截断/look-ahead 的周不能进证据计数。面板段:V14.2 vs V14.3raw + 分歧 + 证据 n/gate 周[读 governance forward_live_weeks_min]+ 前向表现/pending + 数据质量;明标非生产、绝不与 overlay/M6.7 混写。`backfill_forward_returns` 是底层 helper(as_of_now optional),审计路径恒传。纯函数、**无 fetch / 无 EGS / 无文件**。
  - **②b 拆两步降风险**:
    - **②b-1(已起草 2026-06-11)纯编排**:`engine/a_short_regime_pipeline.py` —— `extend_ledger(existing, as_of, calendar, feature_provider)` 编码 cadence fail-closed 工作流(`validate_ledger_for_append`→`plan_append`→`feature_provider(d)` 逐日→`merge_rows`→`build_ledger`→`validate_ledger`;同一函数兼做 bootstrap[empty→252 日]与周度增量;provider 行 as_of 必 ==d)+ `weekly_regime_step`(组合 `build_comparison_record` + ②a 审计 backfill/summarize/render,PIT 截 as_of)。**数据侧注入**(`feature_provider`+`csi1000`),纯函数、**无 fetch / 无 EGS / 无文件写**。
    - **②b-2(已起草 2026-06-12)真 I/O + runner**:`runners/a_short_regime_comparison_runner.py` —— 真 Tushare provider(`make_feature_provider` 包 fetch `stk_limit`+指数+读 IV feed → `compute_regime_daily_features`)、ledger/对比记录/面板**文件持久化**(只写 guard-safe lane,守卫拒 `result/a_short`)、`run_regime_step`(注入数据帧 → `weekly_regime_step` → 落盘,纯可测)、薄真 fetch + CLI(`--confirm-fetch-authorized` 闸)、bootstrap 回填(`--bootstrap`,一次性重 Tushare `执行`,**需用户授权 + TUSHARE_TOKEN**)。**设计偏离(已评估):改用独立 runner 而非 EGS run side-output**——egs_main 生产冻结相邻、风险高;ledger 已摊薄 252 日成本(bootstrap 后每周仅补 ~5 日),独立 runner 与生产完全隔离即可喂对比轨。面板先作 lane 独立 artifact(不动冻结周报渲染)。
- **(可选,远)** 面板接入周报 / EGS side-output / 周度 forward 回填调度——非必须,独立 runner 已闭环。
