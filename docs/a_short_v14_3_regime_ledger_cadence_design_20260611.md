...
- **切片2b-impl,拆两步降风险**:
  ...
  - **②b(待 ②a 审过)接线 + 回填 runner**:EGS run 内 fetch `stk_limit`+指数+读 IV feed → `compute_regime_daily_features` → `validate_ledger_for_append`→`plan_append`→`merge_rows`→`validate_ledger`→写 ledger;把面板段接入周报;bootstrap 回填 runner(一次性重 Tushare `执行`,**需用户授权 + TUSHARE_TOKEN**);周度 forward 回填调度。
