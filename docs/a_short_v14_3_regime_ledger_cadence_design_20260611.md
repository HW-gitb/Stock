...
- **切片2b-impl,拆两步降风险**:
  ...
  - **②b 拆两步降风险**:
    ...
    - **②b-2(待 ②b-1 审过)真 I/O + 接线**:真 Tushare provider(`feature_provider` = fetch `stk_limit`+指数+读 IV feed → `compute_regime_daily_features`)、ledger/对比记录文件持久化、EGS run side-output 调 `weekly_regime_step`、面板接入周报、bootstrap 回填 runner CLI(一次性 `extend_ledger(empty,...)` = 重 Tushare `执行`,**需用户授权 + TUSHARE_TOKEN**)、周度 forward 回填调度。
