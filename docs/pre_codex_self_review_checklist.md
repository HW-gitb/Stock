...
1. **固定 Python + 单一限时入口**:进入执行/修复后只用固定主 Python `C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe`。focused unittest 统一通过 `.tools\run_unittest_with_repo_pythonpath.cmd <unittest args>`（内部固定 300 秒并调用 bounded runner）；full-lane 统一通过第 6 条的 ledger `run`。普通小测试不重复跑 import probe / 完整 A-short preflight；仅改 A-short runtime/dependency/resolver 时额外定向跑一次 preflight。不要试裸 `python`/`py`、PATH/bundled runtime，也不要观察全局 Python PID 猜进度。
2. **先取最便宜的红证据,不机械造新测试**:复现优先级固定为 `existing failing focused test`（已有失败定向测试）> `direct deterministic probe`（直接确定性探针）> `new focused regression test`（新定向回归测试）。已有测试或探针已精确复现 Required 时,不得再写重复红测；纯文档/机械契约修复可用 planted/direct check。只有没有可复现证据,或同类行为/契约缺陷需要永久防复发时,才新增回归测试。红证据只负责证明问题与修复方向,不是为了判断 reviewer 是否“正确”而额外扩展范围。
3. **最小修复 + 条件式共享契约验证**:只改 Required 整类闭合所需文件。共享契约/消费者验证只在改动跨过 schema、source binding、PIT/lineage、序列化、入口或多消费者 seam 时触发；未触及 seam 的局部小修写 `N/A`，不得为了流程完整感追加无关消费者包。若修复循环失败,只回到受影响的红证据/消费者,不投机并发或扩成全 lane。
4. **最终定向绿包 + 主线程 A-F 是固定门**:行为/契约最后一次修改后,集中跑一次 rule-1 focused acceptance；任何后续行为/契约修改都会使它失效。随后由主线程完成本文件 A-F/B2（same-class、ripple、reverse、单一来源、route-doc、pre-flight），再冻结行为/契约 diff。`main-thread A-F` 对每刀必需；独立 agent 不是默认步骤。
5. **独立 agent 只在高风险/不可定界时触发**:仅当 `AGENTS.md` verification-tiering rule 7 的敏感 seam、focused evidence 无法定界、或用户明确要求时才用；小而低影响的修复禁止启动。触发后才应用 **self-review anti-hang contract**：prompt 限 `current-diff-only`，只给当前 requirement、same-class closure、必要文件清单和 `unrelated dirty files` 排除项，默认 `fork_context=false`，输出只允许 `PASS / FAIL` 或 `FAIL: <must-fix list>`，并写 `do not run big packs`。`2-3 minutes` 无结果即 `close and restart once`；`second timeout` 后 `do not keep waiting`，记录 timeout 并走 `main-thread checklist fallback`，不得无限等。
6. **full 只走一次原子命令**:只有 `AGENTS.md` rule 3 触发 full-lane 才执行 `.tools\codex_main_python.ps1 .tools\full_pack_ledger.py run <lane> "<触发理由>" "<focused 证据>" <timeout-seconds> -- <unittest args>`；它在同一条命令内检查可复用指纹、绑定 A-F prepare、限时运行并只对真实 PASS 落账。不得手动拆成 prepare/test/record，不得复跑拿更好看的计数；无 `prepared_fingerprint` 的 legacy green 不能作为收口 PASS。未触发 full 时到此直接交接，reviewer 只独立复跑 focused acceptance，不默认重跑同一全量。
7. **Proof-of-use 必写决策证据**:SESSION_LOG 的 `Pre-Codex self-review` 行要写清红证据采用三种路径中的哪一种、共享 seam 包触发或 `N/A`、main-thread A-F、独立 agent 触发理由或 `N/A`、full trigger 或 `N/A`，以及固定包是否集中一次跑完；这样未来能机器检查有没有把条件步骤偷换成默认慢路径。
