...
1. **固定 Python + 单一限时入口**:进入执行/修复后只用固定主 Python `C:\Users\cnhea\AppData\Local\Programs\Python\Python313\python.exe`。focused unittest 统一通过 `.tools\run_unittest_with_repo_pythonpath.cmd <unittest args>`（默认 300 秒、先断言一次 `jsonschema`，再调用 bounded runner）；已实测超过默认值的窄包才可显式用 `--timeout-seconds <N>`（`301 <= N <= 1300`，在 `Verify` 写明测得理由）。full-lane 统一通过下条的 ledger `run`，且只接受对应 lane 的 discovery pattern。普通小测试不跑完整 A-short preflight；仅改 A-short runtime/dependency/resolver 时额外定向跑一次 preflight。不要试裸 `python`/`py`、PATH/bundled runtime，也不要观察全局 Python PID 猜进度。
2. **红绿节奏保持窄（两层循环，必须遵守）**:先写能复现 Required 的 focused 红测,最小修复到 focused 绿；若自审或定向测试发现问题，只复跑受影响 focused/消费者包，绝不投机并发第二个包。**改共享 guard、AST/conformance 矩阵或其调用点前，先在一次计划里列出所有新增/移动调用点，以及每点的 registry / frozen-coordinate / dying-test 决定；不得靠连续长 conformance 重跑逐个猜。**所有小循环收敛后完成 A-F 并冻结行为/契约 diff；若 `AGENTS.md §Verification tiering and one-full-run rule` 触发 full-lane，只执行一次 `.tools\codex_main_python.ps1 .tools\full_pack_ledger.py run <lane> "<触发理由>" "receipt:<receipt-id>" <timeout-seconds> -- <unittest args>`，自动完成缓存检查、prepare、限时运行和 PASS record。reviewer 只独立复跑 focused acceptance，不默认重跑同一全量。
...
`door=` 是**交接门**，与 lane 无关、无条件：交出前跑一次 `.githooks/pre-commit` 那两个守卫（`tests.test_route_doc_ledger_status_consistency` + `tests.test_doc_governance_guard`），把终端结果贴进来（例：`door=route 14 OK + doc-governance 41 OK`）。理由是这两个守卫在**每一次** commit 都会跑，所以任何要被提交的修复本来就必须先过它们；把结果前移到交接时，等于把提交门变成交接门，**不需要**任何「改动面 → 守卫」映射表（那种表每加一刀就会过期，且不全时反而给出假安全感）。跑不了就写 `door=BLOCKED: <原因>`——这不是占位符，让阻塞可见正是目的；写 `TBD` / `n/a` / 留空会被守卫判红。反例来自 K3-R81：一次 `AGENTS.md` 改动的 `focused=44 OK` 属实但一个文档守卫都没有，红要到审查才暴露。
