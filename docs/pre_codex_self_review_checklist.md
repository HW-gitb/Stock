...
2. **红绿节奏保持窄（两层循环，必须遵守）**:先写能复现 Required 的 focused 红测,最小修复到 focused 绿；若自审或定向测试发现问题，回到本步，只复跑受影响 focused/消费者包并做增量同类自审，绝不立刻重跑 full-lane。所有小循环收敛后，完整完成 A-F 并冻结行为/契约 diff；若 `AGENTS.md §Verification tiering and one-full-run rule` 触发 full-lane，先执行 `.tools\codex_main_python.ps1 .tools\full_pack_ledger.py prepare <lane> "<触发理由>" "<focused 证据>"`，再对该指纹跑唯一一次全量并 `record`。`record` 无 matching prepare 会拒绝；行为/契约再变即回到本步，docs/register/SESSION_LOG-only 不使 prepare 失效。reviewer 只独立复跑 focused acceptance，不默认重跑同一全量。
