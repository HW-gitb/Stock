# 美股中长线系统设计 v3 — `us_long`

> 文件：`docs/us_long_system_design.md`（in-repo 单一设计权威）　原稿日期：2026-06-22　入 repo：2026-06-24（移植自桌面 `v3_us_long_design.md`，桌面稿已退役）
...
> **仓库纪律**：每片 schema-first + tests + 交 Codex 审查；串行不交叉 A 股；不 auto-push（push 仅用户显式指令 + 隐私审计后）。
...
| D6 | 复用而非另造 | 手填 CSV 转换器 / 私密路径守卫 / 跨 LLM 登记表 / 赛道生命周期，**全复用 us_short 现成模式**；引擎用扁平 `engine/us_long_*.py`。 |
...
trades.csv     （可选，执行日志，仅 advisory 对账）
```

- `account` 仅为券商标签（如 IBKR），喂 `positions_review.账户`，非资金字段。
- 首版实现不收集 `available_cash/total_equity` 等资金字段；仅在后续 sizing/组合协调 slice 明确需要时再加。
...
止损/止盈：**结构位 + ATR 波动止损；盈利 50% 后分批止盈 + 上移止损**。价格几何复用 us_short `price_engine` 概念（日线，无分钟级）。

```text
建仓区间 entry_band   ← technical_entry_score + support_level + valuation_risk
止损价   stop_loss    ← support_level + atr_daily + risk_control_score
目标价   target_price ← growth_quality + theme_catalyst + valuation_upside + resistance_level
止盈价   take_profit  ← target_price + resistance_level + unrealized_pct(持仓)
```
...
```text
selected_top20:     运行日期,资金状态,排名,Ticker,公司,行业,细分行业,赛道,赛道状态,现价,选股分,最终分,
                    建议,建仓区间,止损价,3-6个月目标价,止盈价,持仓状态,50%止盈提示,
                    核心利好,核心风险,最终理由,数据源,factor_trace
recommended_build_5:运行日期,资金状态,推荐标签,排名,Ticker,公司,行业,细分行业,赛道,赛道状态,现价,最终分,
                    建仓区间,止损价,3-6个月目标价,止盈价,推荐理由,主要风险,数据源,factor_trace
not_recommended_15: 运行日期,资金状态,排名,Ticker,公司,行业,细分行业,赛道,赛道状态,现价,最终分,建议,
                    不推荐/观察理由,未通过因子,当前值_vs_门槛,重新观察条件,升级为建仓条件,
                    主要风险,数据源,factor_trace
positions_review:   运行日期,资金状态,Ticker,账户,股数,成本价,现价,浮盈比例,50%止盈提示,
                    建议止盈价,建议止损价,持仓动作,动作理由,factor_trace
...
1. **私密路径守卫** `engine/us_long_private_paths.py`（复用 us_short 逻辑）：用 `git check-ignore` 真值判定，results_private 必须 gitignored，否则 fail-closed 拒写。
2. **结果目录排他守卫** `engine/us_long_results_guard.py`：写前校验 results_private 目录**只含白名单 6 文件**，出现草稿/中间/测试/非白名单文件 → 失败。

`.gitignore` 追加 `state/*/results_private/`（授权后改）。
...
engine/
  ...
  us_long_position_review.py    持仓恒复核 + 50%止盈 + 系统止损/止盈
...
Tier 2  机器骨架（样例数据）：schema + 治理 preset + 纯引擎 + runner + CSV 转换器 + 私密/排他守卫。
        用本地样例输入跑出 Top20/Top5/not_recommended/positions_review，明确标"样例·非真推荐"，
        只写 staging/mock 或临时目录，不写 results_private，不联网。
Tier 3  因子闭环强制：field_registry + lineage 账本 + 断路器。
        任一 active/provisional 因子悬空 → 停、不写 results_private；FAIL manifest 写 logs/staging。
...
Tier 5  真实数据最小可运行版：真 Top20/Top5（仍 capital_mode=paper_only / research-only）。
Tier 6  前向观察 + 校准：所有待校准项进登记表，攒 forward 结果定权重/阈值/赛道转正/capital_mode 升级。
```

每片：schema-first + tests + 交 Codex 审查 + 串行不交叉 A 股 + 不 auto-push（仓库纪律）。
工程量预估：Tier 2-3 约 2-3 个 slice；Tier 4-5 约 2-4 个 slice；Tier 6 时间门（≥3-6 月有体感，12 月才谈 ship-gate）。
...
Tier 2（机器骨架·样例数据）= 进行中。用户已授权；按 §16 逐刀建 schema + 治理 preset + 纯引擎 + runner + CSV 转换器 + 私密/排他守卫，样例端到端只写 staging/mock，不写 results_private、不联网。
数据/接入边界仍生效：Tier 2-3 只用本地样例；真实数据 / provider / adapter / DataHub / 联网留 Tier 4+ 单独授权。
仓库纪律：每片 schema-first + tests + 交 Codex 审查；串行不交叉 A 股；不 auto-push。
