...
3. `SR-RESOURCE-001` - Local resource budget and job-spec contracts now exist, but future Phase 7c / runner code-level enforcement remains open before any DataHub table, adapter, runner, or report job implementation.
...
- Mitigation evidence: reviewed slices add `schemas/datahub_local_resource_budget.schema.json`, `docs/datahub_local_resource_budget_contract_20260602.json`, `schemas/datahub_job_spec.schema.json`, and `schemas/examples/datahub_job_spec.example.json`, and route them through `docs/datahub_design.md`, `docs/README.md`, `docs/CURRENT.md`, `AGENTS.md`, and the Phase 7 handoff. The contracts require default `single_slice_incremental` behavior, a declared job spec, and blocks default all-system / all-market / all-lane / full-refresh runs.
- Required next action: before any Phase 7c DataHub table, adapter, runner, or report job implementation, enforce the contract in code / executable job specs with tests that require budget profile, market, lane, `as_of_date` or date window, provider family, artifact type, estimated input / output sizes, cache policy, checkpoint / abort behavior, data boundaries, approval gates, and explicit approval for heavy runs.
- Verification: `tests.schema.test_datahub_local_resource_budget_schema` validates the resource-budget schema / artifact, default slice-first behavior, budget profiles, implementation gates, and scope-creep rejection. `tests.schema.test_datahub_job_spec_schema` validates the job-spec schema / example, one-market / one-lane partitioning, resource estimates, execution policy locks, approval gates, and scope-creep rejection.
