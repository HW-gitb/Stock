...
2. `SR-ALONG-DATA-001` - A-long signal search is blocked by the executed data-integrity audit; the Tushare data-route repair plan exists, but route validation, materialization, and a new reviewed audit are still pending.
...
- Evidence: `research/results/a_long_data_integrity_audit_20260603/audit_report.json` records `decision.audit_status = blocked_missing_required_source`; self-tests passed 6/6, but hard checks are blocked for raw PIT fundamentals / restatement lineage, PIT universe / terminal delisting returns, and total-return / benchmark measurement. `docs/a_long_tushare_data_route_repair_plan_20260603.json` defines the next existing-account Tushare route-validation path and rejects A-short derived financial cache substitution, but it is route-only and performs no data fetch.
...
- Required next action: get the Tushare route-repair plan independently reviewed; after Pass and user `执行`, create a separate small route-validation execution packet for raw PIT fundamentals with `ann_date` / `end_date` lineage, full PIT universe, dividend / total-return treatment, and terminal delisting return lineage. Only after route validation and reviewed materialization may a new audit authorization be created. Do not start signal search from the blocked result or from the route plan.
