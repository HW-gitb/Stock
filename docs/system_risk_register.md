...
2. `SR-ALONG-DATA-001` - A-long signal search is blocked by the executed data-integrity audit and the partial Tushare route-validation result; SW industry membership fields and terminal delisting return coverage still need repair before materialization / re-audit.
...
- Evidence: `research/results/a_long_data_integrity_audit_20260603/audit_report.json` records `decision.audit_status = blocked_missing_required_source`; self-tests passed 6/6, but hard checks were blocked for raw PIT fundamentals / restatement lineage, PIT universe / terminal delisting returns, and total-return / benchmark measurement. `docs/a_long_tushare_route_validation_execution_summary_20260604.json` records a real small Tushare route-validation run with `decision.route_validation_status = partial_or_failed_field_presence`: most required field families passed the small field-presence check, but SW industry membership lacks `index_code` / `con_code` and terminal delisting daily price coverage is still not proven.
...
- Required next action: independently review the route-validation result, then create a narrow repair packet for SW industry membership field mapping / endpoint behavior and terminal delisting return coverage. Only after these route pieces pass may a reviewed incremental materialization packet and then a new audit authorization be created. Do not start signal search from the blocked audit, route plan, or partial route-validation result.
