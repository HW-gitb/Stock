...
Current routing note: the corrected-basis A-share burst preregistration failed a frozen-cohort preflight with `valid_signal_events = 0`; do not run outcome / benchmark-excess for that artifact. The ledger-gated full-universe redesigned test used the patched benchmark-open cache, then failed its registered outcome thresholds with `decision = falsified_or_redesign_required`. Owner audit/spec now mark `a_share_burst_minimal_data` as `redesign_required`. No further redesigned A-share burst test is authorized without a new ledger planned test, user approval, and reviewed preregistration. For US EGS data, the user-approved direction is FMP primary candidate + SEC EDGAR fundamentals audit; `docs/provider_evidence_p1_us_sample_validation_access_approval_20260602.json` authorizes only a reviewed $0 AAPL / MSFT small-sample validation using the existing FMP key plus SEC EDGAR public APIs. `yfinance`, full-market fetch, paid upgrade, provider selection, adapters, DataHub, runner changes, and Phase 7c remain blocked by `SR-PROVIDER-001`.

1. `SR-DATA-004` - Maintenance watch item before affected subsystem promotion; requires real weekly suspend-coverage logs.
2. `SR-PROVIDER-001` - Small FMP + SEC sample validation may proceed only under the 2026-06-02 approval artifact after review; resolve remaining blockers before any new token / trial / paid access / `yfinance` check / full-market fetch / provider selection / adapter / Phase 7c use.
...
- Evidence: the user has stated no US data source is currently enabled and accepted the US EGS source direction of FMP as primary candidate plus SEC EDGAR as fundamentals audit source. `docs/provider_evidence_drift_monitor.md` §15 records that `yfinance` is only an optional low-trust price smoke check and cannot replace EDGAR for fundamentals validation. On 2026-06-02 the user approved a $0 small-sample boundary: use the current FMP account / API key, allow SEC EDGAR public APIs, allow local storage of small samples and validation results, only a few symbols, no full-market download, and no paid upgrade. That boundary is recorded in `docs/provider_evidence_p1_us_sample_validation_access_approval_20260602.json`.
- Accepted calibration: the approval unlocks only a later reviewed AAPL / MSFT FMP existing-key + SEC public-API sample-validation packet. It does not prove FMP coverage, license sufficiency, PIT semantics, local-storage / retention rights beyond the approved small sample, SEC parser feasibility at scale, free-float reconciliation, fallback behavior, provider stability, or production readiness. It also does not authorize `yfinance`, full-market fetch, provider selection, adapters, DataHub tables, runner changes, Phase 7c, or ship-gate claims.
- Required next action: after the approval artifact is reviewed and committed, implement one narrow sample-validation packet that checks `FMP_API_KEY` / `SEC_USER_AGENT` presence without printing secrets, fetches only AAPL / MSFT FMP and SEC EDGAR samples, stores raw rows only under gitignored `provider_samples/us_egs_sample_validation_20260602/`, and writes a tracked no-secret summary. Any new token, trial, paid access, `yfinance` check, full-market download, provider selection, adapter, DataHub table, runner consumption, or Phase 7c use requires separate explicit approval and reviewed decision.
