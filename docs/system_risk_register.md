...
A-short steady alpha re-audit preregistration update: `research/preregistrations/a_short_steady_alpha_reaudit_20260603.json` and `research/ledgers/a_short_steady_alpha_reaudit_program_test_budget_ledger_20260603.json` register one planned same-anchor re-audit of the existing 5d CSI1000 clue, with 20d / CSI300 diagnostics plus multiple-testing, slice, factor, veto, and PIT checks. It is `registered_not_run`: no outcome run, data fetch, EGS rerun, runner change, production claim, ship-gate claim, or full-size use is authorized until independent review passes and the user gives a later execute command.
