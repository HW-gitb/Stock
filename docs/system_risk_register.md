...
A-long data-integrity gate update: the current active alpha-search workstream is A-long only, but A-long signal search is blocked until a reviewed executable data-integrity audit passes hard checks and declares a usable signal-search window. `research/preregistrations/a_long_data_integrity_audit_20260603.json` and `research/ledgers/a_long_data_integrity_audit_program_test_budget_ledger_20260603.json` freeze one planned audit for the monthly as-of schedule, required planted-violation self-tests, PIT fundamentals, restatement as-of logic, PIT universe / survivorship, held delisting / terminal returns, return / benchmark measurement, and time-varying coverage. Coverage shortfalls declare a usable start year rather than globally failing A-long; missing / invalid `ann_date` rows are excluded and reported. This preregistration itself does not run the audit, fetch data, search signals, buy data, expand providers, change runners, authorize production, or produce ship-gate evidence.

1. `SR-DATA-004` - Maintenance watch item before affected subsystem promotion; requires real weekly suspend-coverage logs.
2. `SR-ALONG-DATA-001` - A-long signal search is blocked until the registered data-integrity audit hard checks pass and a usable start year is declared; no signal backtest may start from unvalidated PIT fundamentals / universe / return measurement.
...
- Evidence: A-long alpha candidates depend on fundamentals and long-horizon return / benchmark measurement. After the A-short same-anchor repair showed a measurement-basis artifact can inflate apparent alpha, A-long must not start signal search until the frozen as-of schedule, planted-violation self-tests, PIT fundamentals, restatement as-of logic, PIT universe / survivorship, held delisting / terminal returns, total-return / same-anchor benchmark measurement, and time-varying fundamentals coverage are checked with frozen criteria.
- Accepted calibration: this is not a claim that A-long data is bad. It is a blocker saying the data must be proven clean before signal results can be trusted.
- Required next action: independently review `schemas/a_long_data_integrity_audit_preregistration.schema.json`, `research/preregistrations/a_long_data_integrity_audit_20260603.json`, and `research/ledgers/a_long_data_integrity_audit_program_test_budget_ledger_20260603.json`; after review and user `执行`, run only the frozen data-integrity audit. If any hard check fails or blocks, repair data before signal search; if coverage is thin, declare the usable start year before signal-search preregistration.
- Closure requirement: close only after the reviewed audit execution report records hard checks passing, planted-violation self-tests firing, the usable start year declared, and tests prove no signal search / production / ship-gate claim was authorized by the audit run.
