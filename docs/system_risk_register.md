...
2. `SR-PROVIDER-001` - Use the remaining-blocker plan, fallback playbook, incident-log contract, license / storage / retention review, SEC parser scope contract, SEC field-family mapping contract, FMP PIT semantics contract, FMP price/corporate-action semantics contract, and coverage-count access-packet plan before any provider work: actual coverage-count execution, current terms / production storage rights, field-level PIT validation, actual price adjustment / corporate actions, actual SEC parser implementation and field mapping, minimized fixture policy, incident-log writer, executed fallback / incident behavior, stability evidence, and production-readiness evidence remain required before any new token / trial / paid access / `yfinance` check / FMP or SEC endpoint call / raw-payload parse / fixture generation / return calculation / corporate-action reconciliation / field-mapping or parser implementation / full-market fetch / provider status polling / provider selection / adapter / DataHub / runner consumption / Phase 7c use.
...
- Accepted calibration: the approval, SEC sample run, FMP mapping review, FMP stable retry, remaining-blocker plan, fallback playbook, incident-log contract, license / storage / retention review, SEC parser scope contract, SEC field-family mapping contract, FMP PIT semantics contract, FMP price/corporate-action semantics contract, and coverage-count access-packet plan prove only a narrow two-symbol access / response-shape sample plus routing, default-deny design, future-record contract, existing-evidence rights classification, no-access audit-scope artifact, no-access SEC field-family / cross-check gates, no-access FMP historical-use semantics gates, no-access EOD return / liquidity gates, and no-access coverage packet requirements. The SEC scope packet narrows audit role and lineage gates but does not prove SEC parser feasibility at scale, PIT normalization, broader reconstruction, free-float reconciliation, coverage, production storage rights, parser implementation, FMP coverage, license sufficiency, field-level PIT validation, price adjustment, corporate-action reconciliation, incident-log writer behavior, executable fallback behavior, provider stability, or production readiness. The SEC field-family mapping packet narrows audit family, taxonomy/context, FMP cross-check, and fixture blocking rules but does not prove actual SEC field mapping, minimized fixtures, taxonomy correctness, raw-payload retention policy, or parser implementation. The FMP PIT semantics packet narrows missing-date / latest-only / lineage blocking rules but does not prove actual FMP field-level PIT, revision behavior, restatement behavior, or historical DataHub eligibility. The FMP price/corporate-action packet narrows adjustment-mode / split / dividend / delisting / missing-session blocking rules but does not prove actual adjusted returns, total-return semantics, liquidity validity, inactive coverage, or historical DataHub eligibility. The coverage packet plan narrows what a later access request must contain but does not prove coverage counts or authorize execution. They also do not authorize `yfinance`, FMP or SEC endpoint calls, coverage-count execution, raw payload parsing, fixture generation, return calculation, corporate-action reconciliation, field-mapping or parser implementation, full-market fetch, provider status polling, provider selection, adapters, DataHub tables, runner consumption, Phase 7c, or ship-gate claims.
- Required next action: do not rerun or broaden the sample silently. Use `docs/provider_evidence_p1_us_remaining_blocker_resolution_plan_20260602.json`, `docs/provider_evidence_p1_us_fallback_incident_stability_playbook_20260602.json`, `docs/provider_evidence_p1_us_incident_log_contract_20260602.json`, `docs/provider_evidence_p1_us_license_storage_retention_review_20260602.json`, `docs/provider_evidence_p1_us_sec_edgar_audit_parser_scope_contract_20260602.json`, `docs/provider_evidence_p1_us_sec_edgar_field_family_mapping_contract_20260602.json`, `docs/provider_evidence_p1_us_fmp_pit_observed_date_semantics_contract_20260602.json`, `docs/provider_evidence_p1_us_fmp_price_adjustment_corporate_action_semantics_contract_20260602.json`, and `docs/provider_evidence_p1_us_coverage_count_access_packet_plan_20260602.json` as routing sources; any coverage-count execution, current terms web/legal review, FMP or SEC endpoint call, raw-payload parse, fixture generation, return calculation, corporate-action reconciliation, field-mapping or parser implementation, new token, trial, paid access, `yfinance` check, full-market download, provider status polling, provider selection, adapter, DataHub table, runner consumption, fallback execution, incident-log writer implementation, or Phase 7c use requires separate explicit approval and reviewed decision. Remaining P1 provider work must address actual coverage counts / current terms and production storage rights / actual FMP PIT row validation / actual FMP price adjustment and corporate-action validation / actual SEC parser implementation and field mapping / minimized fixtures / incident-log writer / executable fallback / stability evidence before implementation can consume FMP.
