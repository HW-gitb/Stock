...
2. `SR-ALONG-DATA-001` - A-long signal search is blocked by the executed data-integrity audit; route validation, route-gap repair, and thin-slice materialization now have small-sample / thin-slice evidence, but a new data-integrity audit is still pending.
...
### SR-ALONG-DATA-001 - A-long signal search blocked until data-integrity audit passes

- Severity: P1
- Status: open
- Owner phase: Phase 7a A-long alpha validation / data readiness
- Evidence: `research/results/a_long_data_integrity_audit_20260603/audit_report.json` records `decision.audit_status = blocked_missing_required_source`; self-tests passed 6/6, but hard checks were blocked for raw PIT fundamentals / restatement lineage, PIT universe / terminal delisting returns, and total-return / benchmark measurement. `docs/a_long_tushare_route_validation_execution_summary_20260604.json` records the first small Tushare route-validation run as partial. `docs/a_long_tushare_route_gap_repair_execution_summary_20260604.json` then records `decision.gap_repair_status = passed_route_gap_field_presence_only`, repairing the two failed route pieces at tiny-sample field-presence level only. `docs/a_long_tushare_incremental_materialization_execution_summary_20260604.json` records `passed_thin_slice_materialization_shape` for the bounded 2022-2023 three-symbol materialization run, with 29/29 planned calls and all table rollups passing thin-slice shape.
- Accepted calibration: this is not an alpha failure. It means the current local data route cannot yet support trustworthy A-long alpha search.
- Required next action: independently review the thin-slice materialization execution summary, then create a separate reviewed data-integrity audit authorization. Do not start signal search from the blocked audit, route plan, route-validation result, route-gap repair result, materialization packet, or thin-slice summary.
- Closure requirement: close only after a reviewed audit execution report records hard checks passing, planted-violation self-tests firing, a usable start year declared, and tests prove no signal search / production / ship-gate claim was authorized by the audit run.
