...
A-long data-integrity audit execution update: after Claude review and the user's `执行`, `runners/a_long_data_integrity_audit.py` ran local-cache-only and wrote `research/results/a_long_data_integrity_audit_20260603/audit_report.json`. The runner's six planted-violation self-tests all passed, but the real audit is `blocked_missing_required_source`: the local cache has derived A-short `financial_*.pkl` files that omit `ann_date` / `end_date`, no raw PIT `income` / `balancesheet` / `fina_indicator` lineage, no full 2018-2025 PIT universe proof, no dividend / total-return source, and no terminal delisting return lineage. The ledger spent the one data-integrity gate unit as `spent_voided_by_data_integrity_failure`. A-long signal search remains blocked.

1. `SR-DATA-004` - Maintenance watch item before affected subsystem promotion; requires real weekly suspend-coverage logs.
2. `SR-ALONG-DATA-001` - A-long signal search is blocked by the executed data-integrity audit until the data route is repaired and a new reviewed audit passes hard checks plus declares a usable start year.
...
- Evidence: `research/results/a_long_data_integrity_audit_20260603/audit_report.json` records `decision.audit_status = blocked_missing_required_source`; self-tests passed 6/6, but hard checks are blocked for raw PIT fundamentals / restatement lineage, PIT universe / terminal delisting returns, and total-return / benchmark measurement. The local A-short derived financial caches cannot certify A-long PIT fundamentals.
...
- Required next action: repair or replace the A-long data route for raw PIT fundamentals with `ann_date` / `end_date` lineage, full PIT universe, dividend / total-return treatment, and terminal delisting return lineage; then create a new reviewed audit authorization before rerunning any data audit. Do not start signal search from the blocked result.
