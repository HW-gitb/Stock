...
A-long thin-slice materialization-packet update: `docs/a_long_tushare_incremental_materialization_packet_20260604.json` records the next reviewed execution packet, and `runners/a_long_tushare_incremental_materialization_packet.py` is the future runner. The packet fixes only a 2022-2023 three-symbol thin slice (`000001.SZ`, `600519.SH`, `000666.SZ`), CSI300 / CSI1000 benchmarks, 29 planned calls / max 32, gitignored raw storage under `data/a_long/raw/tushare/materialization_thin_slice_20260604/`, and checkpoint resume. It has not executed any Tushare call, read raw rows, written the future summary, rerun audit, searched signals, or authorized full materialization / DataHub / production / ship-gate / full-size use.

1. `SR-DATA-004` - Maintenance watch item before affected subsystem promotion; requires real weekly suspend-coverage logs.
2. `SR-ALONG-DATA-001` - A-long signal search is blocked by the executed data-integrity audit; route validation and route-gap repair now have small-sample field evidence, but reviewed materialization and a new data-integrity audit are still pending.
...
- Required next action: independently review the thin-slice materialization packet, then execute only that packet after a clean review and user `执行`. Only after the thin-slice materialization summary is reviewed may a new data-integrity audit authorization be created. Do not start signal search from the blocked audit, route plan, route-validation result, route-gap repair result, or materialization packet.
