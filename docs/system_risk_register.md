...
- Accepted calibration: the approval, SEC sample run, FMP mapping review, FMP stable retry, remaining-blocker plan, fallback playbook, incident-log contract, license / storage / retention review, SEC parser scope contract, SEC field-family mapping contract, FMP PIT semantics contract, FMP price/corporate-action semantics contract, coverage-count access-packet plan, bounded coverage-count execution summary, missing key-metrics resolution plan, 2026-06-03 validation authorization / execution packet, and validation execution summary prove only narrow two-symbol access / response-shape, a five-active-symbol FMP stable coverage smoke, a no-access missing-field route, user authorization and execution of a bounded validation packet, and response-shape / field-presence clues for that five-symbol sample. They do not prove inactive / delisted coverage, current terms, production storage rights, direct ratio availability, safe derived ratios, field-level PIT validation at scale, price adjustment at scale, corporate-action endpoint templates, corporate-action reconciliation at scale, SEC parser feasibility at scale, minimized fixtures, executable fallback behavior, provider stability, provider selection, DataHub / runner consumption, Phase 7c, production readiness, alpha validation, or ship-gate evidence.
- Required next action: do not rerun or broaden the sample silently. Use the remaining-blocker plan, fallback playbook, incident-log contract, license-storage review, SEC parser / field-family contracts, FMP PIT / price semantics contracts, coverage-count plan / approval / summary, missing key-metrics resolution plan, validation authorization packet, validation execution packet, and validation execution summary as routing sources. Any scope beyond the executed packet, plus any current terms web/legal review, FMP split / dividend endpoint call, fixture generation, derivation implementation, return calculation, corporate-action reconciliation, field-mapping or parser implementation, new token, trial, paid access, `yfinance` check, full-market download, provider status polling, provider selection, adapter, DataHub table, runner consumption, fallback execution, incident-log writer implementation, or Phase 7c use requires separate explicit approval and reviewed decision. Remaining P1 provider work must address current terms and production storage rights / actual FMP PIT row validation / actual FMP price adjustment and corporate-action validation / actual SEC parser implementation and field mapping / minimized fixtures / derivation lineage / incident-log writer / executable fallback / stability evidence before implementation can consume FMP.
