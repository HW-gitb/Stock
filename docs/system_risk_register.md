...
3. `SR-RESOURCE-001` - Local resource budget and job-spec contracts now exist, including market/lane consistency and reviewed lane-concurrency bounds, but future Phase 7c / runner code-level enforcement remains open before any DataHub table, adapter, runner, or report job implementation.
...
- Required next action: before any Phase 7c DataHub table, adapter, runner, or report job implementation, enforce the contract in code / executable job specs with tests that require budget profile, market/lane consistency, `as_of_date` or date window, provider family, artifact type, estimated input / output sizes, cache policy, checkpoint / abort behavior, data boundaries, approval gates, reviewed concurrency limits, and explicit approval for heavy runs.
- Verification: `tests.schema.test_datahub_local_resource_budget_schema` validates the resource-budget schema / artifact, default slice-first behavior, `max_concurrent_lanes <= 2`, budget profiles, implementation gates, and scope-creep rejection. `tests.schema.test_datahub_job_spec_schema` validates the job-spec schema / example, one-market / one-lane partitioning, market/lane mismatch rejection, resource estimates, execution policy locks, approval gates, and scope-creep rejection.
