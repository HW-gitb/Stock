...
Current routing note: the corrected-basis A-share burst preregistration has now failed a frozen-cohort preflight with `valid_signal_events = 0`. Do not run outcome / benchmark-excess calculation for that preregistration. A ledger-gated full-universe redesigned preregistration has been drafted and appended as a planned ledger test, but it is pending Claude review and is not run-authorized.

1. `SR-RESEARCH-001` - Current corrected-basis A-share burst preregistration is spent / failed by zero valid signal events; full-universe redesigned preregistration is pending review, then pre-outcome preflight only.
...
- Progress: `research/preregistrations/a_share_minimal_data_burst_full_universe_redesign_20260531.json` and a corresponding planned ledger test have been drafted for the full EGS candidate-surface redesign. They are pending Claude review and do not authorize any run; after review, the first allowed step is pre-outcome event-count / input-integrity preflight only.
