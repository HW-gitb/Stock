...
- Evidence: the user has stated no US data source is currently enabled and accepted the US EGS source direction of FMP as primary candidate plus SEC EDGAR as fundamentals audit source. `docs/provider_evidence_drift_monitor.md` §15 records that `yfinance` is only an optional low-trust price smoke check and cannot replace EDGAR for fundamentals validation. On 2026-06-02 the user approved a $0 small-sample boundary: use the current FMP account / API key, allow SEC EDGAR public APIs, allow local storage of small samples and validation results, only a few symbols, no full-market download, and no paid upgrade. That boundary is recorded in `docs/provider_evidence_p1_us_sample_validation_access_approval_20260602.json`. The sample packet `docs/provider_evidence_p1_us_sample_validation_summary_20260602.json` ran 17 endpoint calls, kept raw payloads under gitignored `provider_samples/us_egs_sample_validation_20260602/`, logged no secrets, found SEC EDGAR company-ticker mapping / submissions / companyfacts success for AAPL and MSFT, and found HTTP 403 legacy-endpoint errors on all sampled FMP v3 endpoint families. The docs-only mapping review `docs/provider_evidence_p1_us_fmp_current_endpoint_mapping_review_20260602.json` identifies current FMP stable endpoint candidates for the failed endpoint families, but does not live-validate them.
- Accepted calibration: the approval, sample run, and mapping review do not prove FMP coverage, license sufficiency, PIT semantics, local-storage / retention rights beyond the approved small sample, SEC parser feasibility at scale, free-float reconciliation, fallback behavior, provider stability, or production readiness. The FMP sampled legacy/v3 endpoints did not validate, and the stable endpoint candidates are not live-validated yet. The run and mapping review also do not authorize `yfinance`, full-market fetch, provider selection, adapters, DataHub tables, runner consumption, Phase 7c, or ship-gate claims.
- Required next action: after review/commit of the mapping artifact, the next narrow candidate is updating or parameterizing the standalone sample-validation runner for an AAPL / MSFT stable-endpoint retry under the existing $0 / existing-key / no-secret / ignored-raw-sample boundary. Any new token, trial, paid access, `yfinance` check, full-market download, provider selection, adapter, DataHub table, runner consumption, or Phase 7c use requires separate explicit approval and reviewed decision.
