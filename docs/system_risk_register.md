...
- Required next action: independently review the route-gap repair result, then create a reviewed incremental materialization packet that keeps scope bounded and writes raw rows only under gitignored storage. Only after materialization may a new data-integrity audit authorization be created. Do not start signal search from the blocked audit, route plan, route-validation result, or route-gap repair result.
