...
- A-long signal search is blocked by the executed data-integrity gate. `research/results/a_long_data_integrity_audit_20260603/audit_report.json` passed runner self-tests but found missing required sources. `docs/a_long_tushare_route_validation_execution_summary_20260604.json` executed a small existing-account Tushare packet; `docs/a_long_tushare_route_gap_repair_execution_summary_20260604.json` then passed the two failed route pieces at tiny-sample field level: SW industry membership maps through current `index_member_all` fields (`ts_code` / `l2_code` / `l2_name` / `in_date` / `out_date`), and older delisted terminal return inputs are available for sample `000666.SZ`. `docs/a_long_tushare_incremental_materialization_packet_20260604.json` is the next bounded 2022-2023 materialization packet, but it has not executed. Hard data findings are not alpha results; run the reviewed thin slice and re-audit before any A-long signal search.
