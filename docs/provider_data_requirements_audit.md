...
2026-06-04 A-long exception note: if an already-delisted fixed-panel A-share symbol has a reviewed no-source result for SW membership / coarse industry in the current Tushare route, it may be excluded only from industry-neutralization denominators and must remain in universe, returns, terminal delisting return, risk, and coverage reporting. Active-symbol missing industry, silent fill, or dropping a delisted name from returns/risk remains forbidden.
...
- A-long signal search is blocked by the data-readiness gate and by the user's A-share main-board-only scope. The old 2018-2025 fixed panel included `300750.SZ` (ChiNext), so it is historical only and cannot be used as the current audit-rerun or signal-search basis. Current A-long materialization / audit code rejects non-main-board active symbols, and the current packet boundary replaces `300750.SZ` with `600887.SH`. The corrected `000666.SZ` supplement found no usable SW membership source in the current Tushare route. A reviewed main-board-only replacement panel plus a passing data-integrity audit are required before any A-long signal search.
