...
- A-long signal search is blocked by the executed data-integrity gate and the partial Tushare route validation. `research/results/a_long_data_integrity_audit_20260603/audit_report.json` passed runner self-tests but found missing required sources. `docs/a_long_tushare_route_validation_execution_summary_20260604.json` then executed a small existing-account Tushare packet: calendar, stock_basic L+D PIT universe shape, raw `income` / `balancesheet` / `cashflow` / `fina_indicator` `ann_date` + `end_date` fields, daily / adj_factor / dividend, and CSI300 benchmark fields passed the small field-presence check. Remaining route blockers are SW industry membership fields (`index_code` / `con_code`) and terminal delisting daily price coverage. Hard data findings are not alpha results; repair and re-audit the data route before any A-long signal search.
