...
`schemas/a_short_weekly_report.schema.json` 的 `run_lineage` 以 `run_id`、`candidate_digest`、`stage_status`（`complete` / `degraded_no_new_entries` / `partial_holdings_only`）、`analysis_input`、`selection_bucket`、`iv_feed`、`iv_feed_status`、`account_ref`、`account_status`、`sizing_mode`、`account_snapshot`、`price_freshness` 绑定同一批候选、账户 bundle 快照与价格时钟，并附 `iv_freshness` / `market_regime`。`complete` 必须绑定 `iv_feed_status=ready`；IV 非 ready 时只允许降级或仅持仓管理，`iv_feed` 必须为 null，且消费方不得读取旧 feed。`run_id` / `candidate_digest` 必须来自通过完整契约校验且已发布 `official_publish.json` 的 EGS `analysis_input`，不得把目录日期或路径相似当成同一次运行。账户摘要只暴露 snapshot id/digest、facts/decision 日期、持仓数与 integrity 状态；`market_regime` 同时记录 raw source 与 production effective 状态；unknown/missing 必须 effective=`shock`/震荡期。

不变量(`validate_weekly_report`):`n_stocks==len(reports)`;`boundary` 全 false;每张 `report.as_of==weekly.as_of`;`ts_code` 不重复;逐票 §4 不变量;ready 时读入的 feed 过 `validate_feed_summary_consistency`，非 ready 时不得传入或读取 feed；`run_lineage` 一致(account_status=absent ⇒ sizing_mode=observation_only_no_account、sized ⇒ provided)。最终发布以 `weekly_m67.json`、对应 Markdown、`weekly_m67.receipt.json` 与可选 holding-ratchet 为同一事务：先全部生成并校验，再原子替换；任一替换失败即回滚，禁止只留下部分新文件。三种 publishable receipt（`complete` / `degraded_no_new_entries` / `partial_holdings_only`）均绑定同一 `run_id` / digest、JSON/Markdown `outputs_digest` 与确定性 Markdown；正式 evidence 消费方仍只能通过 complete-only 严格验证器，运维/持仓健康面通过 operation loader 读取实际 stage，不得硬编码 complete。post-run sidecar-health 也是正式消费者：PowerShell 显式传本轮 `requested/skipped/not_run` 意图；该意图优先于目录旧态，`skipped/not_run` 时不得读取旧三件套或旧 pipeline manifest。只有本轮 requested 时，Python 才验证 canonical 三件套并从 snapshot 派生实际 publishable stage、run identity 与 receipt digest；failed receipt、缺件或篡改分别记录诚实的 failed/unavailable，caller 自报不得覆盖。请求 M6.7 后的前置或 pipeline 失败由 wrapper 写 `failed` receipt 并以非零退出码结束。M6.7 是唯一正式运维周报，旧 analysis 报告仅作 research 输入。
...
## 14A reader compatibility and consumer-role pin

The publish writer is the current weekly/report schema gate. The published-bundle reader must not retroactively apply the current weekly/report schema to immutable historical JSON; it validates the receipt schema plus exact JSON/Markdown bytes, receipt identity, path/revision binding, deterministic rendering, stage/content contract, effect ledger, and output digests. A pre-14A complete bundle may use an in-memory `iv_feed_status=ready` compatibility view only when its non-empty IV reference and aligned freshness evidence prove that status; no historical bytes are rewritten, and degraded/partial states never use that inference. Formal evidence consumers are registered separately from operational consumers: the former may call only `validate_published_weekly_bundle`, while launcher/health surfaces may call only `validate_published_weekly_operation_bundle`.
