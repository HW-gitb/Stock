...
`runners/weekly_screening.ps1` 仅在 live canonical 运行时先调用 `runners/a_short_factor_comparison_v2_cache_build.py`。该文件保留历史名称，但现在是 A短 v2/P5/P2/P3 **唯一**日线 provider writer：它先加载现有私密 cache，再从四条已冻结 capture/ledger 计算真正缺失的 symbol/window；在固定 91-call ceiling 内固定 v2 优先、P5 其次、P2/P3 最后，超额请求只记 `deferred_due_to_budget` 留待下周，不再因全部 symbol 的理论上限让所有 consumer 一起失败。每个选中 symbol 仍只调用现有获批 Tushare 的 daily/adj_factor/stk_limit，原子合并后写回同一个 gitignored `daily_cache.json`；缺失复权因子写为 `provider_missing`，停牌/缺行保留不可评价状态，绝不前填或默认 observed。
