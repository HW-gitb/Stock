...
Phase 7b-2 has started with three P1 snapshots: `docs/provider_evidence_p1_us_public_sources_20260528.json` for SEC EDGAR submissions/XBRL, SEC ticker files, Nasdaq symbol directory, and GICS methodology; `docs/provider_evidence_p1_us_market_data_candidates_20260528.json` for Massive / Polygon and Norgate US market-data candidate documentation; and `docs/provider_evidence_p1_us_authorization_cost_stability_20260528.json` for Massive / Polygon and Norgate pricing, API-key / subscription / trial access, license / EULA, export / retention, stability constraints, and Norgate current-fundamentals latest-only limitations. These snapshots make P1 `partial`, not implementation-ready. Phase 7b-2 still needs to continue actual provider capability evidence in the P1 to P4 order above: coverage counts, direct benchmark return sources, issuer-level PIT GICS membership, fundamentals / filing observed-date provider candidates beyond latest-only sources, fallback behavior, incident / stability evidence, and limitations before moving to P2-P4.
