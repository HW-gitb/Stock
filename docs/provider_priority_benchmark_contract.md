...
Phase 7b-2 has six P1 snapshots: `docs/provider_evidence_p1_us_public_sources_20260528.json` for SEC EDGAR submissions/XBRL, SEC ticker files, Nasdaq symbol directory, and GICS methodology; `docs/provider_evidence_p1_us_market_data_candidates_20260528.json` for Massive / Polygon and Norgate US market-data candidate documentation; `docs/provider_evidence_p1_us_authorization_cost_stability_20260528.json` for Massive / Polygon and Norgate pricing, API-key / subscription / trial access, license / EULA, export / retention, stability constraints, and Norgate current-fundamentals latest-only limitations; `docs/provider_evidence_p1_us_benchmark_gics_candidates_20260528.json` for S&P DJI / Nasdaq / FTSE Russell / LSEG benchmark methodology and S&P Global / MSCI GICS candidate evidence; `docs/provider_evidence_p1_us_fundamentals_observed_date_candidates_20260528.json` for SEC EDGAR public reconstruction, Intrinio filing fundamentals, FMP SEC filings / as-reported statements, and Nasdaq Data Link / Sharadar fundamentals candidate context; and `docs/provider_evidence_p1_us_coverage_fallback_incident_candidates_20260528.json` for coverage, fallback, incident / status, and license / sample-row validation evidence. `schemas/provider_p1_readiness_review.schema.json` and `docs/provider_evidence_p1_us_readiness_review_matrix_20260529.json` synthesize those snapshots field-by-field. `schemas/provider_p1_access_decision_plan.schema.json` and `docs/provider_evidence_p1_us_access_decision_sample_validation_plan_20260531.json` convert the matrix blockers into a plan-only access / sample-validation boundary with approved spend = 0. `schemas/provider_p1_sample_validation_access_approval.schema.json` and `docs/provider_evidence_p1_us_sample_validation_access_approval_20260602.json` then record the user's narrow $0 AAPL / MSFT FMP existing-key + SEC public-API sample approval. P1 remains `partial` and not implementation-ready; provider contact, new token / trial / paid access, `yfinance`, full-market data fetch, provider selection, P2-P4 queue transition, or Phase 7c consumption still require later reviewed decisions.
