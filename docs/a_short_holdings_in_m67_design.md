...
**状态**：S1-S3 全部实现并提交(S1 持仓恒列入 + 三档 Tier;S2 持仓语义接入;S3a 系统算止损/止盈被动显示;S3b R1-R4b 主动持仓管理:持仓处置/减仓价/清仓价/到价提示/移保本/跨周 ratchet),4.2 缺口数据接入全 5 轮亦完成。详见 `docs/CURRENT.md` §0 + `docs/README.md` A-short 路由。本文件为 S1 的 in-repo 权威设计摘要(历史切片记录)。
